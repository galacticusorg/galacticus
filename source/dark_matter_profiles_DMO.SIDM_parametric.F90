...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  !!{RST
  An implementation of dark matter halo profiles for self-interacting dark matter following the "SIDM parametric" model of :cite:t:`yang_parametric_2024`.
  !!}
  ...
  !![
  <darkMatterProfileDMO name="darkMatterProfileDMOSIDMParametric" docformat="rst">
    <description>
    Dark matter halo profiles for self-interacting dark matter following the "SIDM parametric" model of :cite:t:`yang_parametric_2024` are built via the :galacticus-class:`massDistributionSIDMParametricProfile` class.
    </description>
  </darkMatterProfileDMO>
  !!]
  type, extends(darkMatterProfileDMOClass) :: darkMatterProfileDMOSIDMParametric
     !!{RST
     A dark matter halo profile class implementing profiles for self-interacting dark matter following the "SIDM parametric" model of :cite:t:`yang_parametric_2024`.
     !!}
     private
  ...
  interface darkMatterProfileDMOSIDMParametric
     !!{RST
     Constructors for the ``sidmParametric`` dark matter halo profile class.
     !!}
...
contains
  ...
  function sidmParametricConstructorParameters(parameters) result(self)
    !!{RST
    Constructor for the ``sidmParametric`` dark matter halo profile class which takes a parameter set as input.
    !!}
    ...
    <inputParameter docformat="rst">
      ...
      <description>
      The value :math:`\beta` in a SIDMParametric-model mass distribution.
      </description>
  ...
  function sidmParametricConstructorInternal(beta,darkMatterParticle_,darkMatterHaloScale_) result(self)
    !!{RST
    Internal constructor for the ``sidmParametric`` dark matter profile class.
    !!}
  ...
  subroutine sidmParametricDestructor(self)
    !!{RST
    Destructor for the ``sidmParametric`` dark matter halo profile class.
    !!}
    implicit none
  ...
  function sidmParametricGet(self,node,weightBy,weightIndex) result(massDistribution_)
    ...
    if (darkMatterProfile%floatRank0MetaPropertyGet(self%radiusCoreSIDMID) == 0.0d0) then
