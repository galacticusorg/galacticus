...
module Merger_Tree_Read_Importers
  ...
  type :: importerUnits
   ...
   contains
     !![
     <methods>
       <method description="Multiply by another \source{importerUnits} object." method="operator(*)" />
...
contains
  ...
  function importerUnitsMultiply(units1,units2)
    !!{
    Multiply to \source{importerUnits} objects.
    !!}
    implicit none
  ...
  function importerUnitsExponentiate(units1,exponent)
    !!{
    Exponentiate \source{importerUnits} objects.
    !!}
    implicit none
  ...
  logical function importerUnitsAreEqual(units1,units2)
    !!{
    Test whether two \source{importerUnits} objects are equal.
    !!}
  ...
  logical function importerUnitsAreNotEqual(units1,units2)
    !!{
    Test whether two \source{importerUnits} objects are not equal.
    !!}
    implicit none
    class(importerUnits), intent(in   ) :: units1
    type (importerUnits), intent(in   ) :: units2
