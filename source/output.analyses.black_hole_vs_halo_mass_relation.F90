...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <outputAnalysis name="outputAnalysisBlackHoleVsHaloMassRelation">
    <description>
      ...
      redshiftIntervalN} where {\normalfont \ttfamily N} is an integer. Each such group specifies the black hole mass--halo mass
      ...
      \begin{itemize}
       ...
       \item dataset {\normalfont \ttfamily massBlackHole}: black hole mass in units of $\mathrm{M}_\odot$;
       \item dataset {\normalfont \ttfamily massBlackHoleError}: uncertainty in black hole mass in units of $\mathrm{M}_\odot$;
       \item dataset {\normalfont \ttfamily massBlackHoleScatter}: scatter in black hole mass in units of dex;
       \item dataset {\normalfont \ttfamily massBlackHoleScatterError}: uncertainty in scatter in black hole mass in units of dex;
  ...
  type, extends(outputAnalysisClass) :: outputAnalysisBlackHoleVsHaloMassRelation
     ...
     double precision                            , allocatable, dimension(:  ) :: systematicErrorPolynomialCoefficient            , systematicErrorMassHaloPolynomialCoefficient, &
          &                                                                       massBlackHoleLogarithmicTarget                  , massBlackHoleScatterTarget
     double precision                            , allocatable, dimension(:,:) :: massBlackHoleLogarithmicCovarianceTarget        , massBlackHoleScatterCovarianceTarget
     type            (varying_string            )                              :: analysisLabel                                   , fileNameTarget
   contains
     final     ::                  blackHoleVsHaloMassRelationDestructor
     procedure :: analyze       => blackHoleVsHaloMassRelationAnalyze
     procedure :: finalize      => blackHoleVsHaloMassRelationFinalize
     procedure :: reduce        => blackHoleVsHaloMassRelationReduce
     procedure :: logLikelihood => blackHoleVsHaloMassRelationLogLikelihood
  ...
  end interface outputAnalysisBlackHoleVsHaloMassRelation
...
contains
  ...
  function blackHoleVsHaloMassRelationConstructorParameters(parameters) result (self)
    ...
    implicit none
    type            (outputAnalysisBlackHoleVsHaloMassRelation)                              :: self
  ...
  function blackHoleVsHaloMassRelationConstructorInternal(fileNameTarget,redshiftInterval,likelihoodBinsAutomatic,likelihoodBins,likelihoodNormalize,computeScatter,systematicErrorPolynomialCoefficient,systematicErrorMassHaloPolynomialCoefficient,cosmologyParameters_,cosmologyFunctions_,darkMatterProfileDMO_,virialDensityContrast_,outputTimes_) result (self)
    ...
    use :: Galactic_Filters                      , only : filterList                                                    , galacticFilterAll                              , galacticFilterHaloIsolated, galacticFilterBlackHoleMass
    ...
    double precision                                                 , allocatable  , dimension(:  ) :: massHalo                                                     , massBlackHoleLogarithmicTarget                         , &
         &                                                                                              massBlackHoleTarget                                          , massBlackHoleScatterTarget                             , &
         &                                                                                              massBlackHoleErrorTarget                                     , massBlackHoleScatterErrorTarget                        , &
         &                                                                                              massHaloLogarithmic
    double precision                                                 , allocatable  , dimension(:,:) :: outputWeight                                                 , massBlackHoleLogarithmicCovarianceTarget               , &
         &                                                                                              massBlackHoleScatterCovarianceTarget
    type            (galacticFilterBlackHolemAss                    ), pointer                       :: galacticFilterBlackHoleMass_
    ...
    double precision                                                 , parameter                     :: massBlackHoleLimit                                    = 1.0d0 ! A minimal black hole mass to consider (to avoid attempting to analyze black holes with non-positive masses).
    double precision                                                 , parameter                     :: massStellarLimit                                      = 1.0d0 ! A minimal stellar mass to consider (used in setting up volume weights).
    ...
    call groupRedshift%readDataset  ('massBlackHole'            ,massBlackHoleTarget            )
    ...
    allocate(massBlackHoleLogarithmicCovarianceTarget(massHaloCount,massHaloCount))
    allocate(massBlackHoleScatterCovarianceTarget    (massHaloCount,massHaloCount))
    massHaloLogarithmic                     =log10(massHalo           )
    massBlackHoleLogarithmicTarget          =log10(massBlackHoleTarget)
    massBlackHoleLogarithmicCovarianceTarget=0.0d0
    massBlackHoleScatterCovarianceTarget    =0.0d0
    do iBin=1,massHaloCount
       massBlackHoleLogarithmicCovarianceTarget(iBin,iBin)=+(                                      &
            &                                              +massBlackHoleErrorTarget        (iBin) &
            &                                              /massBlackHoleTarget             (iBin) &
            &                                              /log(10.0d0)                            &
            &                                             )**2
       massBlackHoleScatterCovarianceTarget    (iBin,iBin)=+(                                      &
            &                                               +massBlackHoleScatterErrorTarget(iBin) &
            &                                             )**2
    end do
    ...
    if (size(self%likelihoodBins) > 0) then
       ...
       do iBin=1,massHaloCount
          if (.not.any(self%likelihoodBins == iBin)) then
             massBlackHoleLogarithmicTarget          (iBin     )=0.0d0
             massBlackHoleScatterTarget              (iBin     )=0.0d0          
             massBlackHoleLogarithmicCovarianceTarget(iBin,iBin)=covarianceLarge
             massBlackHoleScatterCovarianceTarget    (iBin,iBin)=covarianceLarge
          end if
       end do
    end if
    allocate(self%massBlackHoleLogarithmicTarget          ,source=massBlackHoleLogarithmicTarget          )
    allocate(self%massBlackHoleScatterTarget              ,source=massBlackHoleScatterTarget              )
    allocate(self%massBlackHoleLogarithmicCovarianceTarget,source=massBlackHoleLogarithmicCovarianceTarget)
    allocate(self%massBlackHoleScatterCovarianceTarget    ,source=massBlackHoleScatterCovarianceTarget    )
    ! Build a filter which selects central galaxies with black hole mass above some coarse lower limit suitable for this sample.
    allocate(galacticFilterBlackHoleMass_     )
    ...
    filters_%next%filter_ => galacticFilterBlackHoleMass_
    !![
    <referenceConstruct object="galacticFilterBlackHoleMass_" constructor="galacticFilterBlackHoleMass(massThreshold=massBlackHoleLimit)"/>
    ...
    if (computeScatter) then
       analysisLabel            =var_str('blackHoleHaloMassRelationScatter')//labelTarget//'z'//redshiftInterval
       weightPropertyLabel      =var_str('massBlackHoleLog10Scatter'       )
       weightPropertyDescription=var_str('σ_{log₁₀(Black hole mass/M☉)}'   )
       allocate(outputAnalysisScatterFunction1D :: self%outputAnalysis_)
    else
       analysisLabel            =var_str('blackHoleHaloMassRelation'       )//labelTarget//'z'//redshiftInterval
       weightPropertyLabel      =var_str('massBlackHoleLog10'              )
       weightPropertyDescription=var_str('⟨log₁₀(Black hole mass/M☉)⟩'     )
       ...
       <referenceConstruct isResult="yes" object="outputAnalysis_">
        <constructor>
         outputAnalysisScatterFunction1D(                                                                                                                                                    &amp;
          &amp;                                                  analysisLabel                                                                                                             , &amp;
          &amp;                                                  var_str('Scatter in black hole-halo mass relation for $')//redshiftMinimumLabel//' \le z &lt; '//redshiftMaximumLabel//'$', &amp;
          &amp;                                                  var_str('massHalo'                                      )                                                                 , &amp;
          &amp;                                                  var_str('Halo mass'                                     )                                                                 , &amp;
          &amp;                                                  var_str('M☉'                                            )                                                                 , &amp;
          &amp;                                                  massSolar                                                                                                                 , &amp;
       ...
       <referenceConstruct isResult="yes" object="outputAnalysis_">
        <constructor>
         outputAnalysisMeanFunction1D   (                                                                                                                                          &amp;
          &amp;                                               analysisLabel                                                                                                      , &amp;
          &amp;                                               var_str('Black hole-halo mass relation for $'    )//redshiftMinimumLabel//' \le z &lt; '//redshiftMaximumLabel//'$', &amp;
          &amp;                                               var_str('massHalo'                               )                                                                 , &amp;
          &amp;                                               var_str('Halo mass'                              )                                                                 , &amp;
          &amp;                                               var_str('M☉'                                     )                                                                 , &amp;
          &amp;                                               massSolar                                                                                                          , &amp;
    ...
    <objectDestructor name="galacticFilterBlackHoleMass_"                          />
  ...
  subroutine blackHoleVsHaloMassRelationReduce(self,reduced)
    !!{
    Implement reduction for the {\normalfont \ttfamily blackHoleVsHaloMassRelation} output analysis class.
    !!}
    use :: Error, only : Error_Report
    implicit none
    class(outputAnalysisBlackHoleVsHaloMassRelation), intent(inout) :: self
  ...
  subroutine blackHoleVsHaloMassRelationFinalize(self,groupName)
    !!{
    Implement a {\normalfont \ttfamily blackHoleVsHaloMassRelation} output analysis finalization.
    !!}
  ...
  double precision function blackHoleVsHaloMassRelationLogLikelihood(self) result(logLikelihood)
    !!{
    Return the log-likelihood of a {\normalfont \ttfamily blackHoleVsHaloMassRelation} output analysis.
    !!}
    use :: Error                       , only : Error_Report
