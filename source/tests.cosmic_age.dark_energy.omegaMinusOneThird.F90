...
program Tests_Cosmic_Age_Dark_Energy_Omega_Minus_One_Third
  !% Tests cosmic age calculations for a dark energy Universe where dark energy has equation of state w=-1/3, in which case the
  !% expansion history should be the same as for a model with no dark energy. Ages calculated using Python implementation of Ned
  !% Wright's cosmology calculator available from: http://www.astro.ucla.edu/~wright/CC.python
  use Unit_Tests
  ...
  ! Begin unit tests.
  call Unit_Tests_Begin_Group("Cosmic age: dark energy (ω=-1/3) cosmology")

  ! Test cosmic age in a dark energy universe.
  parameterFile='testSuite/parameters/cosmicAge/darkEnergyOmegaMinusOneThird.xml'
...
end program Tests_Cosmic_Age_Dark_Energy_Omega_Minus_One_Third
