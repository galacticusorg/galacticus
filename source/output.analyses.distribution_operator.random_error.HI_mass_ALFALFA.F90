  ...
  <outputAnalysisDistributionOperator name="outputAnalysisDistributionOperatorRandomErrorALFLF">
   <description>
    ...
    \end{equation}
     ...
     included in the priors.} of $a=$\source{a}$=0.100 \pm 0.010$, $b=$\source{b}$=5.885 \pm
     0.100$, and $c=$\source{c}$=0.505 \pm 0.020$ as shown in Fig.~\ref{fig:ALFALFAErrorModel}. The total random
...
contains
  ...
  double precision function randomErrorHIALFALFARootVariance(self,propertyValue,node)
    ...
    \source{constraints/dataAnalysis/hiMassFunction\_ALFALFA\_z0.00/alfalfaHIMassErrorModel.pl} for details.
    !!}
    implicit none
