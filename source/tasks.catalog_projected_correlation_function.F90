...
contains
  ...
  subroutine catalogProjectedCorrelationFunctionPerform(self,status)
    ...
    call correlationFunctionGroup%writeDataset(separation       ,'separation'                ,description='Galaxy separation.'                                ,datasetReturned=dataset)
    call dataset%writeAttribute(megaParsec,'unitsInSI')
    call dataset%close         (                      )
    call correlationFunctionGroup%writeDataset(correlation      ,'projectedCorrelation'      ,description='Projected correlation function from the full mock.',datasetReturned=dataset)
    call dataset%writeAttribute(megaParsec,'unitsInSI')
    call dataset%close         (                      )
    call correlationFunctionGroup%writeDataset(correlationSurvey,'projectedCorrelationSurvey',description='Projected correlation function from survey region.',datasetReturned=dataset)
    call dataset%writeAttribute(megaParsec,'unitsInSI')
    call dataset%close         (                      )
