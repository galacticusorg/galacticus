...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <posteriorSampleSimulation name="posteriorSampleSimulationAnnealedDffrntlEvltn">
   <description>
    This class the \source{differentialEvolution} class to include an annealing schedule---the simulation begins at
    ...
    the options for the \source{differentialEvolution} algorithm, the details of the algorithm are controlled by the
    following sub-parameters:
    \begin{description}
    \item[\source{[temperatureMaximum]}] The maximum temperature to use when tempering.
    \item[\source{[temperatureLevels]}] The number of temperature levels to use.
    \end{description}
    ...
    \end{equation}
    where $T_\mathrm{max}=$\source{[temperatureMaximum]} and $N=$\source{[temperatureLevels]}.
   </description>
