    ...
    if (.not.File_Exists(cloudyPath//"/source/cloudy.exe")) then
       ...
       if (.not.File_Exists(cloudyPath)) then
          ...
          if (.not.File_Exists(cloudyPath//".tar.gz")) then
             ...
             call download('https://data.nublado.org/cloudy_releases/c'//char(cloudyVersionMajor)//'/'//char(cloudyVersion)//'.tar.gz',char(cloudyPath)//'.tar.gz',status=status,retries=5,retryWait=60)
             if (status /= 0) then
                ! Try the "old/" subdirectory. Sometimes the source is moved to this path prior to a new release.
                call download('https://data.nublado.org/cloudy_releases/c'//char(cloudyVersionMajor)//'/old/'//char(cloudyVersion)//'.tar.gz',char(cloudyPath)//'.tar.gz',status=status,retries=5,retryWait=60)
