...
!!}
  ...
  <nodePropertyExtractor name="nodePropertyExtractorDescendants">
   <description>
    ...
    \ttfamily read}'' \refClass{mergerTreeConstructorClass}) providing \source{[presetMergerNodes]} and {\normalfont
    \ttfamily [presetMergerTimes]} are both set to \source{true}. When using randomly assigned satellite orbits
...
contains
  ...
  function descendantsExtract(self,node,time,instance)
    !!{
    Implement a \source{descendants} node property extractor.
    !!}
