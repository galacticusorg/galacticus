...
!!}
  ...
  type, extends(nbodyOperatorClass) :: nbodyOperatorTimeFormation
     ...
     double precision                                                      :: massParentMinimum                 , massParentMaximum            , &
          &                                                                   redshiftFormationMinimum          , redshiftFormationMaximum     , &
          &                                                                   fractionFormation
     integer         (c_size_t                )                            :: massParentCountPerDecade          , redshiftFormationCountPerUnit, &
          &                                                                   snapshotParents
     type            (varying_string          )                            :: simulationReference               , simulationURL                , &
          &                                                                   description
   contains
     final     ::            timeFormationDestructor
     procedure :: operate => timeFormationOperate
  ...
  end interface nbodyOperatorTimeFormation
...
contains
  ...
  function timeFormationConstructorParameters(parameters) result (self)
    ...
    !!]
    self=nbodyOperatorTimeFormation(massParentMinimum,massParentMaximum,massParentCountPerDecade,redshiftFormationMinimum,redshiftFormationMaximum,redshiftFormationCountPerUnit,snapshotParents,fractionFormation,description,simulationReference,simulationURL,cosmologyParameters_)
    !![
  ...
  function timeFormationConstructorInternal(massParentMinimum,massParentMaximum,massParentCountPerDecade,redshiftFormationMinimum,redshiftFormationMaximum,redshiftFormationCountPerUnit,snapshotParents,fractionFormation,description,simulationReference,simulationURL,cosmologyParameters_) result (self)
    ...
    double precision                            , intent(in   )         :: massParentMinimum       , massParentMaximum            , &
         &                                                                 redshiftFormationMinimum, redshiftFormationMaximum     , &
         &                                                                 fractionFormation
    integer         (c_size_t                  ), intent(in   )         :: massParentCountPerDecade, redshiftFormationCountPerUnit, &
         &                                                                 snapshotParents
    type            (varying_string            ), intent(in   )         :: simulationReference     , simulationURL                , &
         &                                                                 description
    class           (cosmologyParametersClass  ), intent(in   ), target :: cosmologyParameters_
    !![
    <constructorAssign variables="massParentMinimum, massParentMaximum, massParentCountPerDecade, redshiftFormationMinimum, redshiftFormationMaximum, redshiftFormationCountPerUnit, snapshotParents, fractionFormation, description, simulationReference, simulationURL, *cosmologyParameters_"/>
    !!]
  ...
  subroutine timeFormationOperate(self,simulations)
    ...
    implicit none
    class           (nbodyOperatorTimeFormation), intent(inout)                 :: self
    type            (nBodyData                 ), intent(inout), dimension(:  ) :: simulations
    double precision                            , allocatable  , dimension(:  ) :: massParentBin        , redshiftFormationBin
    double precision                            , pointer      , dimension(:  ) :: mass                 , expansionFactor
    integer         (c_size_t                  ), pointer      , dimension(:  ) :: treeID               , snapshotID
    double precision                            , allocatable  , dimension(:,:) :: redshiftFormation
    ...
    integer         (c_size_t                  )                                :: massParentCount      , redshiftFormationCount  , &
         &                                                                         iSimulation          , countParents            , &
         &                                                                         i                    , j                       , &
         &                                                                         k                    , l                       , &
         &                                                                         m                    , n
    double precision                                                            :: binParentWidthInverse, binRedshiftWidthInverse , &
         &                                                                         redshiftParent       , expansionFactorFormation, &
         &                                                                         redshiftFormation_
...
#endif
       call displayIndent('compute formation time statistics',verbosityLevelStandard)
#ifdef USEMPI
    end if
#endif
    ! Construct bins in parent mass and in formation redshift.
    massParentCount         =int(log10(self%massParentMaximum       /self%massParentMinimum       )*dble(self%massParentCountPerDecade     ),kind=c_size_t)
    redshiftFormationCount  =int(     (self%redshiftFormationMaximum-self%redshiftFormationMinimum)*dble(self%redshiftFormationCountPerUnit),kind=c_size_t)
    allocate(massParentBin       (massParentCount                       ))
    allocate(redshiftFormationBin(                redshiftFormationCount))
    allocate(countParentBin      (massParentCount                       ))
    allocate(countBin            (massParentCount,redshiftFormationCount))
    allocate(redshiftFormation   (massParentCount,redshiftFormationCount))
    ...
    redshiftFormationBin   =Make_Range(self%redshiftFormationMinimum,self%redshiftFormationMaximum,int(redshiftFormationCount),rangeTypeLinear     ,rangeBinned=.true.)
    binParentWidthInverse  =1.0d0/log10(massParentBin       (2)/massParentBin       (1))
    binRedshiftWidthInverse=1.0d0/     (redshiftFormationBin(2)-redshiftFormationBin(1))
    ...
    do iSimulation=1_c_size_t,size(simulations)
       ...
       countBin=0_c_size_t
       !$omp parallel do private(j,k,l,m,n,expansionFactorFormation,redshiftFormation_) reduction(+:countBin) schedule(dynamic)
       do i=1_c_size_t,countParents
          ...
          expansionFactorFormation=+huge(0.0d0)          
          ! Iterate over progenitor halos of this tree.
          ...
          do while (j > 0_c_size_t .and. treeID(indexTreeID(j)) == treeIDParents(i))
             ...
             if (mass(indexTreeID(j)) > self%fractionFormation*mass(indexParents(i)) .and. expansionFactor(indexTreeID(j)) < expansionFactorFormation) &
                  & expansionFactorFormation=expansionFactor(indexTreeID(j))
             j=j-1_c_size_t
          end do
          ! Accumulate into bin.
       ...
       redshiftFormation=dble(countBin)*binRedshiftWidthInverse
       do k=1,redshiftFormationCount
          where (countParentBin > 0_c_size_t)
             redshiftFormation(:,k)=redshiftFormation(:,k)/dble(countParentBin)
          elsewhere
             redshiftFormation(:,k)=0.0d0
          end where
       end do     
#ifdef USEMPI
       if (mpiSelf%isMaster()) then
          ...
          call resultsGroup%writeDataset  (redshiftFormation        ,'distribution'     )
...
#endif
    return
  end subroutine timeFormationOperate
