   ...
   <description>
    ...
    \begin{itemize}
     ...
     \begin{itemize}
      ...
      \ttfamily [destinationGasMinorMerger]} parameter (either ``\source{disk}'' or ``{\normalfont \ttfamily
      spheroid}''), stars from the satellite moves to the spheroid of the central and mass in the central does not move.
     \end{itemize}
    \end{itemize}
    Here, $f_\mathrm{major}=$\source{[massRatioMajorMerger]} is the mass ratio above which a merger is
    considered to be ``major'', while $f_\mathrm{burst}=$\source{[ratioMassBurst]} and
    $f_\mathrm{gas,crit}=$\source{[fractionGasCriticalBurst]}.
   </description>
  </mergerMassMovements>
  !!]
