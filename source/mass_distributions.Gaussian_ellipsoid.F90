...
contains
  ...
  double precision function gaussianEllipsoidDensity(self,coordinates)
    !!{
    Return the density at the specified \source{coordinates} in a Gaussian ellipsoid mass distribution.
    !!}
  ...
  function gaussianEllipsoidAcceleration(self,coordinates)
    !!{
    Computes the gravitational acceleration at \source{coordinates} for Gaussian ellipsoid mass distributions.
    !!}
    use :: Coordinates                     , only : assignment(=)                 , coordinateCartesian
    use :: Linear_Algebra                  , only : assignment(=)                 , operator(*)        , vector
