...
contains
  ...
  function adiabaticGnedin2004ConstructorParameters(parameters) result(self)
    !!{
    Default constructor for the \source{adiabaticGnedin2004} dark matter halo profile class.
    !!}
    ...
    <inputParameter>
      ...
      <source>parameters</source>
      <description>Selects how solutions are computed when no analytic solution is available. If set to ``\source{fallThrough}'' then the solution ignoring adiabatic contraction by baryons is used, while if set to ``\source{numerical}'' then numerical solvers are used to find solutions.</description>
    </inputParameter>
  ...
  subroutine adiabaticGnedin2004Initialize(self,node,massDistributionBaryonic,darkMatterDistributedFraction,massFractionInitial)
    !!{
    Initialize the dark matter mass distribution for the given \source{node}.
    !!}
