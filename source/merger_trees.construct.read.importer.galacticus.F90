...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  type, public, extends(nodeData) :: nodeDataGalacticus
     !!{
     Extension of the \source{nodeData} class for \glc\ format merger trees. Stores particle indices and counts for nodes.
     !!}
  ...
  <mergerTreeImporter name="mergerTreeImporterGalacticus">
    ...
    construct such a file can be found in the \source{tests/nBodyMergerTrees} folder. In that folder, the
    \source{getMillenniumTrees.pl} script will retrieve a sample of merger trees from the
    ...
    \source{getMillenniumTrees.pl} script requires you to have a username and password to access the Millennium
    ...
    one from \href{mailto:contact@g-vo.org}\source{contact@g-vo.org}.}. These can be entered manually or stored in
    a section of the
    ...
    \begin{verbatim}
      ...
      &lt;/millenniumDB>
    \end{verbatim}
    Here, each \source{host} section describes rules for a given computer (with ``default'' being used if no specific
    match to the regular expression give in \source{name} is found). The \source{user} element gives the
    user name to use, while the \source{passwordFrom} element specifies how the password should be obtained. Currently
    ...
    \source{password} element which contains the password itself. Of course, this is insecure\ldots
    
    The optional \source{treePath} element gives the location where merger trees from the Millennium Simulation can be
