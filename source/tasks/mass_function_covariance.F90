...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  !![
  <task name="taskMassFunctionCovariance" docformat="rst">
   <description>
   ...
   Details of the survey geometry and depth are given for each individual constraints. Computing the large-scale structure contribution to the covariance function requires integration of the non-linear matter power spectrum over the Fourier transform of the survey window function. We use the method of :cite:t:`peacock_non-linear_1996` to determine the non-linear matter power spectrum, because of its simplicity and speed. We have checked that using a more accurate non-linear matter power spectrum (e.g. :cite:author:`lawrence_coyote_2010` :cite:year:`lawrence_coyote_2010`) makes negligible difference to our results. If the angular power spectrum of the survey mask is available\footnote{Typically if the survey geometry is defined by :term:`mangle` polygons, allowing the angular power spectrum to be found using the :term:`mangle` ``harmonize`` utility.}, this is used to compute the relation
   ...
   where :math:`(2\ell+1) C^{ij}_\ell = \sum_{m=-\ell}^{+\ell} \Psi^i_{\ell m} \Psi^{j*}_{\ell m}`, :math:`\Psi^i_{\ell m}` are the spherical harmonic coefficients of the :math:`i^\mathrm{th}` field of the survey, :math:`V` is the maximum distance to which a galaxy of mass :math:`M` can be seen, :math:`P(k)` is the nonlinear power spectrum and
   ...
   where :math:`\tensor*[_1]{\stackrel{\sim}{F}}{_2}` is the regularized generalized hypergeometric function. In other cases, where the angular power spectrum is not available, the survey geometry is realized on a grid which is when Fourier transformed to obtain the appropriate window function.

   To find a suitable :term:`HOD` to describe the observed galaxies we adopt the model of :cite:t:`behroozi_comprehensive_2010`. This is an 11 parameter model which describes separately the numbers of satellite and central galaxies occupying a halo of given mass---the reader is referred to :cite:t:`behroozi_comprehensive_2010` for a complete description of the functional form of this parametric :term:`HOD`. An :term:`MCMC` approach is used to to constrain the :term:`HOD` parameters to fit the observational data. We use a likelihood

   .. math::

      \ln \mathcal{L} = -{1\over 2} \Delta\cdot \mathcal{C}^{-1}\cdot \Delta^\mathrm{T} - {N \over 2} \ln(2\pi) - {\ln |\mathcal{C}| \over 2},

   where :math:`N` is the number of bins in the mass function, :math:`\mathcal{C}` is the covariance matrix of the observed mass function, and :math:`\Delta_i = \phi_i^\mathrm{(HOD)} - \phi_i^\mathrm{(observed)}`. Of course, it is precisely this covariance matrix, :math:`\mathcal{C}`, that we are trying to compute. We therefore adopt an iterative approach as follows:
   ...
   #. find the maximum likelihood parameters of the :term:`HOD` given the observed mass function and the current estimate of the covariance matrix;
   #. using this :term:`HOD` and the framework of :cite:t:`smith_how_2012`, compute a new estimate of the covariance matrix, including all three contributions;
...
contains
  ...
  double precision function massFunctionCovarianceGalaxyRootPowerSpectrum(iBin,timeMinimum,timeMaximum)
    !!{RST
    Computes the quantity :math:`\int_{t_\mathrm{min}}^{t_\mathrm{max}} \mathrm{d} t b(t) \sqrt{P(k,t)} \mathrm{d} V / \mathrm{d}t`, where :math:`b(t)` is galaxy bias, and :math:`P(k,t)` is the non-linear galaxy power spectrum.
    !!}
    use :: Numerical_Integration, only : integrator
    implicit none
  ...
  subroutine massFunctionCovarianceLSSWindowFunction(self,massBinCount,redshiftMinimum,redshiftMaximum,varianceLSS)
    !!{RST
    Compute variance due to large scale structure by directly summing over the Fourier transform of the survey selection function.
    !!}
