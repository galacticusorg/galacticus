...
!% Contains a module which implements calculations of linear growth factor in simple cosomologies.

module Linear_Growth_Simple
  !% Implements calculations of linear growth factor in simple cosomologies.
...
contains
  ...
  subroutine Linear_Growth_Factor_Simple_Tabulate(time,growthTableNumberPoints,growthTableTime,growthTableGrowthFactor &
    ...
    if (Omega_DE()>0.0d0) then
       aEquality=Epoch_of_Matter_Dark_Energy_Equality(requestTypeExpansionFactor)
       
       ! Find a later time when matter is suitably dominant.
       aMatterDominant=min(aEquality/(dominateFactor**(1.0d0/3.0d0)),Expansion_Factor(growthTableTimeMinimum))
    else
       aMatterDominant=Expansion_Factor(growthTableTimeMinimum)
    end if
    ...
    linearGrowthFactorPresent=Interpolate(growthTableNumberPoints,growthTableTime,growthTableGrowthFactor,interpolationObject &
         &,interpolationAccelerator,tPresent,reset=resetInterpolation)
    call Interpolate_Done(interpolationObject,interpolationAccelerator,resetInterpolation)
    resetInterpolation=.true.
    growthTableGrowthFactor=growthTableGrowthFactor/linearGrowthFactorPresent
    
    ! Compute relative normalization factor such that growth factor behaves as a at early times.
    normalizationMatterDominated=((9.0d0*Omega_0()/4.0d0)**(1.0d0/3.0d0))*((H_0_invGyr()*growthTableTime(1))**(2.0d0/3.0d0))&
         &/growthTableGrowthFactor(1)
    return
