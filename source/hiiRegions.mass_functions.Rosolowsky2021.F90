...
!+    Contributions to this file made by: Sachi Weerasooriya, Andrew Benson
  ...
  <hiiRegionMassFunction name="hiiRegionMassFunctionRosolowsky2021">
   <description>
     ...
     $\epsilon=$\source{[epsilon]} of their mass into stars, and the giant molecular cloud mass function is given
     by \cite{rosolowsky_giant_2021}:
     \begin{equation}
     \phi(M) \propto \left(\frac{m}{M}\right)^{\alpha} \exp\!\left(-\frac{m}{M}\right),
     \end{equation}
     where $M$ is the mass of giant molecular cloud, $M_\mathrm{min}=$\source{[massMinimum]} and
     $M_\mathrm{max}=$\source{[massMaximum]} and the minimum and maximum giant molecular cloud masses respectively,
     and $\alpha=$\source{[exponent]}.
   </description>
