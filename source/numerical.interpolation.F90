...
module Numerical_Interpolation
  ...
  type :: interpolator
   ...
   contains
     ...
     <methods>
       ...
       <method description="Locate the position in the array corresponding to the given \source{x}." method="locate"              />
  ...
  interface interpolator
     !!{
     Constructor for the \source{interpolator} class.
     !!}
  ...
  interface interpolator2D
     !!{
     Constructor for the \source{interpolator2D} class.
     !!}
...
contains
  ...
  function interpolatorConstructor(x,y,interpolationType,extrapolationType) result(self)
    !!{
    Constructor for \source{interpolator} objects.
    !!}
  ...
  subroutine interpolatorDestructorRank0(self)
    !!{
    Destructor for rank-0 \source{interpolator} objects.
    !!}
    implicit none
  ...
  subroutine interpolatorDestructorRank1(self)
    !!{
    Destructor for rank-1 \source{interpolator} objects.
    !!}
    implicit none
  ...
  subroutine interpolatorDestructorRank2(self)
    !!{
    Destructor for rank-2 \source{interpolator} objects.
    !!}
    implicit none
    type   (interpolator), intent(inout), dimension(:,:) :: self
    integer                                              :: i1  , i2
  ...
  subroutine interpolatorLinearFactors(self,x,i,h)
    !!{
    Return interpolating factors for linear interpolation in the array \source{xArray()} given \source{x}.
    !!}
    use :: Error       , only : Error_Report
  ...
  subroutine interpolatorLinearWeights(self,x,i,h)
    !!{
    Return interpolation weights factors for linear interpolation in the array \source{xArray()} given \source{x} and \source{i}.
    !!}
    implicit none
    class           (interpolator)                , intent(inout) :: self
    double precision                              , intent(in   ) :: x
  ...
  double precision function interpolatorInterpolateNoYa(self,x)
    !!{
    Interpolate a function to \source{x}.
    !!}
    use :: Error, only : Error_Report
    implicit none
    class           (interpolator  ), intent(inout) :: self
  ...
  double precision function interpolatorDerivativeNoYa(self,x)
    !!{
    Interpolate the derivative of the function to \source{x}.
    !!}
    use :: Error, only : Error_Report
    implicit none
    class           (interpolator  ), intent(inout) :: self
  ...
  function interpolatorLocate(self,x,closest) result(i)
    !!{
    Locate the element in the table for interpolation of \source{x}.
    !!}
    implicit none
    integer         (c_size_t    )                          :: i
    class           (interpolator), intent(inout)           :: self
  ...
  function interpolator2DConstructor(x,y,z) result(self)
    !!{
    Constructor for \source{interpolator2D} objects.
    !!}
    use :: Error, only : Error_Report
    implicit none
  ...
  subroutine interpolator2DDestructorRank0(self)
    !!{
    Destructor for rank-0 \source{interpolator2D} objects.
    !!}
    implicit none
  ...
  double precision function interpolator2DInterpolate(self,x,y)
    !!{
    Interpolate a function to \source{(x,y)}.
    !!}
    use :: Error, only : Error_Report
    implicit none
    class           (interpolator2D), intent(inout)  :: self
