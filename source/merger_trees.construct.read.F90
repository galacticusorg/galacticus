...
contains
  ...
  subroutine Merger_Tree_Read_Do(thisTree,skipTree)
    ...
    if (processTree) then
       ...
       class default
          call Galacticus_Error_Report('Merger_Tree_Read_Do','nodes arrays is of wrong class')
       end select
       ! Deallocate nodes.
       call Memory_Usage_Record(sizeof(nodes),addRemove=-1)
       deallocate(nodes)
       ...
       if (returnSplitForest .and. splitForestNextTree == size(splitForestTreeStart)) then
          call Dealloc_Array(splitForestTreeSize )
          call Dealloc_Array(splitForestTreeStart)
          call Dealloc_Array(splitForestPushTo   )
          call Dealloc_Array(splitForestPullFrom )
          call Dealloc_Array(splitForestPushType )
          call Dealloc_Array(splitForestMapIndex )
       end if
    end if
    return
  ...
  subroutine Root_Node_Affinities_Initial(nodes)
    ...
    call Alloc_Array(splitForestMapIndex,shape(nodes))
    ...
    call Alloc_Array(splitForestTreeSize ,[treeCount])
    call Alloc_Array(splitForestTreeStart,[treeCount])
    treeIndexPrevious=-1
    treeStartPrevious= 0
    treeCount        = 0
    do i=1,size(nodes)
    ...
    call Alloc_Array(splitForestPushTo   ,[pushCount])
    call Alloc_Array(splitForestPullFrom ,[pushCount])
    call Alloc_Array(splitForestPushType ,[pushCount])
    call Alloc_Array(splitForestPushTime ,[pushCount])
    call Alloc_Array(splitForestIsPrimary,[pushCount])
    call Alloc_Array(splitForestPushDone ,[pushCount])
    call Alloc_Array(splitForestPullDone ,[pushCount])
    pushCount=0
    do i=1,size(nodes)
       ! Process only nodes with a descendent.
       if (nodes(i)%descendentIndex >= 0) then
