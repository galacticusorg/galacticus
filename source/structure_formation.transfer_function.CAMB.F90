  ...
  subroutine cambCheckRange(self,wavenumber)
    ...
    if (.not.self%initialized.and.File_Exists(self%fileName)) then
       ...
       if (.not.File_Exists(galacticusPath(pathTypeDataDynamic)//"CAMB/camb")) then
          ...
          call System_Command_Do('cd '//galacticusPath(pathTypeDataDynamic)//'/CAMB/; sed -r -i~ s/"ifortErr\s*=.*"/"ifortErr = 1"/ Makefile; sed -r -i~ s/"gfortErr\s*=.*"/"gfortErr = 0"/ Makefile; sed -r -i~ s/"^FFLAGS\s*\+=\s*\-march=native"/"FFLAGS+="/ Makefile; sed -r -i~ s/"^FFLAGS\s*=\s*.*"/"FFLAGS = -Ofast -fopenmp"/ Makefile; make -j1',status);
