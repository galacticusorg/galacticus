...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  use :: Star_Formation_Histories              , only : starFormationHistoryClass
  type:: emissionLineLuminosityTemplateDisk
     !!{
     Type used to store luminosity templates for emission lines.
     !!}
     private
     integer         (c_size_t)                                :: countLines=-1_c_size_t
     double precision          , allocatable, dimension(:,:,:) :: emissionLineLuminosityDisk
  end type emissionLineLuminosityTemplateDisk

  !![
  <nodePropertyExtractor name="nodePropertyExtractorLuminosityEmissionLineDisk">
  ...
  !!]
  type, extends(nodePropertyExtractorTuple) :: nodePropertyExtractorLuminosityEmissionLineDisk
     !!{
     ...
     type            (emissionLineLuminosityTemplateDisk    ), allocatable, dimension(:) :: templates
   ...
   contains
     ...
     !!]
     final     ::                            emissionLineLuminosityDiskDestructor
     procedure :: historyHashedDescriptor => emissionLineLuminosityDiskHistoryHashedDescriptor
     procedure :: elementCount            => emissionLineLuminosityDiskElementCount
     procedure :: extract                 => emissionLineLuminosityDiskExtract
     procedure :: names                   => emissionLineLuminosityDiskNames
     procedure :: descriptions            => emissionLineLuminosityDiskDescriptions
     procedure :: unitsInSI               => emissionLineLuminosityDiskUnitsInSI
     procedure :: luminosityMean          => emissionLineLuminosityDiskMean
     procedure :: indexTemplateTime       => emissionLineLuminosityDiskIndexTemplateTime
     procedure :: indexTemplateNode       => emissionLineLuminosityDiskIndexTemplateNode 
  end type nodePropertyExtractorLuminosityEmissionLineDisk
  
  interface nodePropertyExtractorLuminosityEmissionLineDisk
     !!{
     Constructors for the ``emissionLineLuminosityDisk'' output analysis class.
     !!}
     module procedure emissionLineLuminosityDiskConstructorParameters
     module procedure emissionLineLuminosityDiskConstructorInternal
  end interface nodePropertyExtractorLuminosityEmissionLineDisk
      
contains

  function emissionLineLuminosityDiskConstructorParameters(parameters) result(self)
    !!{
    ...
    implicit none
    type            (nodePropertyExtractorLuminosityEmissionLineDisk           )                :: self
    ...
    !!]
    self=nodePropertyExtractorLuminosityEmissionLineDisk(enumerationComponentTypeEncode(char(component),includesPrefix=.false.),lineNames,toleranceRelative,starFormationHistory_,outputTimes_,hiiRegionLuminosityFunction_)
    !![
    ...
    !!]
    return
  end function emissionLineLuminosityDiskConstructorParameters

  function emissionLineLuminosityDiskConstructorInternal(component,lineNames,toleranceRelative,starFormationHistory_,outputTimes_,hiiRegionLuminosityFunction_) result(self)
    !!{
...
!             & *sqrt(rateHydrogenIonizingPhotonsMinimum*rateHydrogenIonizingPhotonsMaximum) 
            &             *luminosities(             &
            &                                      :,           &
            &                                      :,           &
            &                                      i,           &
            &                                      :           &
            &                                     )
        !write (0,*) "SHAPE ",shape(self%luminositiesReduced )
     end do
    ...
    !write (0,*) "lineNames", lineNames
    !write (0,*) "lineNames luminosity",self%luminositiesReduced(1,1,1)
    ...
    return    
  end function emissionLineLuminosityDiskConstructorInternal

  subroutine emissionLineLuminosityDiskDestructor(self)
    !!{
    Destructor for the {\normalfont \ttfamily emission line luminosity} property extractor class.
    !!}
    implicit none
    type(nodePropertyExtractorLuminosityEmissionLineDisk), intent(inout) :: self
    !![
    ...
    !!]
    return
  end subroutine emissionLineLuminosityDiskDestructor
  
  integer function emissionLineLuminosityDiskElementCount(self,time)
    !!{
    Return the number of elements in the {\normalfont \ttfamily emissionLineLuminosityDisk} property extractors.
    !!}
    implicit none
    class     (nodePropertyExtractorLuminosityEmissionLineDisk), intent(inout) :: self
    double precision                                       , intent(in   ) :: time
    !$GLC attributes unused :: self, time

    emissionLineLuminosityDiskElementCount=self%countLines
    !write (0,*) "emissionLineLuminosityDiskElementCount",emissionLineLuminosityDiskElementCount
    return
  end function emissionLineLuminosityDiskElementCount

  function emissionLineLuminosityDiskExtract(self,node,time,instance)
    !!{
    ...
    implicit none
    double precision                          , dimension(:  )          , allocatable :: emissionLineLuminosityDiskExtract
    class           (nodePropertyExtractorLuminosityEmissionLineDisk), intent(inout)   , target                :: self
    ...
    allocate(emissionLineLuminosityDiskExtract(self%countLines))
    emissionLineLuminosityDiskExtract=0.0d0
    ...
    if (indexTemplate > 0) then
       ...
       luminosityTemplate_ => self%templates(indexTemplate)%emissionLineLuminosityDisk
    else
    ...
    do iLines=1,size(emissionLineLuminosityDiskExtract,dim=1)
      emissionLineLuminosityDiskExtract(iLines)=sum(luminosityTemplate_(iLines,:,:)*starFormationHistory%data(:,:))
    end do
    return
  end function emissionLineLuminosityDiskExtract

  subroutine emissionLineLuminosityDiskNames(self,time,names)
    !!{
    ...
    names=self%names_
    return
  end subroutine emissionLineLuminosityDiskNames
  
  subroutine emissionLineLuminosityDiskDescriptions(self,time,descriptions)
    !!{
    ...
    return
  end subroutine emissionLineLuminosityDiskDescriptions
  
  function emissionLineLuminosityDiskUnitsInSI(self,time)
  !!{
    Return the units of the {\normalfont \ttfamily emissionLineLuminosityDisk} properties in the SI system.
    !!}
    use :: Numerical_Constants_Units, only : ergs
    implicit none
    double precision                          , allocatable  , dimension(:)      :: emissionLineLuminosityDiskUnitsInSI
    class           (nodePropertyExtractorLuminosityEmissionLineDisk), intent(inout) ::self
    double precision                          , intent(in   )                    :: time
    !$GLC attributes unused :: time

    allocate(emissionLineLuminosityDiskUnitsInSI(self%countLines))
    emissionLineLuminosityDiskUnitsInSI=ergs
    return
  end function emissionLineLuminosityDiskUnitsInSI

  integer function emissionLineLuminosityDiskIndexTemplateTime(self,time)
    !!{
    ...
    emissionLineLuminosityDiskIndexTemplateTime=-1
    ...
    emissionLineLuminosityDiskIndexTemplateTime=int(indexOutput)
    return
  end function emissionLineLuminosityDiskIndexTemplateTime

  integer function emissionLineLuminosityDiskIndexTemplateNode(self,node,starFormationHistory)
    !!{
    ...
    emissionLineLuminosityDiskIndexTemplateNode=-1
    if (.not.self%useluminosityTemplates) return
    ...
    emissionLineLuminosityDiskIndexTemplateNode=int(indexOutput)
    ...
    if (.not.allocated(self%templates(emissionLineLuminosityDiskIndexTemplateNode)%emissionLineLuminosityDisk)) then
       ...
       if (File_Exists(fileName)) then
          ...
          if (file%hasDataset('luminosityTemplate')) then
             ...
             call file%readDataset('luminosityTemplate',self%templates(emissionLineLuminosityDiskIndexTemplateNode)%emissionLineLuminosityDisk)
          end if
          call file%close()
          !$ call hdf5Access%unset()
       end if
       if (.not.allocated(self%templates(emissionLineLuminosityDiskIndexTemplateNode)%emissionLineLuminosityDisk)) then
          self%templates(emissionLineLuminosityDiskIndexTemplateNode)%emissionLineLuminosityDisk=self%luminosityMean(self%outputTimes_%time(indexOutput),starFormationHistory,parallelize=.true.)
          !$omp critical(gfortranInternalIO)
          write (label,'(f12.8)') self%outputTimes_%time(indexOutput)
          !$omp end critical(gfortranInternalIO)
          ...
          call file%writeDataset(self%templates(emissionLineLuminosityDiskIndexTemplateNode)%emissionLineLuminosityDisk,'luminosityTemplate')
          call file%close()
          !$ call hdf5Access%unset()
       end if
    ...
    return
  end function emissionLineLuminosityDiskIndexTemplateNode

  double precision function emissionLineLuminosityDiskMean(self,time,starFormationHistory,parallelize)
    !!{
    ...
    implicit none
    double precision                                            , dimension(:,:,:), allocatable :: emissionLineLuminosityDiskMean
    class           (nodePropertyExtractorLuminosityEmissionLineDisk      ), intent(inout)                 :: self
    double precision                                            , intent(in   )                 :: time
    type            (history                                   ), intent(in   )                 :: starFormationHistory
    ...
    allocate(emissionLineLuminosityDiskMean(self%countLines,size(starFormationHistory%data,dim=1),size(starFormationHistory%data,dim=2)))
    ...
    integratorMetallicity=integrator(emissionLineLuminosityDiskIntegrandMetallicity,toleranceRelative=self%toleranceRelative)
    ...
    do iterator=0,counterMaximum-1
       ...
       if (metallicityMaximum > metallicityMinimum) then
          emissionLineLuminosityDiskMean(iLine,iTime,iMetallicity)=+integratorMetallicity%integrate(metallicityMinimum,metallicityMaximum) &
               &                                            /                               (timeMaximum       -timeMinimum       ) &
               &                                            /                               (metallicityMaximum-metallicityMinimum)
       else
          metallicity_=metallicityMinimum
          emissionLineLuminosityDiskMean(iLine,iTime,iMetallicity)=+integratorTime       %integrate(timeMinimum       ,timeMaximum       ) &
               &                                            /                               (timeMaximum       -timeMinimum       )
       end if
  ...
  contains
    ...
    double precision function emissionLineLuminosityDiskIntegrandMetallicity(metallicity)
      !!{
      ...
      emissionLineLuminosityDiskIntegrandMetallicity=integratorTime%integrate(timeMinimum,timeMaximum)
      return
    end function emissionLineLuminosityDiskIntegrandMetallicity

    double precision function emissionLineLuminosityDiskIntegrandTime(timeBirth)
      !!{
    ...
    double precision                                      , dimension(0:1)        :: interpolateFactorTime, interpolateFactorMetallicity
      ...
      emissionLineLuminosityDiskIntegrandTime=0.0d0
      do iTime=0,1
         do iMetallicity=0,1
           emissionLineLuminosityDiskIntegrandTime=+emissionLineLuminosityDiskIntegrandTime &
                 ! & +1.0d0&
      ...
      return
    end function emissionLineLuminosityDiskIntegrandTime
    
  end function emissionLineLuminosityDiskMean

  function emissionLineLuminosityDiskHistoryHashedDescriptor(self,starFormationHistory)  
    !!{
    ...
    use :: Histories           , only : history
    implicit none
    type     (varying_string          )                :: emissionLineLuminosityDiskHistoryHashedDescriptor
    class    (nodePropertyExtractorLuminosityEmissionLineDisk), intent(in   ) :: self
    type     (history                 ), intent(in   ) :: starFormationHistory
    ...
    descriptorString=descriptorString//":sourceDigest{"//String_C_To_Fortran(nodePropertyExtractorLuminosityEmissionLineDisk5)//"}"
    !![
    <workaround type="gfortran" PR="102845" url="https:&#x2F;&#x2F;gcc.gnu.org&#x2F;bugzilla&#x2F;show_bug.cgi=102845">
     <description>
      Memory leak possibly due to OpenMP parallelism, or some failing of gfortran.
    ...
    !!]
    emissionLineLuminosityDiskHistoryHashedDescriptor=Hash_MD5(descriptorString)
    return
  end function emissionLineLuminosityDiskHistoryHashedDescriptor
