...
contains
  ...
  subroutine accretionDiskSpectraConstruct(parameters,accretionDiskSpectra_)
    !!{
    Build a \source{accretionDiskSpectra} object from a given parameter set. This is a globally-callable function
  ...
  double precision function accretionDiskSpectraSpectrumNode(accretionDiskSpectra_,node,wavelength) result(spectrum)
    !!{
    Evaluate AGN spectra using a \source{accretionDiskSpectra} object passed to us as an unlimited polymorphic object.
    !!}
    use :: Error                 , only : Error_Report
  ...
  subroutine accretionDiskSpectraDestruct(accretionDiskSpectra_)
    !!{
    Destruct a \source{accretionDiskSpectra} object passed to us as an unlimited polymorphic object.
    !!}
    use :: Error                 , only : Error_Report
    use :: Accretion_Disk_Spectra, only : accretionDiskSpectraClass
    implicit none
