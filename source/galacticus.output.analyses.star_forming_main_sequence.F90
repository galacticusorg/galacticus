...
contains
  ...
  function starFormingMainSequenceConstructorParameters(parameters) result (self)
    ...
    if (parameters%isPresent('fileName')) then
	  ...
	  !!]
       ...
       if (countMasses <= 1_c_size_t) call Galacticus_Error_Report('>1 mass bins is required'//{introspection:location})
  ...
  function starFormingMainSequenceConstructorInternal(label,comment,massesStellar,galacticFilter_,surveyGeometry_,cosmologyFunctions_,cosmologyFunctionsData,outputTimes_,outputAnalysisPropertyOperator_,outputAnalysisDistributionOperator_,outputAnalysisWeightPropertyOperator_,starFormationRateDisks_,starFormationRateSpheroids_,targetLabel,functionValueTarget,functionCovarianceTarget,massesStellarBinWidthLogarithmic) result(self)
    ...
    use :: Node_Property_Extractors                , only : nodePropertyExtractorMassStellar           , nodePropertyExtractorStarFormationRate, nodePropertyExtractorRatio
    ...
    if (size(massesStellar) == 1) then
       if (.not.present(massesStellarBinWidthLogarithmic)) call Galacticus_Error_Report('bin width must be specified'//{introspection:location})
