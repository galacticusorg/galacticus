...
program Tests_SIDM_Parametric_Maps
  ...
  double precision                                          :: timescaleC1           , timescaleC2, &
       &                                                       densityV1             , densityV2  , &
       &                                                       densityR2
  ...
  ! Derivatives dVmax/dτ and dRmax/dτ, eqn. 2.4.
  ...
  ! τ=0 (CDM) limit: the maps are the identity and the core radius vanishes.
  call Assert("velocityMaximumNFW τ=0 is the identity",velocityMaximumNFW(100.0d0,0.0d0),100.0d0,relTol=1.0d-9)
  call Assert("radiusMaximumNFW τ=0 is the identity"  ,radiusMaximumNFW  ( 10.0d0,0.0d0), 10.0d0,relTol=1.0d-9)
  call Assert("densityScale τ=0 is the identity"      ,densityScale      (1.0d7  ,0.0d0),1.0d7  ,relTol=1.0d-6)
  call Assert("radiusScale τ=0 is the identity"       ,radiusScale       (0.01d0 ,0.0d0),0.01d0 ,relTol=1.0d-6)
  call Assert("radiusCore τ=0 vanishes"               ,radiusCore        (0.01d0 ,0.0d0),0.0d0  ,absTol=1.0d-9)
  ! τ>1 (core-collapsed) limit: the velocity and radius derivatives vanish.
  call Assert("velocityMaximumRateTau τ>1 vanishes",velocityMaximumRateTau(1.5d0,100.0d0),0.0d0,absTol=1.0d-12)
  call Assert("radiusMaximumRateTau τ>1 vanishes"  ,radiusMaximumRateTau  (1.5d0, 10.0d0),0.0d0,absTol=1.0d-12)
  ! τ is clamped to [0,1]: evaluating beyond 1 returns the τ=1 result.
  call Assert("velocityMaximumNFW τ is clamped to [0,1]",velocityMaximumNFW(100.0d0,1.5d0),velocityMaximumNFW(100.0d0,1.0d0),relTol=1.0d-12)
  ...
  call Assert("densityScaleNFW proportional to Vmax²",densityV2/densityV1,4.00d0,relTol=1.0d-9)
  densityR2=densityScaleNFW(0.02d0, 50.0d0)
  call Assert("densityScaleNFW proportional to 1/Rₛ²",densityR2/densityV1,0.25d0,relTol=1.0d-9)
