...
module File_Utilities
  ...
  interface Count_Lines_in_File
     !!{
     Generic interface for \source{Count\_Lines\_in\_File} function.
     !!}
  ...
  interface
     function mkdir_C(name) bind(c,name='mkdir_C')
       !!{
       Template for a C function that calls \source{mkdir()} to make a directory.
       !!}
       import
  ...
  interface
     function rmdir_C(name) bind(c,name='rmdir_C')
       !!{
       Template for a C function that calls \source{rmdir()} to remove a directory.
       !!}
       import
  ...
  interface
     function unlink_C(name) bind(c,name='unlink_C')
       !!{
       Template for a C function that calls \source{unlink()} to remove a file.
       !!}
       import
  ...
  interface
     function rename_C(nameOld,nameNew) bind(c,name='rename_C')
       !!{
       Template for a C function that calls \source{rename()} to rename a file.
       !!}
       import
  ...
  interface
     function flock_C(name,ld,lockIsShared,timeSleep,countAttempts) bind(c,name='flock_C')
       !!{
       Template for a C function that calls \source{flock()} to lock a file.
       !!}
       import
  ...
  interface
     subroutine funlock_C(ld) bind(c,name='funlock_C')
       !!{
       Template for a C function that calls \source{flock()} to unlock a file.
       !!}
       import
       type(c_ptr) :: ld
  ...
  interface
     function access_C(name) bind(c,name='access_C')
       !!{
       Template for a C function that calls \source{access()} to check for file existence.
       !!}
       import
...
contains
  ...
  logical function File_Exists_VarStr(fileName)
    !!{
    Checks for existence of file \source{fileName} (version for varying string argument).
    !!}
  ...
  logical function File_Exists_Char(fileName)
    !!{
    Checks for existence of file \source{fileName} (version for character argument).
    !!}
  ...
  integer function Count_Lines_in_File_VarStr(in_file,comment_char)
    !!{
    Returns the number of lines in the file \source{in\_file} (version for varying string argument).
    !!}
    use :: ISO_Varying_String, only : char
    implicit none
  ...
  integer function Count_Lines_in_File_Char(in_file,comment_char)
    !!{
    Returns the number of lines in the file \source{in\_file} (version for character argument).
    !!}
  ...
  contains
    ...
    subroutine Get_Paths(pathsLength)
      !!{
      Retrieve the \source{PATH} environment variable.
      !!}
