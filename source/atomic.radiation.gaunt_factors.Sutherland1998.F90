...
!% Contains a module which computes Gaunt factors for Bremsstrahlung emission from ions using the fitting functions of \cite{sutherland_accurate_1998}.

module Gaunt_Factors_Sutherland1998
  !% Contains a function which computes Gaunt factors for Bremsstrahlung emission from ions using the fitting functions of \cite{sutherland_accurate_1998}.
  private
  public :: Gaunt_Factor_Sutherland1998_Initialize

  ! Arrays to hold coefficients of the fitting function.
  integer         , parameter                   :: coefficientCount      =  41
  double precision, dimension(coefficientCount) :: energyScalesLogarithmic=[                                                                 &
...
contains
  ...
  !# <gauntFactorMethod>
  !#  <unitName>Gaunt_Factor_Sutherland1998_Initialize</unitName>
  !# </gauntFactorMethod>
  subroutine Gaunt_Factor_Sutherland1998_Initialize(gauntFactorMethod,Gaunt_Factor_Get)
    !% Initializes the ``Sutherland1998'' Gaunt Factor module.
    use ISO_Varying_String
    implicit none
    type     (varying_string             ), intent(in   )          :: gauntFactorMethod
    procedure(Gaunt_Factor_Sutherland1998), intent(inout), pointer :: Gaunt_Factor_Get

    ! Check if this Gaunt Factor method has been selected.
    if (gauntFactorMethod == 'sutherland1998') Gaunt_Factor_Get => Gaunt_Factor_Sutherland1998
    return
  end subroutine Gaunt_Factor_Sutherland1998_Initialize
  ...
  double precision function Gaunt_Factor_Sutherland1998(atomicNumber,electronNumber,temperature) 
    ...
    if (temperature <= 0.0d0) then
       Gaunt_Factor_Sutherland1998=0.0d0
       return
    end if
    ! Validate input.
    if (electronNumber > atomicNumber) call Galacticus_Error_Report('Gaunt_Factor_Sutherland1998','number of electrons exceeds atomic number')
    ...
    if (Atomic_Ionization_Potential(atomicNumber,electronNumber) == 0.0d0) then
       ...
       iTable=Search_Array(energyScalesLogarithmic,energyScaleLogarithmic)
       if (iTable <=                0) iTable=               1
       if (iTable >= coefficientCount) iTable=coefficientCount
       ! Interpolate to get actual value.
       energyScaleOffset          =+energyScaleLogarithmic          &
            &                      -energyScalesLogarithmic(iTable)
       Gaunt_Factor_Sutherland1998=+gauntFactorZeroPoint   (iTable) &
            &                      +    energyScaleOffset           &
            &                      *(                               &
    ...
    return
  end function Gaunt_Factor_Sutherland1998

end module Gaunt_Factors_Sutherland1998
