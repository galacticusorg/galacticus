  ...
  subroutine massAccretionHistoryOperatePreEvolution(self,tree)
    ...
    do while (associated(treeCurrent))
       ...
       if (self%outputGroup%hasGroup(char(groupName)))                                                                                                                                  &
            ...
            &                   ' to be reset to the index of the root node by adding the following highglighted option to your input parameter file'            //char(10)//char(10)// &
            &                   stringXMLFormat('<mergerTreeConstructor value="read">**B<treeIndexToRootNodeIndex value="true"/>**C</mergerTreeConstructor>')                        // &
            &                   {introspection:location}                                                                                                                                &
            &                  )
       ...
       call                             accretionDataset%writeAttribute(unitsMake(unitsInSI=gigaYear  ),'units'                                                       )
       call                             treeGroup       %writeDataset  (nodeMass           ,'nodeMass'           ,'Mass of the node [M⊙].'       ,datasetReturned=accretionDataset)
       call                             accretionDataset%writeAttribute(unitsMake(unitsInSI=massSolar ),"units"                                                                   )
