...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <nodePropertyExtractor name="nodePropertyExtractorVirialProperties">
   <description>
    ...
    \begin{description}
     \item [\source{nodeVirialRadius}] The virial radius (following whatever definition of virial overdensity is
     specified by the virial density contrast (see \refPhysics{virialDensityContrast}) in units of Mpc;
     \item [\source{nodeVirialVelocity}] The circular velocity at the virial radius (in km/s).
    \end{description}
   </description>
...
contains
  ...
  integer function virialPropertiesElementCount(self,time)
    !!{
    Return the number of elements in the \source{virialProperties} property extractors.
    !!}
    implicit none
  ...
  subroutine virialPropertiesNames(self,time,names)
    !!{
    Return the names of the \source{virialProperties} properties.
    !!}
    implicit none
    class           (nodePropertyExtractorVirialProperties), intent(inout)                             :: self
    double precision                                       , intent(in   )                             :: time
  ...
  subroutine virialPropertiesDescriptions(self,time,descriptions)
    !!{
    Return the descriptions of the \source{virialProperties} properties.
    !!}
    implicit none
    class           (nodePropertyExtractorVirialProperties), intent(inout)                             :: self
    double precision                                       , intent(in   )                             :: time
  ...
  function virialPropertiesUnitsInSI(self,time)
    !!{
    Return the units of the \source{virialProperties} properties in the SI system.
    !!}
