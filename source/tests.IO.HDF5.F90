...
program Tests_IO_HDF5
  ...
  do iPass=1,2
     ...
     case(1)
        ...
        groupObject=fileObject%openGroup("myGroup",description="This is my group.",objectsOverwritable=.true.,chunkSize=1024_hsize_t&
             &,compressionLevel=9)
        appendableOK=.true.
     case (2)
        call Unit_Tests_Begin_Group("Tests with chunking disabled")
        groupObject=fileObject%openGroup("myGroup",description="This is my group.",objectsOverwritable=.true.,chunkSize=-1_hsize_t&
             &,compressionLevel=-1)
        appendableOK=.false.
     end select
     ...
     do i=1,10
        ...
        do j=1,11-i
           varDoubleArray2D(i)%row(j)=float(2*i-3*j)
        end do
     end do
     call groupObject%writeDataset(varDoubleArray2D,"varDoubleDataset2dArray")
     ! Read the variable-length 2D double array dataset back.
     ...
     if (iPass==2) then
       ...
       ! Open the root group.
       groupObject=fileObject%openGroup("/",description="Root group.")
       ! Read the dataset.
