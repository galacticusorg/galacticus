...
!+    Contributions to this file made by: Claude.
  ...
  !![
  <outputAnalysis name="outputAnalysisStellarVsHaloMassRelation" docformat="rst">
    <description>
    A stellar vs. halo mass relation output analysis class. Target data is read from an :term:`HDF5` file specified by the ``[fileNameTarget]`` parameter. This file must contain one or more groups named ``redshiftIntervalN`` where ``N`` is an integer. Each such group specifies the stellar mass--halo mass relation in one redshift interval, and must contain the following datasets and attributes:
    ...
    * dataset ``massStellarError``: uncertainty in stellar mass in units of :math:`\mathrm{M}_\odot`;
    * dataset ``massStellarScatter``: scatter in stellar mass in units of dex;
    * dataset ``massStellarScatterError``: uncertainty in scatter in stellar mass in units of dex;
...
contains
  ...
  function stellarVsHaloMassRelationConstructorInternal(fileNameTarget,redshiftInterval,likelihoodBinsAutomatic,likelihoodBins,likelihoodNormalize,computeScatter,systematicErrorPolynomialCoefficient,systematicErrorMassHaloPolynomialCoefficient,cosmologyParameters_,cosmologyFunctions_,darkMatterProfileDMO_,virialDensityContrast_,outputTimes_) result (self)
    ...
    use :: Output_Analysis_Property_Operators    , only : outputAnalysisPropertyOperatorAntiLog10                       , outputAnalysisPropertyOperatorCsmlgyLmnstyDstnc, propertyOperatorList      , outputAnalysisPropertyOperatorLog10, &
          &                                               outputAnalysisPropertyOperatorSequence                        , outputAnalysisPropertyOperatorSystematicPolynomial, 
    ...
    type            (outputAnalysisPropertyOperatorSystematicPolynomial), pointer                       :: outputAnalysisWeightPropertyOperatorSystematicPolynomial_       , outputAnalysisPropertyOperatorSystematicPolynomial_
   ...
   ! Build a filter which selects central galaxies with stellar mass above some coarse lower limit suitable for this sample.
    ...
    !![
    <referenceConstruct object="outputAnalysisPropertyOperatorSystematicPolynomial_"              constructor="outputAnalysisPropertyOperatorSystematicPolynomial       (errorPolynomialMassHaloZeroPoint,systematicErrorMassHaloPolynomialCoefficient                                                 )"/>
    !!]
    ...
    propertyOperatorsMassHalo_%next%operator_ => outputAnalysisPropertyOperatorSystematicPolynomial_
    ...
    propertyOperators_          %next               %operator_ => outputAnalysisWeightPropertyOperatorSystematicPolynomial_
  ...
  subroutine stellarVsHaloMassRelationFinalize(self,groupName)
    !!{RST
    Implement a ``stellarVsHaloMassRelation`` output analysis finalization.
    !!}
  ...
  double precision function stellarVsHaloMassRelationLogLikelihood(self) result(logLikelihood)
    !!{RST
    Return the log-likelihood of a ``stellarVsHaloMassRelation`` output analysis.
    !!}
    ...
    double precision                                         , allocatable  , dimension(:,:) :: massStellarLogarithmicCovariance
    double precision                                         , allocatable  , dimension(:  ) :: massStellarLogarithmic
    ...
    class is (outputAnalysisMeanFunction1D   )
       ...
       logLikelihood=Output_Analysis_Log_Likelihood_Relation(massStellarLogarithmic,massStellarLogarithmicCovariance,self%massStellarLogarithmicTarget,self%massStellarLogarithmicCovarianceTarget,self%likelihoodBins,self%likelihoodBinsAutomatic,massStellarLogarithmicTiny,self%likelihoodNormalize)
    class is (outputAnalysisScatterFunction1D)
       ...
       logLikelihood=Output_Analysis_Log_Likelihood_Relation(massStellarLogarithmic,massStellarLogarithmicCovariance,self%massStellarScatterTarget,self%massStellarScatterCovarianceTarget,self%likelihoodBins,self%likelihoodBinsAutomatic,massStellarLogarithmicTiny,self%likelihoodNormalize)
    class default
