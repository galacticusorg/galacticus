...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  !!{RST
  Implements an output analysis class that computes mass-size relations for Local Group satellite galaxies.
  !!}

  !![
  <outputAnalysis name="outputAnalysisLocalGroupMassSizeRelation" docformat="rst">
   <description>
  ...
  type, extends(outputAnalysisClass) :: outputAnalysisLocalGroupMassSizeRelation
   ...
   contains
     ...
     procedure :: finalize      => localGroupMassSizeRelationFinalize
     procedure :: reduce        => localGroupMassSizeRelationReduce
     procedure :: logLikelihood => localGroupMassSizeRelationLogLikelihood
  end type outputAnalysisLocalGroupMassSizeRelation
  ...
  end interface outputAnalysisLocalGroupMassSizeRelation
...
contains
  ...
  function localGroupMassSizeRelationConstructorParameters(parameters) result(self)
    ...
    allocate(sizeSystematicErrorPolynomialCoefficient(max(1,parameters%count('sizeSystematicErrorPolynomialCoefficient',zeroIfNotPresent=.true.))))
    ...
    <inputParameter docformat="rst">
      <name>sizeSystematicErrorPolynomialCoefficient</name>
      <source>parameters</source>
      <variable>sizeSystematicErrorPolynomialCoefficient</variable>
      <defaultValue>[0.0d0]</defaultValue>
      <description>
      The coefficients of the size systematic error polynomial.
      </description>
    </inputParameter>
    <inputParameter docformat="rst">
    ...
    <inputParameter docformat="rst">
      ...
      <description>
      The number of bins per decade of halo mass to use when constructing Local Group mass-size relation covariance matrices for main branch galaxies.
      </description>
    </inputParameter>
    <inputParameter docformat="rst">
      ...
      <description>
      The minimum halo mass to consider when constructing Local Group mass-size relation covariance matrices for main branch galaxies.
      </description>
    </inputParameter>
    <inputParameter docformat="rst">
      ...
      <description>
      The maximum halo mass to consider when constructing Local Group mass-size relation covariance matrices for main branch galaxies.
      </description>
    </inputParameter>
    <inputParameter docformat="rst">
  ...
  function localGroupMassSizeRelationConstructorInternal(outputTimes_,positionType,randomErrorMinimum,randomErrorMaximum,randomErrorPolynomialCoefficient,systematicErrorPolynomialCoefficient,sizeSystematicErrorPolynomialCoefficient,covarianceBinomialBinsPerDecade,covarianceBinomialMassHaloMinimum,covarianceBinomialMassHaloMaximum) result (self)
    ...
    use :: Node_Property_Extractors                , only : nodePropertyExtractorMassStellar                    , nodePropertyExtractorRadiusHalfMassStellar
    ...
    use :: Output_Analysis_Property_Operators      , only : outputAnalysisPropertyOperatorAntiLog10             , outputAnalysisPropertyOperatorLog10       , outputAnalysisPropertyOperatorSequence, outputAnalysisPropertyOperatorSystematicPolynomial, &
    ...
    logical                                                                 , allocatable  , dimension(:  ) :: isPresentMasses                                             , isPresentSizes                                                   , &
         &                                                                                                     isPresentSizesUncertainties
    double precision                                                        , allocatable  , dimension(:  ) :: masses                                                      , massesTarget                                                     , &
         &                                                                                                     sizesTarget                                                 , functionValueTarget                                              , &
         &                                                                                                     sizesUncertaintiesTarget                                    , functionVarianceMeasurementTarget                                , &
    ...
    double precision                                                        , parameter                     :: bufferWidthLogarithmic                          =+3.0d+0    , errorZeroPoint                                       =10.00d0    , &
         &                                                                                                     sizeErrorPolynomialZeroPoint                    =-4.0d+0    , covarianceLarge                                      = 1.00d0    , &
    ...
    call localGroupDB_%getProperty('radiusHalfLight'   ,sizesTarget             ,isPresentSizes                                           )
    call localGroupDB_%getProperty('radiusHalfLight'   ,sizesUncertaintiesTarget,isPresentSizesUncertainties,attributeUncertainty         )
    ! Convert sizes to logarithmic values.
    where (isPresentSizes .and. isPresentSizesUncertainties)
       sizesUncertaintiesTarget=sizesUncertaintiesTarget/sizesTarget/log(10.0d0)
    end where
    where (isPresentSizes)
       sizesTarget             =log10(sizesTarget)
    end where
    ! Where sizes have no uncertainty, assign a default one.
    where (isPresentSizes .and. .not.isPresentSizesUncertainties)
       sizesUncertaintiesTarget=sizeUncertaintyDefault*log(10.0d0)
    end where
    ...
    do i=1,size(massesTarget)
       if     (                                     &
            ...
            &  .or.                                 &
            &   .not.isPresentSizes             (i) &
            & ) cycle
       j=int((log10(massesTarget(i))-log10(massMinimum))/(masses(2)-masses(1))+0.5d0,kind=c_size_t)+1_c_size_t
       if (j > 0 .and. j <= binCount) then
          functionValueTarget              (j)=functionValueTarget              (j)+sizesTarget             (i)
          functionVarianceMeasurementTarget(j)=functionVarianceMeasurementTarget(j)+sizesUncertaintiesTarget(i)**2
          functionVarianceSampleTarget     (j)=functionVarianceSampleTarget     (j)+sizesTarget             (i)**2
    ...
    !![
    <referenceConstruct object="outputAnalysisWeightPropertyOperatorSystematicPolynomial_" constructor="outputAnalysisPropertyOperatorSystematicPolynomial(sizeErrorPolynomialZeroPoint,sizeSystematicErrorPolynomialCoefficient)"/>
    !!]
    ...
    weightPropertyOperators_%next%operator_ => outputAnalysisWeightPropertyOperatorSystematicPolynomial_
    ...
    type is (outputAnalysisMeanFunction1D)
	 ...
	 <constructor>
	   ...
	   &amp;                        var_str('localGroupMassSizeRelation'                         ), &amp;
	   &amp;                        var_str('Mass-size relation of Local Group satellites'       ), &amp;
	   &amp;                        var_str('massStellar'                                        ), &amp;
    ...
    return
  end function localGroupMassSizeRelationConstructorInternal
  ...
  subroutine localGroupMassSizeRelationAnalyze(self,node,iOutput)
    !!{RST
    Implement a ``localGroupMassSizeRelation`` output analysis.
    !!}
    implicit none
    ...
    return
  end subroutine localGroupMassSizeRelationAnalyze
  ...
  subroutine localGroupMassSizeRelationFinalize(self,groupName)
    !!{RST
    Implement a ``localGroupMassSizeRelation`` output analysis finalization.
    !!}
    implicit none
    class(outputAnalysisLocalGroupMassSizeRelation), intent(inout)           :: self
  ...
  double precision function localGroupMassSizeRelationLogLikelihood(self)
    !!{RST
    Return the log-likelihood of a ``localGroupMassSizeRelation`` output analysis.
    !!}
    implicit none
    class(outputAnalysisLocalGroupMassSizeRelation), intent(inout) :: self

    localGroupMassSizeRelationLogLikelihood=self%outputAnalysis_%logLikelihood()
    return
  end function localGroupMassSizeRelationLogLikelihood
