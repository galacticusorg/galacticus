...
!!}
  ...
  <mergerTreeFilter name="mergerTreeFilterAll">
   <description>A merger tree filter class which is the \source{all} combination of a set of other filters.</description>
  ...
  type, extends(mergerTreeFilterClass) :: mergerTreeFilterAll
     !!{
     A merger tree filter class which is the \source{all} combination of a set of other filters.
     !!}
     private
...
contains
  ...
  logical function allPasses(self,tree)
    !!{
    Apply a set of filters to a \source{tree} combined with ``all'' operations.
    !!}
    implicit none
