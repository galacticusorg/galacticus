...
contains
  ...
  subroutine Node_Component_Spheroid_Standard_Scale_Set(node)
    !!{
    Set scales for properties of \source{node}. Note that gas masses get an additional scaling down since they can approach
  ...
  subroutine satelliteMerger(self,node)
    !!{
    Transfer any standard spheroid associated with \source{node} to its host halo.
    !!}
  ...
  subroutine Node_Component_Spheroid_Standard_Star_Formation_History_Extend(node,timeEnd)
    !!{
    Extend the range of a star formation history in a standard spheroid component for \source{node}.
    !!}
    use :: Galacticus_Nodes, only : nodeComponentSpheroid, treeNode
    implicit none
  ...
  subroutine Node_Component_Spheroid_Standard_Stellar_Prprts_History_Extend(node,timeEnd)
    !!{
    Extend the range of a stellar properties history in a standard spheroid component for \source{node}.
    !!}
    use :: Galacticus_Nodes, only : nodeComponentSpheroid, treeNode
    implicit none
    type            (treeNode             ), intent(inout), target   :: node
