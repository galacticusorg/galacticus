...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <massDistribution name="massDistributionSphericalAdiabaticGnedin2004">
   <description>
    ...
    that model are specified via input parameters \source{A} and \source{omega} respectively.
    ...
    \end{equation}    
    where the pivot radius $r_0$ is set to $f_0 r_\mathrm{vir}$ where $f_0=$\source{[radiusFractionalPivot]}, and
...
contains
  ...
  double precision function sphericalAdiabaticGnedin2004Density(self,coordinates) result(density)
    !!{
    Return the density at the specified \source{coordinates} in an adiabatically-contracted spherical mass distribution.
    !!}
  ...
  double precision function sphericalAdiabaticGnedin2004MassEnclosedBySphere(self,radius) result(mass)
    !!{
    Computes the mass enclosed within a sphere of given \source{radius} for adiabatically-contracted mass distributions.
    !!}
    implicit none
  ...
  double precision function sphericalAdiabaticGnedin2004RadiusOrbitalMean(self,radius)
    !!{
    Returns the orbit averaged radius for dark matter corresponding the given \source{radius} using the model of
    \cite{gnedin_response_2004}.
    !!}
    implicit none
    class           (massDistributionSphericalAdiabaticGnedin2004), intent(inout) :: self
  ...
  double precision function sphericalAdiabaticGnedin2004RadiusOrbitalMeanDerivative(self,radius)
    !!{
    Returns the derivative of the orbit averaged radius for dark matter corresponding the given \source{radius} using the model of
    \cite{gnedin_response_2004}.
    !!}
    implicit none
    class           (massDistributionSphericalAdiabaticGnedin2004), intent(inout) :: self
