...
!    to be located.
!
!    Output, integer ( kind = 4 ) ITI, the triangle number, when the point is inside the
...
!    estimating partial derivatives at each data point,
!
!    Input, integer ( kind = 4 ) IPC(NCP*NDP), the indices of NCP data points closest to
...
!    Input, integer ( kind = 4 ) NL, the number of border line segments.
!
!    Input, integer ( kind = 4 ) IPL(3*NL), the point numbers of the end points of the 
...
!
!    Input, integer ( kind = 4 ) I1, I2, I3, I4, indices of four points P1, P2,
!    P3, and P4 that form a quadrilateral with P3 and P4 connected diagonally.
!
!    Output, integer ( kind = 4 ) IDXCHG, is 1 if an exchange is recommended, and 0 otherwise.
!
  implicit none
