...
!!}
  ...
  <outputAnalysis name="outputAnalysisMassFunctionStellar">
   <description>Computes the galaxy stellar mass function as a number density per unit log-mass, with user-specified mass bin centers, \mono{label}, \mono{comment}, optional target dataset for likelihood evaluation, and binomial covariance matrix construction parameters for halo mass range.</description>
  </outputAnalysis>
  !!]
