...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <nodePropertyExtractor name="nodePropertyExtractorVelocityDispersion">
   <description>
    ...
    dispersion should be computed. This direction should be one of \source{radial} (computes the radial
    component of velocity dispersion), \source{lineOfSight\{\textless luminosity\textgreater\}} (computes the
    line-of-sight velocity dispersion), \source{lineOfSightInteriorAverage\{\textless luminosity\textgreater\}}
    ...
    \citealt{cappellari_sauron_2007})---in the latter three cases \source{\{\textless luminosity\textgreater\}}
    specifies which band should be used to weight the velocity dispersion, alternatively setting {\normalfont \ttfamily
    \{\textless luminosity\textgreater\}}$=$\source{mass} (or just leaving off this specifier entirely) will use
...
contains
  ...
  integer function velocityDispersionElementCount(self,time)
    !!{
    Return the number of elements in the \source{velocityDispersion} property extractors.
    !!}
    implicit none
  ...
  function velocityDispersionSize(self,time)
    !!{
    Return the number of array elements in the \source{velocityDispersion} property extractors.
    !!}
    implicit none
  ...
  function velocityDispersionExtract(self,node,time,instance)
    !!{
    Implement a \source{velocityDispersion} property extractor.
    !!}
  ...
  subroutine velocityDispersionNames(self,names,time)
    !!{
    Return the names of the \source{velocityDispersion} properties.
    !!}
    implicit none
    class           (nodePropertyExtractorVelocityDispersion), intent(inout)                             :: self
  ...
  subroutine velocityDispersionDescriptions(self,descriptions,time)
    !!{
    Return descriptions of the \source{velocityDispersion} property.
    !!}
    implicit none
    class           (nodePropertyExtractorVelocityDispersion), intent(inout)                             :: self
    double precision                                         , intent(in   ), optional                   :: time
  ...
  subroutine velocityDispersionColumnDescriptions(self,descriptions,values,valuesDescription,valuesUnitsInSI,time)
    !!{
    Return column descriptions of the \source{velocityDispersion} property.
    !!}
    implicit none
    class           (nodePropertyExtractorVelocityDispersion), intent(inout)                            :: self
    double precision                                         , intent(in   ), optional                  :: time
  ...
  function velocityDispersionUnitsInSI(self,time)
    !!{
    Return the units of the \source{velocityDispersion} properties in the SI system.
    !!}
  ...
  double precision function velocityDispersionSolidAngleInCylinder(radius)
    !!{
    Computes the solid angle of a spherical shell of given \source{radius} that lies within a cylinder of radius {\normalfont \ttfamily
    radiusImpact}.
    !!}
    use :: Numerical_Constants_Math, only : Pi
    implicit none
  ...
  double precision function velocityDispersionLineOfSightVelocityDispersionIntegrand(radius)
    !!{
    Compute the line-of-sight velocity dispersion at the given \source{radius}.
    !!}
