...
contains
  ...
  function powerLawDTDDifferentialConstructorParameters(parameters) result(self)
    ...
    <inputParameter>
      ...
      <defaultSource>\citep[][field galaxy DTD]{freundlich_delay_2012}</defaultSource>
      <description>The power-law index $\alpha$ in the Type Ia supernova delay time distribution $\mathrm{d}N/\mathrm{d}t \propto t^\alpha$ (differential form), describing how the rate declines with time after star formation; the default value of $-1.07$ follows \cite{freundlich_delay_2012}.</description>
    </inputParameter>
    <inputParameter>
