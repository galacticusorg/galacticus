  ...
  type, extends(cosmologicalVelocityFieldClass) :: cosmologicalVelocityFieldFilteredPower
   ...
   contains
     ...
     <methods>
      ...
      <method description="Compute the peak correction term for the velocity dispersion of halos of given \source{mass}, e.g. \cite[][eqn.~8]{sheth_peculiar_2001}, and \cite[][eqn. 4.26]{bardeen_statistics_1986}." method="peakCorrection"/>
     </methods>
     !!]
...
contains
  ...
  double precision function filteredPowerVelocityRadialMeanPairwise(self,separation,time,includeHubbleFlow)
    !!{
    Return the mean radial velocity (averaged over all positions) at a given \source{separation} and
    \source{time}. If \source{includeHubbleFlow} is \source{true} then the Hubble
  ...
  double precision function filteredPowerPeakCorrection(self,mass,time)
    !!{
    Compute the peak correction term for the velocity dispersion of halos of given \source{mass},
