...
contains
  ...
  subroutine Node_Component_Hot_Halo_Cold_Mode_Node_Merger(node)
    !!{
    Starve \source{node} by transferring its hot halo to its parent.
    !!}
  ...
  subroutine satelliteMerger(self,node)
    !!{
    Remove any cold mode gas associated with \source{node} before it merges with its host halo.
    !!}
  ...
  subroutine nodePromotion(self,node)
    !!{
    Ensure that \source{node} is ready for promotion to its parent. In this case, we simply
    update the cold mode mass of \source{node} to account for any cold mode gas already in the
    parent.
    !!}
