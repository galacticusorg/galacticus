...
contains
  ...
  double precision function fixedEnergyMean(self,node,host)
    ...
    fixedEnergyMean=Virial_Orbit_Energy_Mean(node,host,self%velocityTotalRootMeanSquared(node,host),Virial_Orbit_Density_Contrast(host,self%virialDensityContrastDefinition_),self%cosmologyParameters_,self%cosmologyFunctions_,self%virialDensityContrast_,self%darkMatterProfileDMO_)
    return
