...
contains
  ...
  function king1962ConstructorParameters(parameters) result(self)
    ...
    !!]
    self=satelliteTidalStrippingRadiusKing1962(efficiencyCentrifugal,applyPreInfall,cosmologyParameters_,darkMatterHaloScale_,satelliteTidalField_)
    !![
  ...
  function king1962ConstructorInternal(efficiencyCentrifugal,applyPreInfall,cosmologyParameters_,darkMatterHaloScale_,satelliteTidalField_) result(self)
    ...
    !![
    <constructorAssign variables="efficiencyCentrifugal, applyPreInfall, *cosmologyParameters_, *darkMatterHaloScale_, *satelliteTidalField_"/>
    !!]
  ...
  double precision function king1962Radius(self,node)
    ...
    frequencyAngular  = +Vector_Magnitude(Vector_Product(position,velocity)) &
         ...
         &              *gigaYear                                            &
         &              /megaParsec
    ! Find the maximum of the tidal field over all directions in the satellite.
    tidalFieldRadial=-self%satelliteTidalField_%tidalTensorDominant(                                         &
         &                                                                                         node    , &
         &                                                          nodeHost                      =nodeHost, &
         &                                                          atPericenter                  =.false. , &
         &                                                          includeCentrifugalAcceleration=.false.   &
         &                                                         )                                         &
         &           *                                             (                                         &
         &                                                          +kilo                                    &
         &                                                          *gigaYear                                &
