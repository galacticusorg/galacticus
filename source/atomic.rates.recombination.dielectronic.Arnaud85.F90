...
contains
  ...
  double precision function arnaud1985Rate(self,atomicNumber,electronNumber,temperature)
    ...
    \source{atomicNumber}: atomic number; \source{electronNumber}: number of electrons;
    \source{temperature}: temperature [K].  Output parameter: rate coefficient [cm$^3$ s$^{-1}$].
    !!}
