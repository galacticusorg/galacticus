...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <powerSpectrumPrimordial name="powerSpectrumPrimordialPowerLaw">
   <description>
    ...
    \end{equation}
    where $n_\mathrm{s}=$\source{[index]} is the power spectrum index at wavenumber
    $k_\mathrm{ref}=$\source{[wavenumberReference]}, $\d n / \d \ln k=$\source{[running]}, and $\d^2 n / \d \ln k^2=$\source{[runningRunning]}
...
contains
  ...
  function powerLawConstructorParameters(parameters) result(self)
    ...
    <inputParameter>
      ...
      <description>If \source{true} then the index runs only for $k > k_\mathrm{ref}$, for smaller $k$ the index is constant.</description>
    </inputParameter>
    !!]
