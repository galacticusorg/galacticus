...
contains
  ...
  double precision function enzoHydrostaticNormalizationDensity(self) result(normalizationDensity)
    !!{
    Return the density normalization in a \source{enzoHydrostatic} mass distribution.
    !!}
    implicit none
  ...
  double precision function enzoHydrostaticDensity(self,coordinates) result(density)
    !!{
    Return the density at the specified \source{coordinates} in an Enzo hydrostatic mass distribution.
    !!}
  ...
  double precision function enzoHydrostaticDensityGradientRadial(self,coordinates,logarithmic) result(densityGradientRadial)
    !!{
    Return the density at the specified \source{coordinates} in an EnzoHydrostatic \citep{navarro_structure_1996} mass distribution.
    !!}
    implicit none
