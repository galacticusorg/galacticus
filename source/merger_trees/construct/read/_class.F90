...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
   ...
   **Dark Matter Scale Radii**:  If ``[presetScaleRadii]``\ :math:`=`\ ``true`` and the ``halfMassRadius`` dataset is available within the ``haloTrees`` group (see :ref:`here &lt;forest-halos-group&gt;`) then the half-mass radii of nodes will be used to compute the corresponding scale length of the dark matter halo profile\ [#]_. This requires a dark matter profile scale component which supports setting of the scale length (see :ref:`here &lt;manual-sec-darkmatterprofilescale&gt;`).
