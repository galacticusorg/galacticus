...
!!{RST
Contains a module which implements dumping of the structure of a merger tree to a file for plotting with `dot <http://www.graphviz.org/>`_.
!!}

module Merger_Trees_Dump
  !!{RST
  Implements dumping of the structure of a merger tree to a file for plotting with `dot <http://www.graphviz.org/>`_.
  !!}
...
contains
  ...
  subroutine Merger_Tree_Dump(tree,highlightNodes,backgroundColor,nodeColor,edgeColor,highlightColor,nodeStyle&
    ...
    !!{RST
    Dumps the tree structure to a file in a format suitable for processing with `dot <http://www.graphviz.org/>`_. Nodes are shown as circles if isolated or rectangles if satellites. Isolated nodes are connected to their descendant halo, while satellites are connected (by red lines) to their host halo. Optionally, a list of node indices to highlight can be specified.
    !!}
