...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <starFormationTimescale name="starFormationTimescaleDynamicalTime">
   <description>
    ...
    \end{equation}
    where $\epsilon_\star=$\source{[efficiency]} and $\alpha_\star=$\source{[exponentVelocity]}
    ...
    minimum value specified by \source{[timescaleMinimum]} (in Gyr).
   </description>
...
contains
  ...
  double precision function dynamicalTimeTimescale(self,component)
    !!{
    Returns the timescale (in Gyr) for star formation in the given \source{component}. The timescale is given by
    \begin{equation}
    \tau_\star = \epsilon_\star^{-1} \tau_\mathrm{dynamical} \left( {V \over 200\hbox{km/s}} \right)^{\alpha_\star},
    \end{equation}
    where $\epsilon_\star$(=\source{efficiency}) is a star formation efficiency and $\alpha_\star$(={\normalfont \ttfamily
