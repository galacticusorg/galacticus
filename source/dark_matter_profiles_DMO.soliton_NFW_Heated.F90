...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  type, extends(darkMatterProfileDMOClass) :: darkMatterProfileDMOSolitonNFWHeated
     ...
     double precision                                             :: radiusCorePrevious                 , radiusSolitonPrevious                     , &
          &                                                          densityCorePrevious                , &
...
contains
  ...
  function solitonNFWHeatedGet(self,node,weightBy,weightIndex) result(massDistribution_)
    ...
    use :: Mass_Distributions        , only : massDistributionSolitonNFWHeated, kinematicsDistributionSolitonNFWHeated, kinematicsDistributionClass, massDistributionSpherical
    implicit none
    class           (darkMatterProfileDMOSolitonNFWHeated), intent(inout)           :: self
    ...
    if (self%radiusCorePrevious < 0.0d0) then
       call self%computeProperties(node,radiusCore,radiusSoliton,densityCore,massCore)
  ...
  subroutine solitonNFWHeatedComputeProperties(self,node,radiusCore,radiusSoliton,densityCore,massCore,weightBy,weightIndex)
    ...
    integer                                               , intent(in   ), optional :: weightIndex
    type            (enumerationWeightByType             )                          :: weightBy_
    double precision                                      , intent(  out)           :: radiusCore                          , radiusSoliton              , &
         &                                                                             densityCore                         , massCore
    ...
    double precision                                                                :: randomOffset                        , massCoreNormal             , &
         &                                                                             expansionFactor
    integer                                                                         :: sampleCountMaximum=50               , status                     , &
         &                                                                             sampleCount
