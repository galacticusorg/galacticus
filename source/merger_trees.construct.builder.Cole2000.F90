...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <mergerTreeBuilder name="mergerTreeBuilderCole2000">
   <description>
    ...
    \begin{description}
     \item [\source{[mergeProbability]}] The maximum probability for a binary merger allowed in a single
     timestep. This allows the probability to be kept small, such the the probability for multiple mergers within a single
     timestep is small.
     \item [\source{[accretionLimit]}] The maximum fractional change in mass due to sub-resolution accretion
     allowed in any given timestep when building the tree.
     \item [\source{[redshiftMaximum]}] The highest redshift to which the tree should be built. Any branch
     ...
     \item [\source{[branchIntervalStep]}] If \source{true}, instead of limiting each time step
     such that the probability of branching is less than \source{mergerTreeBuildCole2000MergeProbability}, the
