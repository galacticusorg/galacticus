  ...
  type :: multiCounter
   ...
   contains
     ...
     <methods>
       ...
       <method method="append"    description="Append a new counter to the multi-counter, with the specified \source{range}."                                                                                                      />
       <method method="increment" description="Increment the state of the multi-counter. Return \source{.false.} if incrementing was not possible (i.e. counter was in the final state), \source{.true.} otherwise."/>
       <method method="isFinal"   description="Return \source{.true.} if the counter is in its final state, \source{.false.} otherwise."                                                                            />
       <method method="state"     description="Return the state of the \source{i}$^\mathrm{th}$ counter."                                                                                                                          />
...
contains
  ...
  subroutine multiCounterAppend(self,range)
    !!{
    Append a new counter with the given \source{range}.
    !!}
    use :: Error, only : Error_Report
    implicit none
    class  (multiCounter), intent(inout)               :: self
