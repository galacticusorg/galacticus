    ...
    if (massBlackHole > 0.0d0) then
       ...
       if (densityGas > densityGasMinimum) then
        ...
        ! Get the nuclear star cluster component
         ...
         position=[radiusAccretion,0.0d0,0.0d0]
         ! Get density of gas at the galactic center.
         densityGas=self%galacticStructure_%density(node,position,coordinateSystem=coordinateSystemCylindrical,componentType=componentTypeNSC,massType =massTypeGaseous)
         ! Check if we have a non-negligible gas density.
         if (densityGas > densityGasMinimum) then
