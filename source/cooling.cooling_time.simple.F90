...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  !![
    ...
    \end{equation}
    where $N=$\source{[degreesOfFreedom]} is the number of degrees of freedom in the cooling gas which has
...
contains
  ...
  double precision function simpleTime(self,node,temperature,density,gasAbundances,chemicalDensities,radiation)
    !!{
    Compute the cooling time (in Gyr) for gas at the given \source{temperature} (in Kelvin), \source{density} (in $M_\odot$
    Mpc$^{-3}$), composition specified by \source{gasAbundances} and experiencing a radiation field as described by \source{radiation}.
    !!}
  ...
  double precision function simpleGradientDensityLogarithmic(self,node,temperature,density,gasAbundances,chemicalDensities,radiation)
    !!{
    Return $\d\ln t_\mathrm{cool}/\d\ln \rho$ for gas at the given \source{temperature} (in Kelvin), \source{density} (in $M_\odot$
    Mpc$^{-3}$), composition specified by \source{gasAbundances} and experiencing a radiation field as described by \source{radiation}.
    !!}
   implicit none
    class           (coolingTimeSimple   ), intent(inout) :: self
  ...
  double precision function simpleGradientTemperatureLogarithmic(self,node,temperature,density,gasAbundances,chemicalDensities,radiation)
    !!{
    Return $\d\ln t_\mathrm{cool}/\d\ln T$ for gas at the given \source{temperature} (in Kelvin), \source{density} (in $M_\odot$
    Mpc$^{-3}$), composition specified by \source{gasAbundances} and experiencing a radiation field as described by \source{radiation}.
    !!}
    implicit none
    class           (coolingTimeSimple   ), intent(inout) :: self
