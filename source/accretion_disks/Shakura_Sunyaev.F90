...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  !!{RST
  Implementation of a :cite:t:`shakura_black_1973` accretion disk.
  !!}

  !![
  <accretionDisks name="accretionDisksShakuraSunyaev" docformat="rst">
   <description>
   A circumnuclear accretion disk class, in which the accretion disks are always described by a radiatively efficient, geometrically thin accretion disk as described by :cite:t:`shakura_black_1973`. The radiative efficiency of the flow is computed assuming that material falls into the black hole without further energy loss from the :term:`ISCO`, while the spin-up rate of the black hole is computed assuming that the material enters the black hole with the specific angular momentum of the :term:`ISCO` (i.e. there are no torques on the material once it begins to fall in from the :term:`ISCO`; :cite:author:`bardeen_kerr_1970` :cite:year:`bardeen_kerr_1970`). For these thin disks, jet power is computed, using the expressions from :cite:author:`meier_association_2001` (:cite:year:`meier_association_2001`; his equations 4 and 5).
   </description>
  </accretionDisks>
  !!]
  type, extends(accretionDisksClass) :: accretionDisksShakuraSunyaev
     !!{RST
     Implementation of a :cite:t:`shakura_black_1973` accretion disk class.
     !!}
     private
   contains
...
contains
  ...
  function shakuraSunyaevConstructorParameters(parameters) result(self)
    !!{RST
    Constructor for the :cite:t:`shakura_black_1973` accretion disk class which takes a parameter set as input.
    !!}
  ...
  double precision function shakuraSunyaevEfficiencyRadiative(self,blackHole,accretionRateMass,accretionDiskType) result(efficiencyRadiative)
    !!{RST
    Return the radiative efficiency of a :cite:t:`shakura_black_1973` accretion disk.
    !!}
  ...
  double precision function shakuraSunyaevPowerJet(self,blackHole,accretionRateMass)
    !!{RST
    Computes the jet power for a :cite:t:`shakura_black_1973` (thin) accretion disk, using the expressions from :cite:author:`meier_association_2001` (:cite:year:`meier_association_2001`; his equations 4 and 5).
    !!}
  ...
  double precision function shakuraSunyaevRateSpinUp(self,blackHole,accretionRateMass) result(rateSpinUp)
    !!{RST
    Compute the rate of spin up of a black hole by a :cite:t:`shakura_black_1973` accretion disk.
    !!}
