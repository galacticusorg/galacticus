...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <nodePropertyExtractor name="nodePropertyExtractorMainBranchStatus">
   <description>
    ...
    tree. The status will be extracted as \source{nodeIsOnMainBranch}, with a value of 1 indicating that the
    node is a primary progenitor of the final halo (i.e. is on the main branch of the tree) and a value of 0 indicating that it
    is not.

    If \source{[includeSubhalos]} is set to true then subhalos of the main branch halo are also assigned a value of
...
contains
  ...
  function mainBranchStatusExtract(self,node,time,instance)
    !!{
    Implement a \source{mainBranchStatus} node property extractor.
    !!}
    implicit none
