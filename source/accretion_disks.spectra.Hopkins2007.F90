...
contains
  ...
  subroutine hopkins2007BuildFile(self)
    ...
    if (makeFile) then
       ...
       call dataset%writeAttribute(unitsMake(unitsInSI=luminositySolar          ,description="Solar luminosities (L☉)",quantity="Solar luminosities (L☉)"),"units")
       call file   %writeDataset  (SED                      ,"SED"                 ,datasetReturned=dataset)
       call dataset%writeAttribute(unitsMake(unitsInSI=luminositySolar          ,description="L☉/Hz",quantity="L☉/Hz"),"units")
       ! Add some metadata.
