...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  type, extends(nodeOperatorClass) :: nodeOperatorEmpiricalMassiveElliptical
     ...
     \end{equation}
     where $M_{\star,0}=$\source{[massStellarFinal]} is the stellar mass in the root node of the tree,
     $\phi_\star=$\source{[rateStarFormationSpecific]}, and $t_0$ is the cosmic time at the root node of the tree.
     !!}
     private
