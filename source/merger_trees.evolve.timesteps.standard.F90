...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  !![
  <mergerTreeEvolveTimestep name="mergerTreeEvolveTimestepStandard" docformat="rst">
   <description>
  ...
  type, extends(mergerTreeEvolveTimestepClass) :: mergerTreeEvolveTimestepStandard
     !!{RST
     Implementation of a merger tree evolution timestepping class which limits the step to the minimum of that given by the ``simple`` and ``satellite`` timesteps.
     !!}
     private
  ...
  <enumeration docformat="rst">
   ...
   <description>
   Enumeration identifying which of the competing timestep criteria (``simple``, ``satellite``, or ``satelliteDestruction``) set the minimum timestep for a given node evolution step.
   </description>
   <entry label="simple"              />
...
contains
  ...
  double precision function standardTimeEvolveTo(self,timeEnd,node,task,taskSelf,report,lockNode,lockType)
    !!{RST
    Determine a suitable timestep for ``node`` by combining the ``simple``, ``satellite``, and ``satelliteDestruction`` timesteps.
    !!}
