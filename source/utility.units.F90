...
module Units_MetaData
  ...
  type, bind(C) :: unitType
     !!{RST
     A derived type that holds unit metadata for a single dataset property.  The ``bind(C)`` attribute ensures a predictable memory layout so that the HDF5 C compound-type offsets computed with ``offsetof`` in ``hdf5_cTypes.c`` are exact.
     !!}
