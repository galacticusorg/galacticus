...
!!}
  ...
  <surveyGeometry name="surveyGeometryLiWhite2009SDSS" docformat="rst">
   ...
   For the angular mask, we make use of the catalog of random points within the survey footprint provided by the NYU-VAGC\footnote{Specifically, `https://zenodo.org/records/10257229/files/lss_random-0.dr72.dat &lt;https://zenodo.org/records/10257229/files/lss_random-0.dr72.dat&gt;`_ (which is a copy of the dataset originally found at the, now defunct, URL ``http://sdss.physics.nyu.edu/lss/dr72/random/lss_random-0.dr72.dat``).} (:cite:author:`blanton_new_2005` :cite:year:`blanton_new_2005`; see also :cite:t:`adelman-mccarthy_sixth_2008,padmanabhan_improved_2008`). :cite:t:`li_distribution_2009` consider only the main, contiguous region and so we keep only those points which satisfy RA\ :math:`&gt;100^\circ`, RA\ :math:`&lt;300^\circ`, and RA\ :math:`&lt;247^\circ` or :math:`\delta&lt; 51^\circ`. When the survey window function is needed, these points are used to determine which elements of a 3D grid fall within the window function.
