...
!!{RST
Implements the survey geometry used by :cite:t:`caputi_stellar_2011`.
!!}

  use :: Cosmology_Functions, only : cosmologyFunctionsClass

  !![
  <surveyGeometry name="surveyGeometryCaputi2011UKIDSSUDS" docformat="rst">
   <description>
   A survey geometry class which implements the UKIDSS UDS survey used by :cite:t:`caputi_stellar_2011`. The survey window function is determined from the set of galaxy positions provided by Caputi (private communication), by finding a suitable bounding box and then cutting out empty regions (corresponding to regions that were removed around bright stars). A set of random points are then found within this mask and are used to find the Fourier transform of the survey volume.
  ...
  interface surveyGeometryCaputi2011UKIDSSUDS
     !!{RST
     Constructors for the :cite:t:`caputi_stellar_2011` survey geometry class.
     !!}
...
contains
  ...
  function caputi2011UKIDSSUDSConstructorParameters(parameters) result(self)
    !!{RST
    Default constructor for the :cite:t:`caputi_stellar_2011` conditional mass function class.
    !!}
    ...
    <inputParameter docformat="rst">
      ...
      <description>
      The redshift bin (0, 1, or 2) of the :cite:t:`caputi_stellar_2011` to use.
      </description>
    </inputParameter>
    !!]
  ...
  function caputi2011UKIDSSUDSConstructorInternal(redshiftBin,cosmologyFunctions_,randomNumberGenerator_) result(self)
    !!{RST
    Internal constructor for the :cite:t:`caputi_stellar_2011` conditional mass function class.
    !!}
  ...
  double precision function caputi2011UKIDSSUDSSolidAngle(self,field)
    !!{RST
    Return the solid angle of the :cite:t:`caputi_stellar_2011` sample. Computed from survey mask (see ``constraints/dataAnalysis/stellarMassFunctions_UKIDSS_UDS_z3_5/surveyGeometryRandoms.py``).
    !!}
    use :: Error, only : Error_Report
    implicit none
    class           (surveyGeometryCaputi2011UKIDSSUDS), intent(inout)           :: self
