...
contains
  ...
  double precision function bode2001HalfModeMass(self,status)
    ...
    bode2001HalfModeMass=Transfer_Function_Mass_From_Wavenumber(self%wavenumberAtSuppression(2.0d0),self%cosmologyParameters_)
  ...
  double precision function bode2001QuarterModeMass(self,status)
    ...
    ! Compute corresponding mass scale. As a default choice, the wavenumber is converted to a length scale assuming
    ! R = λ/2 = π/k [see Eq.(9) of Schneider et al. (2012; http://adsabs.harvard.edu/abs/2012MNRAS.424..684S)].
    bode2001QuarterModeMass=Transfer_Function_Mass_From_Wavenumber(self%wavenumberAtSuppression(4.0d0),self%cosmologyParameters_)
    if (present(status)) status=errorStatusSuccess
    return
  ...
  double precision function bode2001FractionModeMass(self,fraction,status)
    ...
    ! Compute corresponding mass scale. As a default choice, the wavenumber is converted to a length scale assuming
    ! R = λ/2 = π/k [see Eq.(9) of Schneider et al. (2012; http://adsabs.harvard.edu/abs/2012MNRAS.424..684S)].
    bode2001FractionModeMass=Transfer_Function_Mass_From_Wavenumber(self%wavenumberAtSuppression(1.0d0/fraction),self%cosmologyParameters_)
    if (present(status)) status=errorStatusSuccess
    return
