...
module Excursion_Sets_First_Crossing_Farahi
  ...
  integer,                 parameter                   :: varianceTableNumberPerUnit=40,timeTableNumberPerDecade=10,varianceTableNumberPerDecade=400
  ...
  double precision,        parameter                     :: redshiftMinimumRate=0.0d0,redshiftMaximumRate=30.0d0
  double precision                                       :: varianceMaximumRate=0.0d0,timeMinimumRate=0.0d0,timeMaximumRate=0.0d0
  integer                                                :: varianceTableCountRate,varianceTableCountRateBase,timeTableCountRate
  double precision,        allocatable, dimension(:,:,:) :: firstCrossingTableRate
  double precision,        allocatable, dimension(:,:  ) :: nonCrossingTableRate
  double precision,        allocatable, dimension(:    ) :: varianceTableRate,varianceTableRateBase,timeTableRate
  logical                                                :: tableInitializedRate=.false.
  type(fgsl_interp_accel)                                :: interpolationAcceleratorVarianceRate,interpolationAcceleratorVarianceRateBase,interpolationAcceleratorTimeRate
  logical                                                :: interpolationResetVarianceRate=.true.,interpolationResetVarianceRateBase=.true.,interpolationResetTimeRate=.true.
  ...
  double precision                                       :: varianceRatePrevious,timeRatePrevious
...
contains
  ...
  double precision function Excursion_Sets_First_Crossing_Probability_Farahi(variance,time)
    ...
    logical                          :: makeTable
    integer                          :: i,j,iTime,iVariance,jTime,jVariance,loopCount,loopCountTotal
  ...
  subroutine Excursion_Sets_First_Crossing_Rate_Tabulate_Farahi(varianceProgenitor,time)
    ...
    use Kind_Numbers
    implicit none
    double precision    , intent(in)                :: varianceProgenitor,time
    double precision    , parameter                 :: varianceMinimumDefault=1.0d-2
    double precision    , parameter                 :: varianceTolerance     =1.0d-6
    real(kind=kind_quad), dimension(:), allocatable :: varianceTableRateQuad,varianceTableRateBaseQuad,firstCrossingTableRateQuad
    logical                                         :: makeTable
    integer                                         :: i,j,iTime,iVariance,loopCount,loopCountTotal
    double precision                                :: timeProgenitor,varianceMinimumRate
    character(len=6)                                :: label
    type(varying_string)                            :: message
    real(kind=kind_quad)                            :: sigma1f,effectiveBarrierInitial,varianceTableStepRate,crossingFraction
  ...
  double precision function Excursion_Sets_First_Crossing_Rate_Farahi(variance,varianceProgenitor,time)
    ...
    integer                          :: iVarianceProgenitor,jTime,jVariance,jVarianceProgenitor
 ...
 function Excursion_Sets_Barrier_Effective(variance0,time0,variance,time)
    ...
    real(kind=kind_quad), intent(in) :: variance0,variance
    double precision    , intent(in) :: time0,time
 ...
 function erfApproximation(x)
  ...
  subroutine Excursion_Sets_First_Crossing_Farahi_Read_File()
    ...
    double precision, allocatable, dimension(:) :: varianceTableTemporary,varianceTableBaseTemporary
  ...
  function Make_Variance_Range(rangeMinimum,rangeMaximum,rangeNumber,ratioAtMaximum) result (rangeValues)
    ...
    use Galacticus_Error
    implicit none
    double precision, intent(in)             :: rangeMinimum,rangeMaximum,ratioAtMaximum
