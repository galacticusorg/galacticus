...
!!}
  ...
  <surveyGeometry name="surveyGeometryTomczak2014ZFOURGE">
   <description>
    ...
    For the angular mask, we make use of \source{mangle} polygon files constructed by hand using vertices
    matched approximately to the distribution of galaxies in the survey (positions of which were provided by R.~Quadri; private
    communication). The solid angle of each mask is computed using the \source{mangle} {\normalfont \ttfamily
    harmonize} command.
