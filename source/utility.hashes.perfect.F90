...
module Hashes_Perfect
  ...
  type hashPerfect
     ...
     integer(kind=kind_int8)                            :: rowSize,hashSize
...
contains
  ...
  subroutine Hash_Perfect_Create(hash,keys,values,keepInverseTable)
    ...
    integer(kind=kind_int8), dimension(:,:), allocatable            :: A    ! A(i,j)=K (i=K/t, j=K mod t) for each key K.
    integer(kind=kind_int8), dimension(:,:), allocatable            :: B    ! B(i,j)=v (i=K/t, j=K mod t) for each key K.
    integer(kind=kind_int8), dimension(:),   allocatable            :: resizeTemp
    ! row() exists to facilitate sorting the rows of A() by their "fullness".
    type(rowStructure),      dimension(:),   allocatable            :: row  ! Entry counts for the rows in A().
    integer(kind=kind_int8)                                         :: hashTableMax,iRow,iColumn,iKey,i,j,k,offset
