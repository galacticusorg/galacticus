  ...
  type keplerOrbit
   ...
   contains
     ...
     <methods>
       ...
       <method description="Resets orbit properties. If the optional \source{keep} argument is provided and listed properties will \emph{not} be reset." method="reset" />
...
contains
  ...
  subroutine Kepler_Orbits_Builder(self,keplerOrbitDefinition)
    !!{
    Build a \source{keplerOrbit} object from the given XML \source{keplerOrbitDefinition}.
    !!}
  ...
  subroutine Kepler_Orbits_Dump_Raw(self,fileHandle)
    !!{
    Dump a \source{keplerOrbit} object in binary.
    !!}
    implicit none
    class  (keplerOrbit), intent(in   ) :: self
  ...
  subroutine Kepler_Orbits_Read_Raw(self,fileHandle)
    !!{
    Read a \source{keplerOrbit} object in binary.
    !!}
    implicit none
    class  (keplerOrbit), intent(inout) :: self
    integer             , intent(in   ) :: fileHandle
  ...
  subroutine Kepler_Orbits_Masses_Set(orbit,massSatellite,massHost)
    !!{
    Sets the masses of the two orbiting objects in a \source{keplerOrbit} object.
    !!}
    implicit none
    class           (keplerOrbit), intent(inout) :: orbit
  ...
  logical function Kepler_Orbits_Equivalent(orbit1,orbit2) result(equivalent)
    !!{
    Test whether two \source{keplerOrbit} objects are equal.
    !!}
    implicit none
