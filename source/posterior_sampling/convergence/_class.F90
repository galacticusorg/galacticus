...
module Posterior_Sampling_Convergence
  ...
  <functionClass docformat="rst">
   ...
   <description>
   Class providing convergence criteria for Bayesian posterior sampling simulations---diagnostics that assess whether the Markov chains have adequately explored the posterior distribution and can be declared converged. Methods return whether convergence has been reached (and at which step), reset the convergence calculation when chains are restarted, log convergence diagnostics to file, and identify outlier chains. The default implementation uses the Gelman-Rubin :math:`\hat{R}` statistic, which compares within-chain and between-chain variances across all active walkers.
   </description>
