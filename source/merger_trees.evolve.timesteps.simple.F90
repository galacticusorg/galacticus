...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <mergerTreeEvolveTimestep name="mergerTreeEvolveTimestepSimple">
   <description>
    ...
    \end{eqnarray}
    where $t_\mathrm{simple}=$\source{[timestepSimpleAbsolute]}, $\epsilon_\mathrm{simple}=${\normalfont \ttfamily
...
contains
  ...
  double precision function simpleTimeEvolveTo(self,timeEnd,node,task,taskSelf,report,lockNode,lockType)
    !!{
    Determine a suitable timestep for \source{node} using the simple method. This simply selects the smaller of {\normalfont \ttfamily
    timeStepAbsolute} and \source{timeStepRelative}$H^{-1}(t)$.
    !!}
