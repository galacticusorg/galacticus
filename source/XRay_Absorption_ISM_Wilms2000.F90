...
program XRay_Absorption_ISM_Wilms2000
  ...
  ! Write energy table.
  call outputFile%writeDataset(energy(1:energyCount),datasetName="energy",description="Photon energy in keV",datasetReturned=myDataset)
  call myDataset %writeAttribute(kilo*electronVolt,"unitsInSI")
  call myDataset %close()
  ! Write metallicity table.
  call outputFile%writeDataset(metallicity,datasetName="metallicity",description="Metallicity")
  ! Write crossSection table.
  call outputFile%writeDataset(crossSection,datasetName="crossSection",description="Absorption cross section in cm²",datasetReturned=myDataset)
