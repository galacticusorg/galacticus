...
!!{RST
Contains a program which tests the :cite:t:`zhao_accurate_2009` halo mass formation history and halo concentration algorithms in an open Universe.
!!}

program Test_Zhao2009_Open
  !!{RST
  Tests the :cite:t:`zhao_accurate_2009` halo mass formation history and halo concentration algorithms in an open Universe. Comparisons are made to the ```mandc'' <https://web.archive.org/web/20220313133927/http://202.127.29.4/dhzhao/mandc_calculator.htm>`_ Note that comparison tolerances are relatively large since we have not attempted to match details (such as critical density calculation) with "``mandc``".
  !!}
  ...
  do iMass=1,size(logarithmicHaloMasses)
     ...
     fileName=inputPath(pathTypeExec)//'testSuite/data/zhao2009MassAccretionHistories/mandcoutputOpenlgM'
