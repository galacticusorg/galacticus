...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <starFormationRateSurfaceDensityDisks name="starFormationRateSurfaceDensityDisksKennicuttSchmidt">
   <description>
    ...
    \end{equation}
    where $A=$\source{[normalization]} and $N=$\source{[exponent]} are parameters. Optionally, if
    the \source{[truncate]} parameter is set to true, then the star formation rate is truncated below a critical
    ...
    \end{equation}
    Here, $\alpha=$\source{[exponentTruncated]} and $\Sigma_\mathrm{crit}$ is a critical surface density for
    ...
    and $q_\mathrm{crit}=$\source{[toomreParameterCritical]} is a dimensionless constant of order unity which
...
contains
  ...
  double precision function kennicuttSchmidtRate(self,node,radius)
    !!{
    Returns the star formation rate surface density  (in $M_\odot$ Gyr$^{-1}$ Mpc$^{-2}$) for star formation in the galactic disk of \source{node}. The disk is assumed to obey the Kennicutt-Schmidt law:
    \begin{equation}
    \Sigma_\star = A \left(x_\mathrm{H} {\Sigma_\mathrm{gas}\over M_\odot \hbox{pc}^{-2}}\right)^N,
    \end{equation}
    where $A=$\source{[normalization]} and $N=${\normalfont \ttfamily
    ...
    $q_\mathrm{crit}=$\source{[toomreParameterCritical]} is a dimensionless constant of order unity which controls where the critical
    density occurs. $\sigma_\mathrm{gas}$ is assumed to be a constant equal to \source{[velocityDispersionDiskGas]} and the disk is
