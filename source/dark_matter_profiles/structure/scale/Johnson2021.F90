...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <darkMatterProfileScaleRadius name="darkMatterProfileScaleRadiusJohnson2021" docformat="rst">
    ...
    where\ [#]_ :math:`\gamma =`\ ``[correlationRateDecay]``, :math:`\mu =`\ ``[correlationExponent]`` and :math:`M_i` is the mass of the :math:`i^\mathrm{th}` halo in the sub-branch. This results in a scale radius along the sub-branch with the correct mean and scatter, but correlated over mass increment scales in a way that matches the predictions of this algorithm.

    .. [#] For leaf nodes, there are no progenitors for which to apply the above energy calculation, and for halos sufficiently close to the mass resolution the energy calculation may not be reliable due to the poorly-resolved formation history of the node.
    .. [#] As scatter will be added directly by the present class.
    .. [#] These values were found by fitting to results from this class.
    </description>
