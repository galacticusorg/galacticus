...
program Tests_Tree_Branch_Destroy
  !!{RST
  Tests that destroying a branch of a merger tree correctly updates sibling and parent pointers in the remaining nodes, avoiding dangling pointer references that would trigger errors under memory-checking tools such as Valgrind.
  !!}
