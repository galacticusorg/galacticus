...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <nodePropertyExtractor name="nodePropertyExtractorSEDAGN">
    <description>
      ...
      [wavelengthMinimum]} and \source{[wavelengthMaximum]}. If \source{[resolution]} is set to a
      ...
      \refClass{accretionDiskSpectraClass} object. The frame for the SED, \source{rest} or {\normalfont \ttfamily
      observed}, is specified by \source{[frame]}. Note that using \source{observed} merely means
...
contains
  ...
  function sedAGNConstructorParameters(parameters) result(self)
    ...
    <inputParameter>
      ...
      <description>The frame (\source{rest} or \source{observed}) for which to compute the SED.</description>
    </inputParameter>
    <inputParameter>
  ...
  integer function sedAGNElementCount(self,time)
    !!{
    Return the number of elements in the \source{sedAGN} property extractors.
    !!}
    implicit none
  ...
  function sedAGNSize(self,time) result(size)
    !!{
    Return the number of array elements in the \source{sedAGN} property extractors.
    !!}
    use :: Error                         , only : Error_Report
  ...
  function sedAGNExtract(self,node,time,instance) result(sed)
    !!{
    Implement a \source{sedAGN} property extractor.
    !!}
    use :: Error                         , only : Error_Report
    use :: Stellar_Luminosities_Structure, only : frameRest   , frameObserved
    implicit none
  ...
  subroutine sedAGNNames(self,names,time)
    !!{
    Return the names of the \source{sedAGN} properties.
    !!}
    implicit none
    class           (nodePropertyExtractorSEDAGN), intent(inout)                             :: self
  ...
  subroutine sedAGNDescriptions(self,descriptions,time)
    !!{
    Return descriptions of the \source{agnSED} property.
    !!}
    implicit none
    class           (nodePropertyExtractorSEDAGN), intent(inout)                             :: self
    double precision                             , intent(in   ), optional                   :: time
  ...
  subroutine sedAGNColumnDescriptions(self,descriptions,values,valuesDescription,valuesUnitsInSI,time)
    !!{
    Return column descriptions of the \source{sedAGN} property.
    !!}
  ...
  function sedAGNUnitsInSI(self,time) result(unitsInSI)
    !!{
    Return the units of the \source{sedAGN} properties in the SI system.
    !!}
