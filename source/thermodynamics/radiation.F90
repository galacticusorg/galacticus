...
contains
  ...
  double precision function Blackbody_Emission(wavelength,temperature,radianceType)
    !!{RST
    Compute the Planck blackbody spectral radiance (defined per unit wavelength, in units of J s\ :math:`^{-1}` m\ :math:`^{-2}` sr\ :math:`^{-1}` Å\ :math:`^{-1}`, or J s\ :math:`^{-1}` m\ :math:`^{-2}` sr\ :math:`^{-1}` Hz\ :math:`^{-1}` depending on the optional ``radianceType`` argument). Input ``wavelength`` is in Angstroms, input temperature is in Kelvin.
    !!}
