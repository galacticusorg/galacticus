...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <powerSpectrumWindowFunction name="powerSpectrumWindowFunctionTopHatSharpKHybrid">
   <description>
    ...
    \ttfamily [normalization]}$=$\source{natural} then the value of
...
contains
  ...
  double precision function topHatSharpKHybridValue(self,wavenumber,smoothingMass)
    ...
    cut-off wavenumber is $k_\mathrm{s}=a/r_\mathrm{s}$, where $a=$\source{[normalization]}. The two radii are
    chosen such that $r_\mathrm{th}^2 + r_\mathrm{s}^2 = (3 M / 4 \pi \bar{rho})^{1/3}$ and $r_\mathrm{s}=\beta r_{\mathrm
    th}$ where $\beta=$\source{[radiiRatio]}.
    !!}
    implicit none
  ...
  double precision function topHatSharpKHybridWavenumberMaximum(self,smoothingMass)
    ...
    and $r_\mathrm{s}=\beta r_\mathrm{th}$ where $\beta=$\source{[radiiRatio]}.
    !!}
    implicit none
    class           (powerSpectrumWindowFunctionTopHatSharpKHybrid), intent(inout) :: self
  ...
  subroutine topHatSharpKHybridRadii(self,smoothingMass,radiusTopHat,radiusKSpaceSharp)
    ...
    $k_\mathrm{s}=a/r_\mathrm{s}$, where $a=$\source{[normalization]}. The two radii are chosen such that $r_\mathrm{th}^2 +
    r_\mathrm{s}^2 = (3 M / 4 \pi \bar{rho})^{1/3}$ and $r_\mathrm{s}=\beta r_\mathrm{th}$ where $\beta=$\source{[radiiRatio]}.
    !!}
    use :: Numerical_Constants_Math, only : Pi
    implicit none
    class           (powerSpectrumWindowFunctionTopHatSharpKHybrid), intent(inout) :: self
