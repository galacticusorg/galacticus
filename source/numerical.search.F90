...
contains
  ...
  function searchArrayDouble(arrayToSearch,valueToFind)
    !!{
    Searches an array, $x=($\source{arrayToSearch}$)$, for value, $v(=$\source{valueToFind}$)$,
  ...
  function searchArrayClosest(arrayToSearch,valueToFind,tolerance,status)
    !!{
    Searches an array, $x=($\source{arrayToSearch}$)$, for the entry closest to value, $v(=${\normalfont
  ...
  function searchIndexedInteger8(arrayToSearch,arrayIndex,valueToFind)
    !!{
    Searches a long integer array, $x=($\source{arrayToSearch}$)$, which is rank ordered when indexed by
    \source{arrayIndex}, for value, $v(=$\source{valueToFind}$)$, to find the index $i$ such that
    $x(i) \le v < x(i+1)$.
    !!}
