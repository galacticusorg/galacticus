...
!!}
  ...
  type, extends(nbodyOperatorClass) :: nbodyOperatorSubhaloVelocityMaximumMeanFunction
     ...
     type            (varying_string          )          :: simulationReference           , simulationURL, &
          &                                                 description
   contains
     final     ::            subhaloVelocityMaximumMeanFunctionDestructor
     procedure :: operate => subhaloVelocityMaximumMeanFunctionOperate
  ...
  end interface nbodyOperatorSubhaloVelocityMaximumMeanFunction
...
contains
  ...
  function subhaloVelocityMaximumMeanFunctionConstructorParameters(parameters) result (self)
    ...
    !!]
    self=nbodyOperatorSubhaloVelocityMaximumMeanFunction(massMinimum,massMaximum,massCountPerDecade,description,simulationReference,simulationURL,cosmologyParameters_)
    !![
...
&                                                                                               description
  ...
  subroutine subhaloVelocityMaximumMeanFunctionOperate(self,simulations)
    ...
    implicit none
    class           (nbodyOperatorSubhaloVelocityMaximumMeanFunction), intent(inout)               :: self
    type            (nBodyData                                      ), intent(inout), dimension(:) :: simulations
    double precision                                                 , allocatable  , dimension(:) :: velocityFunction            , massBin        , &
         &                                                                                            velocityFunctionVariance
    double precision                                                 , pointer      , dimension(:) :: mass                        , velocityMaximum
...
#endif
       call displayIndent('compute subhalo mean velocity maximum function',verbosityLevelStandard)
#ifdef USEMPI
    end if
#endif
    ...
    allocate(velocityFunction        (massCount))
    allocate(velocityFunctionVariance(massCount))
    ...
    do iSimulation=1_c_size_t,size(simulations)
       ...
       ! Get the velocity maximum data.
       if (simulations(iSimulation)%propertiesReal%exists('velocityMaximum')) then
          velocityMaximum => simulations(iSimulation)%propertiesReal%value('velocityMaximum')
       else
          call Error_Report('velocity maxima are required, but are not available in the simulation'//{introspection:location})
       end if
       ! Accumulate counts.
#ifdef USEMPI
       if (mpiSelf%isMaster()) then
...
#endif
       countBin                =0_c_size_t
       velocityFunction        =0.0d0
       velocityFunctionVariance=0.0d0
       !$omp parallel do private(j) reduction(+:countBin,velocityFunction,velocityFunctionVariance) schedule(dynamic)
       do i=1_c_size_t,size(mass,kind=c_size_t)
          ...
          if (j >= 1 .and. j <= massCount) then
             countBin                (j)=+countBin                (j)+1_c_size_t
             velocityFunction        (j)=+velocityFunction        (j)+velocityMaximum(i)
             velocityFunctionVariance(j)=+velocityFunctionVariance(j)+velocityMaximum(i)**2
          end if
          ! Update progress.
       ...
       velocityFunction        =mpiSelf%sum(velocityFunction        )
       velocityFunctionVariance=mpiSelf%sum(velocityFunctionVariance)
#endif
       ! Compute velocity means.
       where (countBin > 0_c_size_t)
          velocityFunction        =+           velocityFunction             &
               &                   /      dble(countBin                )
          velocityFunctionVariance=+           velocityFunctionVariance     &
               &                   /      dble(countBin                )
          velocityFunctionVariance=+sqrt(                                   &
               &                         +(                                 &
               &                           +   velocityFunctionVariance     &
               &                           -   velocityFunction         **2 &
               &                          )                                 &
       ...
       if (mpiSelf%isMaster()) then
          ...
          call velocityMaximumFunctionGroup%writeDataset  (velocityFunction         ,'velocityMaximumMean'     )
          call velocityMaximumFunctionGroup%writeDataset  (velocityFunctionVariance ,'velocityMaximumMeanError')
          call velocityMaximumFunctionGroup%writeAttribute(self%description         ,"description"             )
          call velocityMaximumFunctionGroup%writeAttribute(Formatted_Date_and_Time(),"timestamp"               )
          call velocityMaximumFunctionGroup%close         (                                                    )
...
#endif
    return
  end subroutine subhaloVelocityMaximumMeanFunctionOperate
