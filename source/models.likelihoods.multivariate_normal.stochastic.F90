...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  <posteriorSampleLikelihood name="posteriorSampleLikelihoodMltiVrtNormalStochastic">
   <description>
    The likelihood is identical to that of the \source{multivariateNormal} class, except that the likelihood function
    is evaluated stochastically. In addition to the parameter of the \source{multivariateNormal} class, two additional
    parameters are required and are specified within the \source{likelihood} element using:
    \begin{verbatim}
    ...
    \end{equation}
    where $N=$\source{realizationCount} and $U(x)$ is a uniform random deviate in the range $0$ to $x$. This results in
    ...
    realizations is reduced to $N/T$, but never allowed to fall below \source{realizationCountMinimum}.
   </description>
