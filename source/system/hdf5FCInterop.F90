...
!!{RST
! Contains a program which determines the C-interoperable Fortran types that correspond to the HDF5 ``hid_t``, ! ``hsize_t``, and ``size_t`` types on the current system.
!!}

program hdf5FCInterop
  !!{RST
  ! Determines C-interoperable Fortran types that correspond to HDF5 types. The results are written to standard output and are ! used during the build process to select appropriate ``ISO_C_Binding`` type constants for each HDF5 type, avoiding ! potential compiler warnings about C interoperability that would arise from using the HDF5-provided type kinds directly.
  !!}
