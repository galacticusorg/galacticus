...
contains
  ...
  double precision function cotDouble(x)
    !!{
    Implements cotangent for double precision \source{x}.
    !!}
    implicit none
  ...
  double complex function cotDoubleComplex(x)
    !!{
    Implements cotangent for double precision complex \source{x}.
    !!}
    implicit none
  ...
  double precision function cosecDouble(x)
    !!{
    Implements cosecant for double precision \source{x}.
    !!}
    implicit none
    double precision, intent(in   ) :: x
  ...
  double complex function cosecDoubleComplex(x)
    !!{
    Implements cosecant for double precision complex \source{x}.
    !!}
    implicit none
    double complex, intent(in   ) :: x
