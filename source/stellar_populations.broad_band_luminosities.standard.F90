                ...
                if (calculateLuminosity) then
                   ...
                   if (self%storeToFile) then
                      ...
                      if (.not.luminositiesFile%hasDataset(trim(datasetName))) &
                           & call luminositiesFile%writeDataset(self%luminosityTables(populationID)%luminosity(luminosityIndex(iLuminosity),:,:),datasetName=trim(datasetName),description="Tabulated luminosities at redshift z="//adjustl(trim(redshiftLabel)))
                      ! Close the file.
                      call luminositiesFile%close()
                      !$ call hdf5Access%unset()
                      call File_Unlock(lockFileDescriptor)
