...
use Galacticus::Build::SourceTree::Process::FunctionClass::LinkedList qw(deepCopyLinkedList stateStoreLinkedList allowedParametersLinkedList autoDescriptorLinkedList assignerLinkedList linkedListRegisterVariable linkedListModule);
use Galacticus::Build::SourceTree::Process::FunctionClass::Descriptor qw(potentialDescriptorParameters);
use Galacticus::Build::SourceTree::Process::FunctionClass::DeepCopy qw(deepCopyDeclarations deepCopyCopiedSelfBlock generateAssignmentAllocatableCode);
use Galacticus::Build::SourceTree::Process::FunctionClass::StateStore qw(stateStoreExplicitFunction stateStoreVariables generateAllocatableStateStoreCode);
...
sub Process_FunctionClass {
	...
	if ( $node->{'type'} eq "functionClass" ) {
	    ...
	    (my $classesRef, my $classesArrRef, my $nonAbstractClassesRef) = &loadAndSortClasses($directive, $directiveLocations);
...
sub loadAndSortClasses {
    ...
    return (\%classes, \@classes, \@nonAbstractClasses);
}
...
sub initCodeContent {
    ...
    return ($codeContent, $modulePreContains, $modulePostContains);
}

sub generateTypeDefinition {
    my $directive         = shift();
    my $methods           = shift();  # hashref
    my $modulePreContains = shift();
    my $node              = shift();
    my $xml               = shift();
    my %methods           = %{$methods};
...
sub generateConstructor {
    my $directive          = shift();
    my $classes            = shift();  # arrayref
    my $nonAbstractClasses = shift();  # arrayref
    my $modulePreContains  = shift();
    my $modulePostContains = shift();
    my $node               = shift();
    my $tree               = shift();
    my @classes            = @{$classes};
...
sub generateClassSubmodules {
    my $directive          = shift();
    my $classes            = shift();  # arrayref
    my $nonAbstractClasses = shift();  # arrayref
    my $modulePreContains  = shift();
    my $codeContent        = shift();
    my $node               = shift();
    my @classes            = @{$classes};
    my @nonAbstractClasses = @{$nonAbstractClasses};
...
sub isFunctionClassPointer {
    # Return true if the declaration is a pointer to a functionClass (or functionClass instance).
