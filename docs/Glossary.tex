...
\newglossaryentry{ddm}{type=\acronymtype, name={DDM}, description=\glslink{ddmg}{decaying dark matter}, first={decaying dark matter (DDM)}, firstplural={decaying dark matters (DDM)}}
...
\newglossaryentry{parent}{name={parent}, description={In a merger tree, the parent of a \gls{node} has one of two meanings, depending on whether that node is an \gls{isolatedHalo} or a \gls{satellite}. For an isolated node the parent is the node to which it is directly connected in the tree at time $t_1>t_0$, i.e. the node into which it descends at the next timestep. For a satellite node the parent is instead its \gls{host}}}

\newglossaryentry{child}{name={child}, description={In a merger tree, the children of a \gls{node} are the nodes at the preceding timestep which merge to form it, i.e. its progenitors. They form a list, entered via the \mono{firstChild} pointer of the parent node and continued via the \gls{sibling} pointer of each child}}

\newglossaryentry{sibling}{name={sibling}, description={In a merger tree, the sibling of a \gls{node} is the next node in the list to which that node belongs: for an \gls{isolatedHalo} it is the next \gls{child} of the same \gls{parent}, while for a \gls{satellite} it is the next satellite of the same \gls{host}}}

\newglossaryentry{host}{name={host}, description={The \gls{node} within which a \gls{satellite} resides. A node which is not a satellite is considered to be its own host}}

\newglossaryentry{satellite}{name={satellite}, description={A \gls{node} which has fallen into, and now orbits within, another node---also referred to as a subhalo. The galaxy which it contains is known as a satellite galaxy. A satellite may itself \gls{host} further satellites}}

\newglossaryentry{isolatedHalo}{name={isolated halo}, description={A \gls{node} which is not contained within any other node---the counterpart of a \gls{satellite}. The galaxy which it contains is known as a central galaxy}}
...
\newglossaryentry{euclidemulator2}{name={EuclidEmulator2}, description={\href{https://github.com/miknab/EuclidEmulator2}{EuclidEmulator2} is a code to emulate the nonlinear correction factor of the matter power spectrum}}

\newglossaryentry{ngenhalofit}{name={NGenHalofit}, description={\href{https://bitbucket.org/ngenhalofitteam/ngenhalofitpublic}{NGenHalofit} is a code to compute the nonlinear matter power spectrum using a recalibration of the halofit model combined with Multi-point Propagator Theory}}

\newglossaryentry{cuba}{name={Cuba}, description={\href{https://feynarts.de/cuba/}{Cuba} is a library for multidimensional numerical integration}}
...
\newglossaryentry{ddmg}{name={DDM},
    description={Decaying dark matter (DDM) is a hypothesized type of dark matter in which the particle decays, on some characteristic lifetime, into lighter daughter particles---which may include a massive daughter that receives a velocity kick, plus massless dark radiation}}
