...
.. warning::
   The ``mergerTreeExtraOutput`` event is triggered by :galacticus-class:`mergerTreeOutputterStandard` alone, so it must be used to *write* extra output only---never to modify the state of a node. State which must be advanced once per output (star formation histories, for example) should instead attach to the ``mergerTreeOutputStateAdvance`` event, which is triggered by the task driving the evolution once each node has been output, and so happens for every :galacticus-class:`mergerTreeOutputterClass` (including :galacticus-class:`mergerTreeOutputterAnalyzer`, used by constrained models, and :galacticus-class:`mergerTreeOutputterNull`).
