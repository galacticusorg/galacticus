...
# --- Fiducial run -----------------------------------------------------------------------------------------------------------
if runModel() != 0:
    print("FAILED: parametric SIDM model failed to run")
    sys.exit(0)
data = gather("outputs/SIDM_parametric.hdf5", [tauName]+sidmNames)
tau  = data[tauName]
if tau.size == 0:
    print("FAILED: parametric SIDM model produced no SIDM node data")
    sys.exit(0)
...
if np.all((tau >= -1.0e-6) & (tau <= 1.0+1.0e-6)):
    ...
    print(f"FAILED: gravothermal time tau outside [0,1] (min={tau.min():.3e}, max={tau.max():.3e})")
    failures += 1
...
if all(np.all(np.isfinite(data[name])) for name in [tauName]+sidmNames):
    ...
    print(f"FAILED: non-finite values in {', '.join(nonFinite)}")
    failures += 1
...
if np.all(densityScale > 0.0) and np.all(radiusScale > 0.0) and np.all(radiusCore >= 0.0):
    ...
    print(f"FAILED: unphysical SIDM profile parameters (min density scale={densityScale.min():.3e}, "
...
if tau.max() > 0.0:
    ...
    print("FAILED: no gravothermal evolution occurred (maximum tau = 0); the test is vacuous")
    failures += 1
...
if velocityMaximumShift.max() > 0.0:
    ...
    print("FAILED: the SIDM-CDM maximum-velocity shift is zero everywhere; the velocity evolution is not exercised")
    failures += 1
...
if runModel("testSuite/parameters/SIDM_parametric_noInteraction.xml") != 0:
    print("FAILED: no-interaction parametric SIDM model failed to run")
    failures += 1
else:
    ...
    if tauZero.size > 0 and tauZero.max() < 1.0e-2:
        ...
        print(f"FAILED: with sigma0 -> 0 the gravothermal evolution was not suppressed (maximum tau = {maximum})")
        failures += 1
    ...
    if velocityMaximumShift.size > 0 and np.all(np.abs(velocityMaximumShift) < 1.0e-3) and np.all(np.abs(radiusMaximumShift) < 1.0e-8):
        ...
        print(f"FAILED: with sigma0 -> 0 the SIDM-CDM differences did not vanish (max|Vmax shift|="
              f"{np.abs(velocityMaximumShift).max():.3e} km/s, max|Rmax shift|={np.abs(radiusMaximumShift).max():.3e} Mpc)")
        failures += 1

# Always exit with status 0; the CI harness detects failures by scanning the output for "FAILED".
if failures > 0:
    print(f"{failures} check(s) did not pass.")
sys.exit(0)
