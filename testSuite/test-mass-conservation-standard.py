...
# Run the model.
subprocess.run(
    "cd ..; ./scripts/aux/launch.py testSuite/parameters/test-mass-conservation-standard.xml --launchMethod local --threadMaximum 1 --ompThreads 4",
    shell=True
)

# Check for failed models.
logFiles = glob.glob("outputs/test-mass-conservation-standard/galacticus_*/galacticus.log")
failures = []
for logFile in logFiles:
    result = subprocess.run(
        f'grep -q -i -e fatal -e aborted -e "Galacticus experienced" {logFile}', shell=True
    )
...
with h5py.File("outputs/test-mass-conservation-standard/galacticus_0:1/galacticus.hdf5", "r") as galacticus:
    ...
    props    = {}
    for key in ("mergerTreeWeight", "diskMassStellar", "diskMassGas",
                "spheroidMassStellar", "spheroidMassGas",
                "hotHaloMass", "hotHaloOutflowedMass", "hotHaloUnaccretedMass",
                "blackHoleMass", "nodeIsIsolated", "basicMass", "mergerTreeIndex"):
...
if np.any(np.abs(massTotal - 1.0) > 1.0e-3):
    print(f"FAILED: mass conservation [standard] failure -> {massTotal - 1.0}")
else:
