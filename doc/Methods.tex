...
\subsubsection{Dark Matter Density Profile Shape}
...
\subsubsection{Halo Formation Events}
...
Extra filters for controlling which galaxies are output can be added using the directive {\tt mergerTreeOutputFilter}. The directive should give the name of the function to be called to apply the filter. A template for this task is:
\begin{verbatim}
  !# <mergerTreeOutputFilter>
  !#  <unitName>Galacticus_Merger_Tree_Output_Filter__Example</unitName>
  !# </mergerTreeOutputFilter>
  logical function Galacticus_Merger_Tree_Output_Filter__Example(thisNode,filterNames,doOutput)
    implicit none
    type(treeNode),       intent(inout), pointer      :: thisNode
    ...
    .
    return
  end function Galacticus_Merger_Tree_Output_Filter__Example
\end{verbatim}
The function will be called for each node, {\tt thisNode}, which is being considered for output. The {\tt filterNames} arrays contains a list of all filters which were requested to be applied. The function should check if its filter is listed in this array and, if so, should determine whether {\tt thisNode} passes its criteria for output. If it does not, {\tt doOutput} should be set to false. If the output criteria are met, then {\tt doOutput} \emph{should not be changed} (as it may already have been set false by some other filter).
