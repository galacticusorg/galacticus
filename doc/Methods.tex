...
\subsubsection{Halo Mass Function Modifiers}

Additional methods for halo mass function modifiers can be added using the {\tt haloMassFunctionModifierMethod} directive. The directive should contain a single argument, giving the name of a subroutine to be called to modify the mass function. For example, the {\tt simpleSystematic} method is described by a directive:
\begin{verbatim}
 !# <haloMassFunctionModifierMethod>
 !#  <unitName>Halo_Mass_Function_Modifier_Simple_Systematic</unitName>
 !# </haloMassFunctionModifierMethod>
\end{verbatim}
Here, {\tt Halo\_Mass\_Function\_Modifier\_Simple\_Systematic} is the name of a subroutine which will be called to modify the mass function. The modification subroutine must have the following form:
\begin{verbatim}
  subroutine Method_Modify(haloTime,haloMass,haloMassFunction)
    implicit none
    ...
    .
    return
  end subroutine Method_Modify
\end{verbatim}
The function should should apply any modifications to the {\tt haloMassFunction} appropriate for the given {\tt haloTime} and {\tt haloMass}.

Currently defined halo mass function modifier methods are:
\begin{description}
 \item [\hyperlink{structure_formation.halo_mass_function.modifier.simple_systematic.F90:halo_mass_function_modifiers_simple_systematic:halo_mass_function_modifier_simple_systematic}{{\tt simpleSystematic}}] Applies simple systematic shifts to the mass function.
\end{description}
