...
Additional methods for computing ram pressure induced mass loss rates in disks can be added using the {\tt ramPressureStrippingMassLossRate(Disks|Spheroids)Method} directive. The directive should contain a single argument, giving the name of a subroutine to be called to initialize the method. For example, the {\tt simple} method for disks is described by a directive:
\begin{verbatim}
