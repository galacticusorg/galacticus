...
Selected with {\tt darkMatterProfileConcentrationMethod}$=${\tt WDM}, under this method the concentration is computed by applying the correction factor of \cite{schneider_non-linear_2012}:
\begin{equation}
c_{\rm WDM} = c_{\rm CDM} \left[ 1 + \gamma_1 \left( {M_{\rm 1/2} \over M_{\rm halo}} \right)^{\gamma_2}\right]^{-1},
\end{equation}
where $\gamma_1=15$, $\gamma_2=0.3$, $M_{\rm 1/2}$ is the mass corresponding to the wavenumber at which the WDM transfer function is suppressed below the CDM transfer function by a factor of 2, and $M_{\rm halo}$ is the mass of the dark matter halo, to a CDM concentration algorithm as specified by {\tt [darkMatterProfileConcentrationCDMMethod]}.
...
The method to be used for computing the satellite vector acceleration due to dynamical friction is specified by the {\tt satelliteDynamicalFrictionMethod} parameter.

\subsection{Null}

Selected with {\tt satelliteDynamicalFrictionMethod}$=${\tt null}, this method assumes that there is no dynamical friction in the system.

\subsection{Chandrasekhar (1943)}

Selected with {\tt satelliteDynamicalFrictionMethod}$=${\tt Chandrasekhar1943}, this method uses the \cite{chandrasekhar_dynamical_1943} formula to compute the acceleration of a satellite at radius $r$ from the center of the host due to dynamical friction:
\begin{equation}
{\bf a}_{DF} = -\frac{4\pi {\rm G}^2M_{\rm sat}\rho_{\rm host}(r)}{v_{\rm sat}^3}\ln \Lambda\left[{\rm erf}(x)-\frac{2x}{\sqrt{\pi}}\exp(-x^2)\right]\mathbf{v}_{\rm sat},
\end{equation}
where $M_{\rm sat}$ and $\mathbf{v}_{\rm sat}$ are the satellite's mass and velocity, respectively, $v_{\rm sat}=|\mathbf{v}_{\rm sat}|$, $\rho_{\rm host}(r)$ is the host's density profile, $\ln\Lambda=${\tt [satelliteDynamicalFrictionChandrasekharCoulombLogarithm]} is the Coulomb logarithm, and $x\equiv v_{\rm sat}/\sqrt{2}\sigma(r)$, where $\sigma(r)$ is the velocity dispersion of the host halo at radius $r$, approximated to be equal to the host virial velocity, $v_{\rm vir}$.
...
The method to be used for computing the satellite mass loss rate due to tidal stripping is specified by the {\tt satelliteTidalStrippingMethod} parameter.

\subsection{Null}

Selected with {\tt satelliteTidalStrippingMethod}$=${\tt null}, this method assumes that there is no tidal stripping in the system.

\subsection{Zentner (2005)}

Selected by default with {\tt satelliteTidalStrippingMethod}$=${\tt Zentner2005}, this method uses the formalism of \cite{zentner_physics_2005} to compute the mass loss rate $\dot{M}_{\rm sat}$:
\begin{equation}
\dot{M}_{\rm sat}=-\alpha \frac{M_{\rm sat}(>r_{\rm tidal})}{T_{\rm orb}},
\end{equation}
where $\alpha=${\tt [satelliteTidalStrippingZentnerRate]}, $T_{\rm orb}$ is the orbital period of the satellite, and $r_{\rm tidal}$ is the tidal radius of the satellite, given by the \cite{king_structure_1962} formula:
\begin{equation}
r_{\rm tidal}=\left(\frac{GM_{\rm sat}}{\omega^2-d^2\Phi/dr^2}\right)^{1/3},
\end{equation}
...
The method to be used for computing the integrated, normalized (i.e. the energy divided by radius squared) tidal heating energy, $Q_{\rm tidal}$, for subhalos is specified by the {\tt satelliteTidalHeatingMethod} parameter.

\subsection{Null}

Selected with {\tt satelliteTidalHeatingMethod}$=${\tt null}, this method assumes that there is no tidal heating.

\subsection{Gnedin (1999)}

Selected by default with {\tt satelliteTidalHeatingMethod}$=${\tt Gnedin1999}, this method uses the formalism of \cite{gnedin_tidal_1999} to compute the heating rate:
\begin{equation}
\dot{Q}_{\rm tidal}=\frac{1}{3}\epsilon\left[1+\left(\frac{T_{\rm shock}}{T_{\rm orb}}\right)^2\right]^{-\gamma}  g_{ij} G^{ij}
\end{equation}
where $T_{\rm orb}$ and $T_{\rm shock}$ are the orbital period and shock duration, respectively, of the satellite, $\epsilon=${\tt [satelliteTidalHeatingGnedinEpsilon]} and $\gamma=${\tt [satelliteTidalHeatingGnedinGamma]} are model parameters, $g_{ij}$ is the tidal tensor, and $G_{ij}$ is the integral with respect to time of $g_{ij}$ along the orbit of the satellite.  Upon tidal heating, a mass element at radius $r_{\rm i}$ expands to radius $r_{\rm f}$, according to the equation
\begin{equation}
\frac{1}{r_{\rm f}}=\frac{1}{r_{\rm i}}-\frac{2r_{\rm i}^3Q_{\rm tidal}}{{\rm G}M_{\rm sat}(<r_{\rm i})}.
\end{equation}
