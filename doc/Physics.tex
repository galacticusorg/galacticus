...
\end{equation}
where $\rho(r)$ is the density profile of the hot halo, and $r_{\rm infall}$ is the infall radius (see \S\ref{AJB}). The cooling rate is also set to zero in halos with virial velocities below {\tt [coolingCutOffVelocity]} at redshifts below {\tt [coolingCutOffRedshift]}.
...
\end{equation}
where $r_{\rm infall}$ is the infall radius (see \S\ref{AJB}) in the hot halo and $\rho(r)$ is the density profile of the hot halo.
...
Selcted with {\tt freefallRadiusMethod}$=${\tt dark matter halo}, this method assumes that the freefall radius corresponds to the radius at which the freefall time in the dark matter halo equals the time available for freefall (see \S\ref{sec:TimeAvailableFreefall}).

\subsection{Cooling: Infall Radius}\index{infall radius}\index{cooling!infall radius}
...
Selected with {\tt starFormationTimescaleDisksMethod}$=${\tt extended Schmidt} this method assumes that the xtended Schmidt law holds \citep{shi_extended_2011}:
\begin{equation}
...
\section{Supermassive Black Hole Binaries: Initial Separation}\index{supermassive black holes!binary separation}
