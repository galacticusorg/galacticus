...
\subsection{Halo Mass Function Modifiers}\index{halo mass function!modifier}\index{dark matter halos!mass function!modifier}

Dark matter halo mass function modifiers are automatically applied to the halo mass function (unless a particular modifier chooses to deactivate itself).

\subsubsection{Simple Systematic}

This modifier applies a simple systematic (intend to model systematic uncertainties in the halo mass function) of the form:
\begin{equation}
 n \rightarrow n [ 1 + \alpha + \beta \log_{10} (M/M_0) ],
\end{equation}
where $n$ is the halo mass function, $\alpha=${\tt [haloMassFunctionSimpleSystematicAlpha]}, $\beta=${\tt [haloMassFunctionSimpleSystematicBeta]}, and $M_0=10^{12}M_\odot$.
...
Selected with {\tt starFormationFeedbackDisksMethod}$=${\tt haloScaling}, this method assumes an outflow rate of:
\begin{equation}
 \dot{M}_{\rm outflow} = f_{\rm out} \left({V_{\rm vir} \over 200\hbox{km/s}}\right)^\alpha (1+z)^\beta {\dot{E} \over E_{\rm canonical}},
\end{equation}
where $V_{\rm vir}$ is the virial velocity of the halo, $f_{\rm out}=${\tt [diskOutflowFraction]}, $\alpha=${\tt [diskOutflowVirialVelocityExponent]}, $\beta=${\tt [diskOutflowRedshiftExponent]}, $\dot{E}$ is the rate of energy input from stellar populations and $E_{\rm canonical}$ is the total energy input by a canonical stellar population normalized to $1 M_\odot$ after infinite time.
