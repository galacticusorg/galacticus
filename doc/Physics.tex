...
\subsection{Naoz \& Barkana (2007)}\label{phys:accretionHalo:accretionHaloNaozBarkana2007}

Selected using {\normalfont \ttfamily accretionHaloMethod}$=${\normalfont \ttfamily naozBarkana2007} this method uses the filtering mass prescription of \cite{naoz_formation_2007} to compute the accreted rate of baryonic material. Specifically, \cite{naoz_formation_2007} assume that the gas mass content of halos is given by $M_{\rm g}(M_{\rm 200b},M_{\rm F}) = (\Omega_{\rm b} / \Omega_{\rm M}) f(M_{\rm 200b}/M_{\rm F}) M_{\rm 200b}$ where $M_{\rm F}$ is the filtering mass, as first introduced by \cite{gnedin_effect_2000} but defined following \cite{naoz_formation_2007}, $M_{\rm 200b}$ is the halo mass defined by a density threshold of 200 times the mean background, and
\begin{equation}
  f(x) = [1-(2^{1/3}-1) x^{-1}]^{-3}.
\end{equation}
The accretion rate onto the halo is therefore assumed to be
\begin{equation}
  \dot{M}_{\rm g} = {\Omega_{\rm b} \over \Omega_{\rm M}} {{\rm d} \over {\rm d} M_{\rm 200b}} \left[ f(M_{\rm 200b}/M_{\rm F}) M_{\rm 200b} \right]  \dot{M}_{\rm total}.
\end{equation}
...
\item $M_{\rm total}$ corresponds to $M_{\rm 200b}$; and
\item The growth of halos occurs through smooth accretion, not through merging of smaller halos.
\end{enumerate}
In practice all three assumptions are violated. As such, the mass fraction in the halo will differ from $f(M_{\rm 200b}/M_{\rm F})$. To address this issue, mass is additionally assumed to flow from the hot halo reservoir to the unaccreted mass reservoir at a rate:
\begin{equation}
\dot{M}_{\rm hot} = - {\alpha_{\rm adjust} \over \tau_{\rm dyn}} [M_{\rm hot}+M_{\rm unaccreted}] [f_{\rm accreted}-f(M_{\rm 200b}/M_{\rm F})],
\end{equation}
where $\alpha_{\rm adjust} = $[{\normalfont \ttfamily accretionHaloNaozBarkana2007RateAdjust}].
...
where 
\begin{equation}
\tau_{\mathrm cool}=\hbox{max}\left[ \tau_{\rm infall} \left({V_{\rm max} \over 200 \hbox{km/s}}\right)^{-\gamma_{\rm infall}} (1+z)^{\alpha_{\rm infall}} \left( 1 + \exp\left[ {\log_{10}(V_{\rm max}/(1+z)^{\delta_{\rm infall}}\mathcal{V}_{\rm infall})] \over \Delta \log_{10} \mathcal{V}_{\rm infall}}\right]\right)^{\beta_{\rm infall}}, \tau_{\rm infall,min} \right],
\end{equation}
with $\tau_{\rm infall}=${\normalfont \ttfamily [coolingRateVelocityMaximumScalingTimescale]}, $\tau_{\rm infall,min}=${\normalfont \ttfamily [coolingRateVelocityMaximumScalingTimescaleMinimum]}, $\alpha_{\rm infall}=${\normalfont \ttfamily [coolingRateVelocityMaximumScalingTimescaleExponent]}, $\beta_{\rm infall}=${\normalfont \ttfamily [coolingRateVelocityMaximumScalingCutOffExponent]}, $\gamma_{\rm infall}=${\normalfont \ttfamily [coolingRateVelocityMaximumScalingVelocityExponent]}, $\delta_{\rm infall}=${\normalfont \ttfamily []coolingRateVelocityMaximumScalingCutOffVelocityRedshiftExponent}, $\mathcal{V}_{\rm infall}=${\normalfont \ttfamily [coolingRateVelocityMaximumScalingCutOffVelocity]}, and $\Delta \log_{10} \mathcal{V}_{\rm infall}=${\normalfont \ttfamily [coolingRateVelocityMaximumScalingCutOffWidth]}.
