...
\end{verbatim}
  \item [\href{http://search.cpan.org/~grantm/XML-Simple-2.18/lib/XML/Simple.pm}{{\tt PDL}}] Provides array math handling in Perl. It is used extensively by \glc\ for analysis of modles. On Fedora the following (as root) will install {\tt PDL}:
\begin{verbatim}
...
\end{verbatim}
  \item [\href{http://search.cpan.org/~cerney/PDL-IO-HDF5-0.6/}{{\tt PDL::IO::HDF5}}] Provides a simple Perl interface to HDF5 files. It is used by analysis scripts to extract data from \glc\ output files. Currently, it requires a patch to compile against recent HDF5 versions. Therefore, follow the following instructions (as root) to install this module:
\begin{verbatim}
 wget http://search.cpan.org/CPAN/authors/id/C/CE/CERNEY/PDL-IO-HDF5-0.6.tar.gz
 tar xvfz PDL-IO-HDF5-0.6.tar.gz
 cd PDL-IO-HDF5-0.6
 sed -i~ s/"\('CCFLAGS'\s*=>\s*\".*\)\""/'\1 -DH5_USE_16_API"'/g  Makefile.PL
 perl Makefile.PL
 make
 make test
 make install
...
! Install PDL::IO::HDF5 (more complicated, because it won't install against recent HDF5 versions with some patching):

perl -MCPAN -e "install PDL::IO::HDF5::Dataset"
! This will fail. After it does, locate the downloaded tarball, probably in $HOME/.cpan/sources/authors/id/C/CE/CERNEY:
cd $HOME/.cpan/sources/authors/id/C/CE/CERNEY
tar xvfz PDL-IO-HDF5-0.6.tar.gz
cd PDL-IO-HDF5-0.6
sed -i~ s/"\('CCFLAGS'\s*=>\s*\".*\)\""/'\1 -DH5_USE_16_API"'/g  Makefile.PL
! Open Makefile.PL, locate the list of directories appearing before the comment "# Add new library paths here!!" and add "/your/install/path/lib" to it. Then locate the list of directories appearing before the comment "# Add new header paths here!!" and add "/your/install/path/include" to it. Then save the editted file.
cd ..
tar cvfz PDL-IO-HDF5-0.6.tar.gz PDL-IO-HDF5-0.6
perl -MCPAN -e "install PDL::IO::HDF5::Dataset"
...
wget http://www.hdfgroup.org/ftp/HDF5/current/src/hdf5-1.8.5.tar.gz
tar xvfz hdf5-1.8.5.tar.gz
cd hdf5-1.8.5
setenv F9X gfortran
...
If present, the file {\tt runGalacticus.xml} is parsed for configuration options. It should have the following structure:
\begin{verbatim}
<config>
  <errorReport>
    <name>My Name</name>
