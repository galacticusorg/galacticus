...
\end{verbatim}
The {\normalfont \ttfamily boxLength} element should give the length of the simulation box (the box will be replicated to span the volume covered by the lightcone), while the {\normalfont \ttfamily maximumDistance} element should specify the largest distance in the lightcone to be considered. The {\normalfont \ttfamily fieldOfView} element must specify the geometry of the field of view. Currently, the only allowed value for the {\normalfont \ttfamily geometry} of the {\normalfont \ttfamily fieldOfView} element is {\normalfont \ttfamily square}, in which case the {\normalfont \ttfamily length} element of {\normalfont \ttfamily fieldOfView} should give the length of the side of the square field of view in radians. The {\normalfont \ttfamily origin} element must contain the $x$, $y$, $z$ {\normalfont \ttfamily coordinate}s of the origin of the lightcone within the simulation box, while the {\normalfont \ttfamily unitVectorX} elements must give unit vectors which point along the lightcone (for {\normalfont \ttfamily X}$=1$, and in the two directions perpendicular to the lightcone (for {\normalfont \ttfamily X}$=2$ and 3). The {\normalfont \ttfamily outputs} element contains lists of properties corresponding to the outputs to be used for building the lightcone.
