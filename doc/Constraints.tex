...
This constraint utilizes the stellar mass function for $0.2 < z< 3.0$ galaxies measured by \cite{tomczak_galaxy_2014} from the ZFOURGE survey. The mass function, and its covariance matrix ${\bf C}_{\rm model}$, is calculated as described in \S\ref{sec:otfAnalysis:ALFALFA}, typically on-the-fly as \glc\ is run. If the {\tt --modelDiscrepancies} option is given, then any multiplicative or additive discrepancies found are applied to the model mass function, and any additional covariance is added to the covariance matrix.
...
When computing the Poisson term in the covariance matrix, the number of galaxies per bin is taken directly from tables supplied by R.~Quadri (private communication).

\subsubsection{Muzzin et al. (2013) ULTRAVISTA Stellar Mass Functions}\label{sec:AnalysisMuzzinULTRAVISTAStellarMassFunction}

This constraint utilizes the stellar mass function for $0.2 < z< 4.0$ galaxies measured by \cite{muzzin_evolution_2013} from the ULTRAVISTA survey. The mass function, and its covariance matrix ${\bf C}_{\rm model}$, is calculated as described in \S\ref{sec:otfAnalysis:ALFALFA}, typically on-the-fly as \glc\ is run. If the {\tt --modelDiscrepancies} option is given, then any multiplicative or additive discrepancies found are applied to the model mass function, and any additional covariance is added to the covariance matrix.
...
Once an emulator has been built, on each successive likelihood evaluation the emulated log-likelihood $\log\mathcal{L}_{\rm e}$ and its error estimate $\sigma_{\log\mathcal{L}_{\rm e}}$ are computed. The emulated likelihood is then returned if:
\begin{equation}
...
\subsubsection{{\tt correlation}}

An extension of the {\tt history} state, this type also computes and stores the correlation length in each parameter (which is taken to be the median correlation length over all non-outlier chains).

\subsection{{\tt stoppingCriterion}}

The {\tt stoppingCriterion} section specifies the type of object used to determine when the simulation should be stopped. The type of stopping criterion object to use is specified by the {\tt type} element. The available choices are described in the following subsections.

\subsubsection{{\tt never}}

This type will cause the simulation to never stop.

\subsubsection{{\tt stepCount}}

This type will cause the simulation to stop when at least a number of steps (as specified in the {\tt stopAfterCunt} element) have accrued post-convergence.

\subsubsection{{\tt correlationLengthCount}}

This type will cause the simulation to stop when at least a number of correlation lengths (as specified in the {\tt stopAfterCunt} element) have accrued post-convergence.
