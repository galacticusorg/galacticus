...
\end{verbatim}
where {\tt constraintFile} is a file which describes the details of the calculation to be performed. An example of such a script follows:

\begin{verbatim}
 <constrain>
  <name>verySimplisticToStellarMassFunction</name>
  <workDirectory>my/work/patn</workDirectory>
  <scratchDirectory>/scratch/my/scratch/path</scratchDirectory>
  <report>no</report>
  <cpulimit>1200</cpulimit>
  <memoryLimit>32gb</memoryLimit>
  <environment>LD_LIBRARY_PATH=/my/lib:$LD_LIBRARY_PATH</environment>
  <environment>PATH=/my/path:$PATH</environment>
  ...
  <storeResults>no</storeResults>

  <!-- MCMC method -->
  <threadsPerNode>12</threadsPerNode>
  <threads>300</threads>
  <galacticusThreads>12</galacticusThreads>
  <galacticusSaveState>no</galacticusSaveState>
  <method>differentialEvolution</method>

  <!-- MCMC definition -->
  <monteCarlo>
    <steps>1000000</steps>
    <stepsSkip>10</stepsSkip>
    <goodSteps>1000</goodSteps>
    <temperingFrequency>100</temperingFrequency>
    <maxTemperature>64.0</maxTemperature>
    <proposalWidth>0.01</proposalWidth>
    <epsWidth>0.001</epsWidth>
    <gamma>0.085875</gamma>
  </monteCarlo>
  ...
  <parameters>
    ...
    <parameter>
      ...
      <prior>
        <distribution>uniform</distribution>
        <mapping>logarithmic</mapping>
        <lowerLimit>1.0e-1</lowerLimit>
        <upperLimit>1.0e+1</upperLimit>
      </prior>
    </parameter>
 
  </parameters>

  <!-- Include the standard WMAP-9 cosmological parameters and priors -->
  <xi:include href="../../constraints/parameters/wmap9Cosmology.xml" 
     xmlns:xi="http://www.w3.org/2001/XInclude" />

  <!-- Include a simple model for systematic errors in the halo mass function -->
  <xi:include href="../../constraints/parameters/haloMassFunctionSystematics.xml" 
     xmlns:xi="http://www.w3.org/2001/XInclude" />
...
\item[{\tt scratchDirectory}] \emph{(Optional)} The full path to a scratch directory where \glc\ model outputs and other temporary data will be written.
...
\item[{\tt memoryLimit}] A memory limit to be passed to PBS.
\item[{\tt environment}] One of more such element can appear. Each specifies the value of an environment variable to be set prior to launching the \gls{bie}.
...
\item[{\tt threadsPerNode}] The number of threads to use per node. Typically this would be set to the number of available cores on each node.
\item[{\tt threads}] The total number of threads (i.e. parallel chains) to use.
\item[{\tt galacticusThreads}] The number of parallel OpenMP threads to use for each \glc\ model. It is recommended that this be set to the number of available cores on each node, and semaphoring (see \S{sec:Semaphores}) be used. In this way, each copy of \glc\ on a node will share resources, but as one instance finishes, the others will be able to make use of the freed resources.
\item[{\tt galacticusSaveState}] If {\tt yes} then \glc\ will save its internal state prior to beginning evolution of each merger tree. This is intended for debugging purposes and so should normally be set to {\tt no}.
\item[{\tt checkpoint}] If present, \gls{bie} will be instructed to checkpoint its calculations. The interval (in number of steps) between checkpoints is specified by the {\tt interval} sub-element. If the {\tt autoRestart} sub-element is set to {\tt true} then a second PBS script will be submitted which will restart the calculation once the original job finishes. This second script will submit a further script to take over once it finishes and so on. This can be useful if your PBS queue has a time limit and you want to keep restarting the calculation until it is done. Use with caution however, some systems forbid automated restarts.
\end{description}
...
\begin{description}
\item[{\tt compilation}] Specifies the path to a compilation file to be used for this analysis.
\end{description}

\subsection{MCMC Method}

\begin{description}
\item[{\tt method}] Specifies the \gls{mcmc} algorithm to be used. Currently accepted values are {\tt differentialEvolution} and {\tt metropolisHastings}. See the \gls{bie} documentation for full details of these two algorithms.
\item[{\tt monteCarlo}] Specifies the behavior of the \gls{mcmc} algorithm:
\begin{description}
\item[{\tt steps}] The maximum number of steps to take.
\item[{\tt stepsSkip}] The number of initial steps to skip before beginning to measure convergence.
\item[{\tt goodSteps}] The number of steps to perform after convergence is attained.
\item[{\tt temperingFrequency}] The number of steps between successive temperings (in which the likelihood function is ``heated up'' to help get chains out of local minima).
\item[{\tt maxTemperature}] The maximum temperature to use when tempering.
\item [{\tt minTemperatureStates}] The minimum number of temperatures states when tempering.
\item[{\tt proposalWidth}] Used for the {\tt metroplosHastings} algorithm, specfies the width of proposals.
\item[{\tt gamma}] Used for the {\tt differentialEvolution} algorithm, specifies the fraction of the vector connecting to chain state to be used as the proposal for another chain.
\item[{\tt epsWidth}] Used for the {\tt differentialEvolution} algorithm, specifies the width of the Cauchy distribution added to each proposal to ensure that the chains are positively recurrent.
\item [{\tt grAlpha}] Set Gelman-Rubin convergence outlier detection confidence, $1-\alpha$.
\item [{\tt grNOutlier}] Set number of iterations before applying an outlier test in the Gelman-Rubin convergence statistic.
\item [{\tt grMaxOut}] Set the maximum number of outliers allowed in the Gelman-Rubin convergence statistic.
\item [{\tt grRhatMax}] The correlation coefficient threshold for the Gelman-Rubin convergence statistic.
\end{description}
\end{description}

\subsection{Post-prior}

If present, the {\tt postPrior} block allows the use of the statelog from a previous calculation as the prior on a new calculation (in addition to any explicitly defined priors). 

\begin{description}
 \item [{\tt stateLog}] The full path to the statelog file containing the results of the prior calculation.
 \item [{\tt restart}] If ``{\tt yes}'' then the posterior of the previous calculation is used only to sample the initial states of the new chains. It \emph{is not} used as a prior on the new calculation. If ``{\tt no}'' then the previous posterior is used as the new prior.
 \item [{\tt burn}] The number of steps from the statelog to burn.
\end{description}


\subsection{Parameters and Priors}

The {\tt parameters} section contains a list of all parameters to be varied in the analysis. Each parameter is described by one {\tt parameter} element. That element must contain a {\tt name} element, which gives the name of the parameter, and a {\tt prior} element 
\begin{itemize}
\item[{\tt distribution}] The type of distribution to be used for the prior. Currently accepted values are {\tt uniform} and {\tt normal}.
\item[{\tt mapping}] The mapping of the parameter to which the prior should be applied. Currently allowed values are {\tt linear} (the prior is on the parameter itself), and {\tt logarithmic} (the prior is on the logarithm of the parameter).
\item[{\tt lowerLimit}] The lower limit for the prior.
\item[{\tt upperLimit}] The upper limit for the prior.
\item[{\tt mean}] For {\tt normal} priors, the mean of the normal distribution.
\item[{\tt variance}] For {\tt normal} priors, the variance (\emph{not} the standard deviation) of the normal distribution.
\end{itemize}
