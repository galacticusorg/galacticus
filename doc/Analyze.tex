...
It is possible to prevent the output of data to these groups by setting {\tt [mergerTreeOutput]} to {\tt false}. This can save on both file size and I/O time if you do not intend to perform any analysis on the raw data output by \glc. (For example, if you're only interested in volume averaged quantities output to the {\tt globalHistory} group.)
