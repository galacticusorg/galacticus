...
These functions (see \refPhysics{virialOrbit}) typically use the \mono{basic} component mass in determining parameters of an orbit, since they are typically calibrated to simulations of collisionless matter only.

\subsection{Satellite Merging Timescales}

These functions (see \refPhysics{satelliteMergingTimescales}) typically use the \mono{basic} component mass in determining parameters of an orbit, since they are typically calibrated to simulations of collisionless matter only.

\subsection{Dynamical Friction}

These functions (see \refPhysics{satelliteDynamicalFriction}) evaluate densities through the relevant galactic structure function, and so correctly account for the fraction of the \mono{basic} component mass which is in the form of dark matter.
...
\begin{description}
 \item [\mono{radius}] the numerical value of the radius at which to compute the property (with units specified by the \mono{radiusType} element);
 \item [\mono{radiusType}] specifies the units of the \mono{radius} element---valid options are \mono{diskRadius}, \mono{hotHaloOuterRadius}, \mono{diskHalfMassRadius}, \mono{spheroidRadius}, \mono{spheroidHalfMassRadius}, \mono{darkMatterScaleRadius}, \mono{virialRadius}, just \mono{radius} (which implies radii are given in units of Mpc), \mono{galacticMassFraction\{\textless fraction\textgreater\}}, \mono{stellarMassFraction\{\textless fraction\textgreater\}}, or \newline \mono{galacticLightFraction\{\textless fraction\textgreater\}\{\textless luminosity\textgreater\}}, where the final three form specify a radius containing a fixed \mono{\textless fraction\textgreater} of the galactic or stellar mass, or light respectively (for the case of galactic light, \mono{\textless luminosity\textgreater} specifies the band, e.g. \mono{SDSS\_r:rest:z0.0000});
 \item [\mono{componentType}] specifies which components of the node should be counted---allowed values are \mono{all}, \mono{disk}, \mono{spheroid}, \mono{hotHalo}, \mono{darkHalo}, and \mono{blackHole};
 \item [\mono{massType}] specifies which types of mass should be counted---allowed values are \mono{all}, \mono{dark}, \mono{baryonic}, \mono{galactic}, \mono{gaseous}, \mono{stellar}, and \mono{blackHole}.
\end{description}
