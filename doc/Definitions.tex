...
These functions (see \refPhysics{virialOrbit}) typically use the \source{basic} component mass in determining parameters of an orbit, since they are typically calibrated to simulations of collisionless matter only.

\subsection{Satellite Merging Timescales}

These functions (see \refPhysics{satelliteMergingTimescales}) typically use the \source{basic} component mass in determining parameters of an orbit, since they are typically calibrated to simulations of collisionless matter only.

\subsection{Dynamical Friction}

These functions (see \refPhysics{satelliteDynamicalFriction}) evaluate densities through the relevant galactic structure function, and so correctly account for the fraction of the \source{basic} component mass which is in the form of dark matter.
...
\begin{description}
 \item [\source{radius}] the numerical value of the radius at which to compute the property (with units specified by the \source{radiusType} element);
 \item [\source{radiusType}] specifies the units of the \source{radius} element---valid options are \source{diskRadius}, \source{hotHaloOuterRadius}, \source{diskHalfMassRadius}, \source{spheroidRadius}, \source{spheroidHalfMassRadius}, \source{darkMatterScaleRadius}, \source{virialRadius}, just \source{radius} (which implies radii are given in units of Mpc), \source{galacticMassFraction\{\textless fraction\textgreater\}}, \source{stellarMassFraction\{\textless fraction\textgreater\}}, or \newline \source{galacticLightFraction\{\textless fraction\textgreater\}\{\textless luminosity\textgreater\}}, where the final three form specify a radius containing a fixed \source{\textless fraction\textgreater} of the galactic or stellar mass, or light respectively (for the case of galactic light, \source{\textless luminosity\textgreater} specifies the band, e.g. \source{SDSS\_r:rest:z0.0000});
 \item [\source{componentType}] specifies which components of the node should be counted---allowed values are \source{all}, \source{disk}, \source{spheroid}, \source{hotHalo}, \source{darkHalo}, and \source{blackHole};
 \item [\source{massType}] specifies which types of mass should be counted---allowed values are \source{all}, \source{dark}, \source{baryonic}, \source{galactic}, \source{gaseous}, \source{stellar}, and \source{blackHole}.
\end{description}
