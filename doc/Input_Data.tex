...
The following shoudl be noted when constructing merger tree files:
\begin{itemize}
...
\item \glc\ currently assumes that nodes never move between trees. Specifically, the host and descendent nodes of each node should always be present in the same tree as the node itself. This is not always the case (for example in trees derived from N-body simulations), depending on the algorithms and tree definitions used. Normally, cases where a node's host node cannot be found in the same tree will cause \glc\ to exit with an error. Setting {\tt [mergerTreeReadMissingHostsAreFatal]}$=${\tt false} will instead circumvent this issue by making any such nodes self-hosting (i.e. they become isolated nodes rather than subhalos). Note that this behavior is not a physically correct way to treat such cases---future versions of \glc\ will allow for nodes to move between trees and so will correctly handle such instances.
\end{itemize}
