...
In the above $\dot{M}_{\rm outflow}$ is the net rate of outflow from any components in the node.
...
\end{equation}
with $\tau_{\rm disk, star~formation}$ being the star formation timescale. The outflow rate, $\dot{M}_{\rm outflow, disk}$, is computed for the current star formation rate and gas properties by the prescriptions for non-expulsive supernova feedback (see \S\ref{sec:sneFeedback}) and is piped to the hot halo component.
...
\subsection{``Very Simple'' Implemenation}

\subsubsection{Properties}

The very simple basic properties implementation defines the following properties:
\begin{description}
