...
\begin{description}
 \item [\mono{mass}] The mass of the black hole: $M_\bullet$ \mono{[blackHoleMass]}.
 \item [\mono{spin}] The spin of the black hole, $j_\bullet$ \mono{[blackHoleSpin]}.
 \item [\mono{radialPosition}] The radial position of the black hole: $r_\bullet$ \mono{[blackHoleRadialPosition]} 
\end{description}

\subsubsection{Initialization}

Black holes are not initialized, they are created (with a seed mass given by \mono{blackHoleSeedMass} and zero spin) as needed.
...
\end{eqnarray}
where $\dot{M}_0$ is the rest mass accretion rate, $\epsilon_\mathrm{radiation}$ is the radiative efficiency of the accretion flow feeding the black hole, $\epsilon_\mathrm{jet}$ is the efficiency with which accretion power is converted to jet power and $\dot{j}(M_\bullet,j_\bullet,\dot{M}_0)$ is the spin-up function of that accretion flow (see \refPhysics{accretionDisks}). The rest mass accretion rate is computed assuming Bondi-Hoyle-Lyttleton accretion from the spheroid gas reservoir (with an assumed temperature of \mono{[bondiHoyleAccretionTemperatureSpheroid]}) enhanced by a factor of \mono{[bondiHoyleAccretionEnhancementSpheroid]} and from the host halo (with whatever temperature that hot halo temperature profile specifies; see \refPhysics{hotHaloTemperatureProfile}) enhanced by a factor of \mono{[bondiHoyleAccretionEnhancementHotHalo]}. For accretion from the hot halo, the Bondi radius is limited to the outer radius of the hot halo. Additionally, the accretion rate is limited to:
\begin{equation}
...
If \mono{[bondiHoyleAccretionHotModeOnly]}$=$\mono{true} then the accretion occurs only from that fraction of the hot halo gas which was accreted in the ``hot mode'', otherwise accretion 
...
\end{equation}
where $\epsilon_{\bullet \mathrm{wind}}=$\mono{[blackHoleWindEfficiency]} is the black hole wind efficiency,\newline $s$=\mono{blackHoleWindEfficiencyScalesWithRadiativeEfficiency}, and
\begin{equation}
...
Interactions between black hole triplets are accounted for if \mono{[tripleBlackHoleInteraction]}$=$\mono{true} (and if at least three black holes exist within the \gls{node} of course). In this case the triple is treated as consisting of an inner binary (assumed to be the central black hole and the black hole closest to it) and a third, singleton black hole. When the tertiary black hole reaches a separation of 
\begin{equation}
...
If the \mono{[blackHoleOutputAccretion]}\index{black holes!accretion}\index{accretion!black holes} input parameter is set to true, then rest mass accretion rate (in $M_\odot$ Gyr$^{-1}$), jet power (in $M_\odot$ km$^2$ s$^{-1}$ Gyr$^{-1}$) and radiative efficiency of the black hole\footnote{Technically of the black hole plus accretion disk system.} are output as \mono{blackHoleAccretionRate}, \mono{blackHoleJetPower} and \mono{blackHoleRadiativeEfficiency} respectively.

If the \mono{[blackHoleOutputData]} input parameter is set to true, then the Masses (in $M_\odot$), Spins (for now just a scalar with no direction), final Radius (in $Mpc$), timescales (in $Gyr$) until merger, accretion rates (in $M_\odot per Gyr$) and radiative Efficiencies of all the black holes in the galaxy are given as outputs in the \mono{blackHole} section of the output hdf5. This also saves the tree \gls{node} and merger tree index for further use when using the data.

The outputs of mergers are also automatically saved, as outputs in the \mono{blackHoleMergers} section of the output hdf5. Those outputs are the time at which mergers happened and the mass ratio between  the two merging black holes.
...
Black holes are not initialized, they are created (with a seed mass given by \mono{blackHoleSeedMass}) as needed.
...
If the \mono{[blackHoleOutputAccretion]}\index{black holes!accretion}\index{accretion!black holes} input parameter is set to true, then rest mass accretion rate (in $M_\odot$ Gyr$^{-1}$) is output as \mono{blackHoleAccretionRate}.
...
\begin{description}
 \item [\mono{mass}] The mass of gas in the hot halo: $M_\mathrm{hot}$ \mono{[hotHaloMass]}.
\end{description}
and the following pipes:
\begin{description}
 \item [\mono{hotHaloCoolingMass}] The net cooling rate of gas mass is sent through this pipe. Any \gls{component} may claim this pipe and connect to it, allowing it to receive the cooling gas.
 \item [\mono{outflowingMass}] Galactic components that wish to expel gas due to an outflow can send that mass  through this pipe, where it will be received into the hot halo component. 
\end{description}
...
\begin{description}
 \item [\mono{outflowedMass}] The mass of gas in the outflowed reservoir of the hot halo: $M_\mathrm{outflowed}$ \mono{[hotHaloOutflowedMass]}.
\end{description}
...
This implementation steals the \mono{outflowingMass} pipe from the very simple component and redirects it to the outflowed mass reservoir. The resulting rates of change of hot and outflowed masses are then:
\begin{eqnarray}
...
\begin{description}
 \item [\mono{unaccretedMass}] The mass of gas which could have accreted onto the halo if it always accreted baryons and dark matter in the universal proportion, but which failed to do so (e.g. perhaps due to being photoheated to a high temperature and so being resistant to accretion into shallow potential wells): $M_\mathrm{failed}$.
 \item [\mono{mass}] The mass of gas in the hot halo: $M_\mathrm{hot}$ \mono{[hotHaloMass]}.
 \item [\mono{angularMomentum}] The angular momentum of the gas in the hot halo, $J_\mathrm{hot}$ \mono{[hotHaloAngularMomentum]}.
 \item [\mono{abundances}] The mass(es) of heavy elements in gas in the hot halo, $M_{Z, \mathrm{hot}}$ \mono{[hotHalo\{abundanceName\}]}.
 \item [\mono{outflowedMass}] The mass of gas from outflows in the hot halo: $M_\mathrm{outflowed}$ \mono{[hotHaloOutflowedMass]}.
 \item [\mono{outflowedAngularMomentum}] The angular momentum of the outflowed gas in the hot halo, $J_\mathrm{outflowed}$ \mono{[hotHaloOutflowedAngularMomentum]}.
 \item [\mono{outflowedAbundances}] The mass(es) of heavy elements in outflowed gas, $M_{Z, \mathrm{outflowed}}$ \mono{[hotHaloOutflowed\{abundanceName\}]}.
 \item [\mono{chemicals}] The mass(es) of molecules in the hot gas, $M_\mathrm{chemical}$ \mono{[hotHaloChemicals\{chemicalName\}]}.
 \item [\mono{outerRadius}] The outer boundary radius of the hot halo: $r_\mathrm{hot, outer}$ \mono{[hotHaloOuterRadius]}.
 \item [\mono{strippedMass}] The mass of gas which has been stripped from the hot halo (by ram pressure or tidal forces for example): $M_\mathrm{hot, stripped}$. This property is computed only if \mono{[hotHaloTrackStrippedGas]}$=$\mono{true}.
 \item [\mono{strippedAbundances}] The mass(es) of heavy elements in gas that has been stripped from the hot halo (by ram pressure or tidal forces for example), $M_{Z, \mathrm{hot, stripped}}$. These properties are computed only if \mono{[hotHaloTrackStrippedGas]}$=$\mono{true}.
\end{description}
and the following pipes:
\begin{description}
 \item [\mono{heatSource}] Energy sent through this pipe is added to the hot halo and used to offset the cooling rate\index{hot halo!heating}\index{heating!hot halo} (see below; heat pushed should be in units if $M_\odot$ (km/s)$^2$ Gyr$^{-1}$).
 \item [\mono{cooling$[$Mass|Angular\_Momentum|Abundances$]$\_To}] The net cooling rate of gas mass (and metal content and magnitude of angular momentum) is sent through this pipe. Any \gls{component} may claim this pipe and connect to it, allowing it to receive the cooling gas.
 \item [\mono{outflowing$[$Mass|AngularMomentum|Abundances$]$\_To}] Galactic components that wish to expel gas due to an outflow can send that mass (plus metals and angular momentum) through this pipe, where it will be received into the hot halo component. 
 \item [\mono{massSink}] Removes gas (and proportionate amounts of angular momentum and elements) from the hot gas halo.
\end{description}
...
\end{equation}
where $\dot{M}_\mathrm{cool}$ is the intrinsic cooling rate in the halo (i.e. the cooling rate in the absence of any heating) and $\alpha_\mathrm{expel}=$\mono{[hotHaloExpulsionRateMaximum]} limits the maximum rate at which mass can be expelled from the halo.
...
\end{equation}
where $\alpha_\mathrm{outflow~return~rate}=($\mono{hotHaloOutflowReturnRate}) is an input parameter controlling the rate at which gas flows from the outflowed to hot reservoirs, and $\dot{M}|\dot{M}_Z|\dot{J}_\mathrm{outflows}$ are the net rates of outflow from any components in the node.
...
\end{equation}
where $\epsilon_\mathrm{strip}=$\mono{[hotHaloOutflowStrippingEfficiency]} is an input parameter, $M_\mathrm{hot, outer}$ is the mass of hot gas contained within the outer radius of the hot halo and $M_\mathrm{hot, virial}$ is the mass of hot gas that would be present if the hot halo extended to the virial radius (i.e. if no stripping had occurred).

A fraction $1-$\mono{[hotHaloAngularMomentumLossFraction]} of the cooling angular momentum rate, $\dot{M}_\mathrm{cooling} r_\mathrm{cool} V_\mathrm{rotate}$, is sent through the \mono{Hot\_Halo\_Cooling\_Angular\_Momentum} pipe.

\subsubsection{Event Evolution}

\noindent\emph{Node mergers:} If the \mono{starveSatellites} parameter is true, then any hot halo properties of the minor \gls{node} are added to those of the major \gls{node} and the hot halo \gls{component} removed from the minor node. Additionally in this case, any material outflowed or stripped from the the satellite galaxy to its hot halo is transferred to the hot halo of the host dark matter halo after each timestep. (Alternatively, if \mono{starveSatellitesOutflowed}$=$\mono{true} then only the outflowed and stripped gas is transferred to the host halo---the main hot gas reservoir is left in place.) If stripped mass is being tracked (i.e. if \mono{[hotHaloTrackStrippedGas]}$=$\mono{true}) then any stripped mass is transferred from the satellite galaxy to the hot halo of the host dark matter halo after each timestep. If \mono{[hotHaloNodeMergerLimitBaryonFraction]}$=$\mono{true} then the hot gas content of the merged node is limited such that the total baryon content of the node (including satellites) does not exceed the universal baryon fraction, if possible. Any gas removed to enforce this limit is placed into the unaccreted gas reservoir, from which is may eventually be reaccreted.\\

\noindent\emph{Satellite merging:} If the \mono{starveSatellites} parameter is false, then any hot halo properties of the satellite \gls{node} are added to those of the host \gls{node} and the hot halo \gls{component} removed from the satellite node.\\
...
\noindent\emph{Halo formation:} If \mono{[hotHaloOutflowReturnOnFormation]}$=$\mono{true} then all outflowed gas is returned to the hot gas reservoir on \href{https://github.com/galacticusorg/galacticus/releases/download/bleeding-edge/Galacticus_Development.pdf\#sec.HaloFormationEvents}{halo formation events}.\\
...
\begin{description}
 \item [\mono{trackedOutflowMass}] The mass of gas in the hot halo which arrived there directly via outflow: $M_\mathrm{outflow, track}$ \mono{[hotHaloTrackedOutflowMass]}.
 \item [\mono{trackedOutflowAbundances}] The mass of elements in the hot halo which arrived there directly via outflow: $M_{Z, \mathrm{outflow, track}}$ \mono{[hotHaloTrackedOutflowAbundances]}.
\end{description}
...
\begin{description}
 \item [\mono{massGas}] The mass of gas in the disk: $M_\mathrm{disk, gas}$ [\mono{diskMassGas}];
 \item [\mono{massStellar}] The mass of stars in the disk: $M_\mathrm{disk, stars}$ [\mono{diskMassStellar}].
\end{description}
...
This implementation extends the \mono{verySimple} disk class by adding a half-mass radius property. No other assumptions about the mass distribution are made.

\subsubsection{Properties}

The very simple size galactic disk implementation defines the following additional properties:
\begin{description}
 \item [\mono{radius}] The half-mass radius of the disk: $r_\mathrm{disk, 1/2}$ [\mono{diskRadius}].
\end{description}
...
This implementation assumes a disk with a cylindrical symmetry and a flattened profile in which stars trace gas. Currently, one option for the density profile is allowed\footnote{The disk density distribution is handled internally using a \protect\refClass{massDistributionClass} object. As such, any mass distribution implemented as an extension of the \mono{massDistribution} class (and which is described by a single length scale) could be trivially added to the standard disk component.}:
\begin{description}
\item [\mono{exponentialDisk}] Assumes an exponential profile, $\rho(R,z) = \rho_0 \exp(-r/r_\mathrm{s}) \hbox{sech}^2(z/z_\mathrm{s})$, for the disk \gls{component} of a galaxy. The thickness, $z_\mathrm{s}/r_\mathrm{s}$ is set by the parameter \mono{[heightToRadialScaleDisk]}.
\end{description}
...
\begin{description}
 \item [\mono{massGas}] The mass of gas in the disk: $M_\mathrm{disk, gas}$ [\mono{diskMassGas}].
 \item [\mono{abundancesGas}] The mass of elements in the gaseous disk: $M_{Z, \mathrm{disk, gas}}$ [\mono{diskAbundancesGas\{abundanceName\}}].
 \item [\mono{massStellar}] The mass of stars in the disk: $M_\mathrm{disk, stars}$ [\mono{diskMassStellar}].
 \item [\mono{abundancesStellar}] The mass of elements in the stellar disk: $M_{Z, \mathrm{disk, stars}}$ [\mono{diskAbundancesStellar\{abundanceName\}}].
 \item [\mono{luminositiesStellar}] The luminosities (in multiple bands) of the stellar disk: $L_\mathrm{disk, stars}$ [\mono{diskLuminositiesStellar\{luminosityName\}}].
 \item [\mono{angularMomentum}] The angular momentum of the disk, $J_\mathrm{disk}$ [\mono{diskAngularMomentum}].
 \item [\mono{radius}] The radial scale length of the disk, $R_\mathrm{disk}$ [\mono{diskRadius}].
 \item [\mono{velocity}] The circular velocity of the disk at $R_\mathrm{disk}$, $V_\mathrm{disk}$ [\mono{diskVelocity}].
 \item [\mono{massStellarFormed}] The total mass of stars ever formed in the disk: $M_\mathrm{disk, stars, formed}$ .
 \item [\mono{fractionMassRetained}] The fraction of mass retained in the disk as a result of transfer processes in which the transfer is proportional to the current mass: $f_\mathrm{retained}$.
\end{description}
...
The standard disk component supports solving for the stellar luminosities as inactive variables, if the parameter \mono{[diskLuminositiesStellarInactive]}$=$\mono{true}. Across a timestep $t_i$ to $t_{i+1}$ the change in luminosity can be written as the following integral:
\begin{eqnarray}
...
If the \mono{[diskOutputStarFormationRate]}\index{disk!star formation rate!output}\index{star formation rate!output!disk} input parameter is set to true, then the instantaneous star formation rate in the disk (in units of $M_\odot$~Gyr$^{-1}$) will be included in the output, as \mono{diskStarFormationRate}.

\subsubsection{Structure}

The radial size of the disk is found solving for equilibrium (i.e. the radius is such that the angular momentum of material at that radius is sufficient to provide rotational support) at the specified \mono{[diskStructureSolverRadius]} which is given in units of the disk scale length. In converting from the mean specific angular momentum of the disk to the angular momentum at that radius, a flat rotation curve is assumed, i.e.:
\begin{equation}
...
\end{equation}
The option \mono{[diskRadiusSolverCole2000Method]}, if set to \mono{true}, alters this behavior to match that of the structure solver used by \cite{cole_hierarchical_2000}, in which adiabatic contraction of the dark matter halo is solved for assuming that the disk has a spherical mass distribution. The specific angular momentum passed to the structure solver will be modified as follows in this case:
\begin{equation}
...
\begin{description}
 \item [\mono{massGas}] The mass of gas in the spheroid: $M_\mathrm{spheroid, gas}$ [\mono{spheroidMassGas}];
 \item [\mono{massStellar}] The mass of stars in the spheroid: $M_\mathrm{spheroid, stars}$ [\mono{spheroidMassStellar}].;
 \item [\mono{radius}] The half-mass radius of the spheroid: $r_\mathrm{spheroid, 1/2}$ [\mono{spheroidRadius}].
\end{description}
...
The standard spheroid implementation assumes a spheroid density profile described by a single length scale in which stars trace gas. Currently, two options for the density profile are allowed\footnote{The spheroid density distribution is handled internally using a \refClass{massDistributionClass} object. As such, any mass distribution implemented as an extension of the \mono{massDistribution} class (and which is described by a single length scale) could be trivially added to the standard spheroid component.}:
\begin{description}
\item [\mono{hernquist}] Assumes a Hernquist profile \citep{hernquist_analytical_1990} for the spheroidal \gls{component} of a galaxy.
\item [\mono{sersic}] Assumes a S\'ersic profile (\citealt{sersic_influence_1963}; see also \citealt{mazure_exact_2002}) for the spheroidal \gls{component} of a galaxy in which stars trace gas. The projected density profile of the spheroid is given by:
\begin{equation}
 \Sigma(R) \propto \exp\left(-b_\mathrm{n} R^{1/n} \right),
\end{equation}
where the S\'ersic index, $n=$\mono{[spheroidSersicIndex]} and the coefficient $b_\mathrm{n}=2.303(0.8689 n-0.1447)$ \cite{wadadekar_two-dimensional_1999}. The 3D density distribution for a given $n$ is inferred by solving the relevant inverse Abel integral.
\end{description}
...
\begin{description}
 \item [\mono{masGas}] The mass of gas in the spheroid: $M_\mathrm{spheroid, gas}$ [\mono{spheroidMassGas}].
 \item [\mono{abundancesGas}] The mass of elements in the gaseous spheroid: $M_{Z, \mathrm{spheroid, gas}}$ [\mono{spheroidAbundancesGas\{abundanceName\}}].
 \item [\mono{massStellar}] The mass of stars in the spheroid: $M_\mathrm{spheroid, stars}$ [\mono{spheroidMassStellar}].
 \item [\mono{abundancesStellar}] The mass of elements in the stellar spheroid: $M_{Z, \mathrm{spheroid, stars}}$ [\mono{spheroidAbundancesStellar\{abundanceName\}}].
 \item [\mono{luminositiesStellar}] The luminosities (in multiple bands) of the stellar spheroid: $L_\mathrm{spheroid, stars}$ [\mono{spheroidLuminositiesStellar\{luminosityName\}}].
 \item [\mono{angularMomentum}] The pseudo-angular momentum\footnote{Effectively the angular momentum that the spheroid would have, were it rotationally supported rather than pressure supported.} of the spheroid, $J_\mathrm{spheroid}$ [\mono{spheroidAngularMomentum}]. The parameter \mono{[spheroidAngularMomentumAtScaleRadius]} controls the ratio of the specific pseudo-angular momentum at the scale radius of the standard spheroid to the mean specific pseudo-angular momentum\index{spheroid!standard!pseudo-angular momentum}\index{spheroid!standard!radius}. By default, this parameter is set to \mono{[spheroidAngularMomentumAtScaleRadius]} $= I_2/I_3$, where
\begin{equation}
I_n = \int_0^\infty \rho(r) r^n \mathrm{d}r,
\end{equation}
and $\rho(r)$ is the spheroid density profile, which is appropriate for a flat rotation curve. In some cases (e.g. the Hernquist profile) one or both of $I_2$ and $I_3$ can be infinite. In such cases \mono{[spheroidAngularMomentumAtScaleRadius]} $=0.5$ is assumed by default. If a finite truncation radius is assumed, or a different rotation curve is assumed, this ratio may be finite. The \mono{[spheroidAngularMomentumAtScaleRadius]} parameter allows control over these assumptions.
 \item [\mono{radius}] The radial scale length of the spheroid, $r_\mathrm{spheroid}$ [\mono{spheroidRadius}].
 \item [\mono{velocity}] The circular velocity of the spheroid at $r_\mathrm{spheroid}$, $V_\mathrm{spheroid}$ [\mono{spheroidVelocity}].
 \item [\mono{massStellarFormed}] The total mass of stars ever formed in the spheroid: $M_\mathrm{spheroid, stars, formed}$.
\end{description}
and the following pipes:
\begin{description}
 \item [\mono{energyInput}] Energy sent through this pipe is added to the gas of the spheroid and will result in an outflow (see below). Input energy should be in units of $M_\odot$ km$^2$ s$^{-2}$ Gyr$^{-1}$ and must be positive (energy cannot be removed from the gas via this pipe).
 \item [\mono{massGasSink}] Removes gas (and proportionate amounts of angular momentum and elements) from the spheroid gas. Removed mass should be in units of $M_\odot$ and must be positive (a negative mass sink would add mass to the spheroid which is not allowed via this pipe).
\end{description}
...
\begin{equation}
 \Psi = {M_\mathrm{spheroid, gas} \over \tau_\mathrm{spheroid, star~formation}}
\end{equation}
with $\tau_\mathrm{spheroid, star~formation}$ being the star formation timescale and $\dot{R}$ is the rate of mass recycling from stars. If \mono{[spheroidStarFormationInSatellites]}$=$\mono{true} then the star formation rate is forced to zero in satellite galaxies. Element abundances (including total metals) evolve according to:
\begin{eqnarray}
...
The standard spheroid component supports solving for the stellar luminosities as inactive variables, if the parameter \mono{[spheroidLuminositiesStellarInactive]}$=$\mono{true}. Across a timestep $t_i$ to $t_{i+1}$ the change in luminosity can be written as the following integral:
\begin{equation}
...
If the \mono{[spheroidOutputStarFormationRate]}\index{spheroid!star formation rate!output}\index{star formation rate!output!spheroid} input parameter is set to true, then the instantaneous star formation rate in the spheroid (in units of $M_\odot$~Gyr$^{-1}$) will be included in the output, as \mono{spheroidStarFormationRate}.
...
\begin{description}
 \item [\mono{mass}] The total mass of the node: $M_\mathrm{node}$ [\mono{basicMass}].
 \item [\mono{time}] The time at which the \gls{node} is defined: $t_\mathrm{node}$.
 \item [\mono{timeLastIsolated}] The time at which the \gls{node} was last an isolated halo (i.e. not a subhalo): [\mono{basicTimeLastIsolated}].
\end{description}
...
\begin{description}
 \item [\mono{massBertschinger}] The ``Bertschinger'' mass of the node, defined as the mass enclosing a density contrast equal to that for the spherical collapse model: $M_\mathrm{node, Bertschinger}$ [\mono{basicMassBertschinger}].
 \item [\mono{accretionRateBertschinger}] The accretion rate of ``Bertschinger'' mass onto the node: $\dot{M}_\mathrm{node, Bertschinger}$ [\mono{basicAccretionRateBertschinger}].
 \item [\mono{radiusTurnaround}] The turnaround radius corresponding to the current ``Bertschinger''mass: $R_\mathrm{ta}$ [\mono{basicRadiusTurnaround}]
\end{description}

\subsubsection{Initialization}

The Bertschinger mass and accretion rate for each node are computed from $M_\mathrm{basic}$ and the dark matter density profile. The turnaround radius is computed from the virial radius (under the appropriate definition for the Bertschinger mass) and the ratio of turnaround to virial radius from spherical collapse models. The specific spherical collapse model to use is determined by the \mono{[nodeComponentBasicExtendedSphericalCollapseType]} parameter---a value of ``\mono{matterLambda}'' causes the solution for matter+cosmological constant universes to be used, while a value of ``\mono{matterDarkEnergy}'' causes the solution for matter+dark energy universes to be used.
...
\begin{description}
 \item [\mono{position}] The 3-D position of the node: $\mathbf{x}$ [\mono{positionPosition[X|Y|Z]}].
 \item [\mono{velocity}] The 3-D velocity of the node: $\mathbf{v}$ [\mono{positionVelocity[X|Y|Z]}].
 \item [\mono{positionHistory}] The history of the node's position in 6-D phase space, usually used for satellite nodes.
\end{description}
...
\noindent\emph{Node mergers:} If \mono{positionsPresetSatelliteToHost}$=$\mono{true} then the position and velocity of the satellite node is set equal to that of the host node (this is useful if position data is not available for orphaned halos for example, which would otherwise remain fixed in physical coordinates at their last known position---the position/velocity will also be updated to that of the new host each time a satellite's host changes), otherwise, none.\\
...
\begin{description}
 \item [\mono{mergeTime}] The time until the satellite will merge with its host: $t_\mathrm{satellite, merge}$ [\mono{satelliteMergeTime}].
 \item [\mono{timeOfMerging}] The cosmological time at which the satellite will merge with its host: $T_\mathrm{satellite, merge}$.
 \item [\mono{boundMass}] The remaining, total bound mass of the satellite (this property is read only---it is determined from the \mono{boundMassHistory} property).
 \item [\mono{boundMassHistory}] A history time-series of the total bound mass of the satellite.
 \item [\mono{virialOrbit}] The orbit (a \href{https://github.com/galacticusorg/galacticus/releases/download/bleeding-edge/Galacticus_Development.pdf\#sec.KeplerOrbits}\mono{keplerOrbit} object) of the satellite at virial orbit crossing.
\end{description}

Note that the \mono{mergeTime} and \mono{timeOfMerging} effectively provide the same information. For that reason, setting one of them will automatically set the other accordingly.
...
\begin{description}
 \item [\mono{timeOfMerging}] The time at which the satellite will merge with its host: $t_\mathrm{satellite, merge}$.
\end{description}
...
\begin{description}
 \item [\mono{boundMass}] The remaining, total bound mass of the satellite: $M_\mathrm{node,bound}$ [\mono{satelliteBoundMass}].
 \item[\mono{virialOrbit}] The orbit (returned as a \href{https://github.com/galacticusorg/galacticus/releases/download/bleeding-edge/Galacticus_Development.pdf\#sec.KeplerOrbits}\mono{keplerOrbit} object) of the satellite at the point of virial radius crossing.
\end{description}
...
\begin{description}
 \item [\mono{position}] The 3-dimensional position of the satellite relative to its host: $\mathbf{r}=(x,y,z)$.
 \item [\mono{velocity}] The 3-dimensional velocity of the satellite relative to its host: $\mathbf{v}=(v_x,v_y,v_z)$.
 \item [\mono{mergeTime}] The time until the satellite will merge with its host: $t_\mathrm{satellite, merge}$ [\mono{satelliteMergeTime}].
 \item [\mono{boundMass}] The remaining, total bound mass of the satellite: $M_\mathrm{node,bound}$ [\mono{satelliteBoundMass}].
 \item[\mono{virialOrbit}] The orbit (returned as a \href{https://github.com/galacticusorg/galacticus/releases/download/bleeding-edge/Galacticus_Development.pdf\#sec.KeplerOrbits}\mono{keplerOrbit} object) of the satellite at the point of virial radius crossing.
 \item[\mono{tidalTensorPathIntegrated}] The time integral of the tidal tensor along the orbit of the satellite from initialization: $G_{ij}$.
 \item[\mono{tidalHeatingNormalized}] The tidal heating energy per radius squared deposited into the satellite: $Q_\mathrm{tidal}$.
\end{description}
...
\end{eqnarray}
with $r=|\mathbf{r}|$, $\mathbf{a}_{DF}$ set to the rate given by the \mono{satelliteDynamicalFriction} (see \refPhysics{satelliteDynamicalFriction}), $g_{ij}$ being the tidal tensor, $T_\mathrm{orb}$ being the satellite's orbital period, $\dot{M}_\mathrm{node,bound}$ set to the rate given by the \mono{satelliteTidalStripping} (see \refPhysics{satelliteTidalStripping}), and $\dot{Q}_\mathrm{tidal}$ set to the rate given by the \mono{satelliteTidalHeating} (see \refPhysics{satelliteTidalHeatingRate}). Note that in the evolution of $G_{ij}$ a decay term is artificially introduced such that the integration of $g_{ij}$ is effectively over just the previous orbit (i.e. over the previous tidal shock).
...
\end{eqnarray}
with $R$ being the node's half-mass radius, $M_\mathrm{node,basic}$ being the node's initial mass, and $f_\mathrm{d}=$\mono{[satelliteOrbitingDestructionMassFraction]}, the node is considered merged and the time to merging is set to zero. The bound mass is set to the current total mass of the node. A virial orbit is selected using the \mono{virialOrbits} (see \refPhysics{virialOrbit}). \\
...
\begin{description}
 \item [\mono{spin}] The spin parameter of the halo: $\lambda$ [\mono{spinSpin}].
\end{description}

\subsubsection{Initialization}

The spin parameter of each node, if not already assigned, is selected at random from a distribution of spin parameters. This value is assigned to the earliest progenitor of the halo traced along its primary branch. The value is then propagated forward along the primary branch until the \gls{node} mass exceeds that of the \gls{node} for which the spin was selected by a factor of \mono{[randomSpinResetMassFactor]}, at which point a new spin is selected at random, and the process repeated until the end of the branch is reached. 
...
\begin{description}
 \item [\mono{spin}] The spin parameter of the halo: $\lambda$ [\mono{spinSpin}].
 \item [\mono{spinGrowthRate}] The growth rate spin parameter of the halo (in units of Gyr$^{-1}$).
\end{description}
...
The type extends the \mono{preset} spin component implementation, and so provides all properties of that implementation plus those described below.

\subsubsection{Properties}

The preset dark matter halo spin implementation defines the following properties:
\begin{description}
 \item [\mono{spinVector}] The spin vector of the halo: $\lambda$ [\mono{spinSpinVector}].
 \item [\mono{spinVectorGrowthRate}] The growth rate of the spin vector of the halo (in units of Gyr$^{-1}$).
\end{description}
...
\end{equation}
where $\alpha=$\mono{[spinVitvitskaMergerRatioExponent]}. 
...
\begin{description}
 \item [\mono{scale}] The scale length of the density profile [\mono{darkMatterProfileScale}];
 \item [\mono{scaleGrowthRate}] The growth rate of the scale length of the density profile.
\end{description}

\subsubsection{Initialization}

The scale length of each node, if not already assigned, is assigned using the concentration parameter function (see \refPhysics{darkMatterProfileConcentration}), but is not allowed to drop below \mono{[darkMatterProfileMinimumConcentration]}, such that the scale length is equal to the virial radius divided by that concentration.
...
\begin{description}
 \item [\mono{scale}] The scale length of the density profile [\mono{darkMatterProfileScale}];
 \item [\mono{scaleGrowthRate}] The growth rate of the scale length of the density profile.
 \item [\mono{shape}] A shape parameter describing the density profile [\mono{darkMatterProfileShape}];
 \item [\mono{shapeGrowthRate}] The growth rate of the shape parameter of the density profile.
\end{description}

\subsubsection{Initialization}

The scale length of each node, if not already assigned, is assigned using the concentration parameter function (see \refPhysics{darkMatterProfileConcentration}), but is not allowed to drop below \mono{[darkMatterProfileMinimumConcentration]}, such that the scale length is equal to the virial radius divided by that concentration. The shape parameter of each \gls{node} is assigned using the dark matter profile shape model (see \refPhysics{darkMatterProfileShape}).
