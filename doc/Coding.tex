...
\end{verbatim}
This can be useful to ensure that a file was generated by the exact same version of \glc. It is also possible, by setting the optional {\tt includeBuild} argument to {\tt .true.}, to append full build information (library versions, environment variables) to the label. Finally, since the returned label can be long and cumbersome, if called with {\tt asHash=.true.} the labelling function will return an \gls{MD5hash} of the label
\begin{verbatim}
