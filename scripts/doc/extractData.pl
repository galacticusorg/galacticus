...
opendir(my $directory,$sourceDir);
...
foreach my $object ( sort(keys(%objects)) ) {
	...
	print $methodsFile "\\emph{Physics model:} \\refPhysics{".$objects{$object}->{'name'}."}\n\n"
	    if ( $objects{$object}->{'isFunctionClass'} );
	...
	if ( scalar(@methodNames) > 0 || $objects{$object}->{'isFunctionClass'} ) {
		...
		print $methodsFile "\\item[]{\\normalfont \\ttfamily ";
	        ...
	        print $methodsFile latex_encode($objects{$object}->{'methods'}->{$method}->{'method'})."(";
			...
			print $methodsFile $arguments;
		    }
		} else {
		    print "Warning: missing arguments for method ".$method." of ".$object." object\n";
		}
	        print $methodsFile ")} ".$objects{$object}->{'methods'}->{$method}->{'description'}."\n";
            }
...
open(my $enumerationsFile         ,">".$outputRoot."Enumerations.tex"         );
open(my $enumerationSpecifiersFile,">".$outputRoot."EnumerationSpecifiers.tex");
foreach ( sort(keys(%enumerations)) ) {
    print $enumerationsFile "\\subsection{\\large {\\normalfont \\ttfamily ".$_."}}\\hypertarget{ht:AutoEnumerations".ucfirst($_)."}{}\\label{sec:AutoEnumerations".ucfirst($_)."}\\index{enumerations!".$_."\@{\\normalfont \\ttfamily ".$_."}}\n\n";
    print $enumerationsFile "\\begin{tabular}{rp{130mm}}\n";
    print $enumerationsFile "Description: & ".$enumerations{$_}->{'description'}." \\\\\n";
    (my $enumerationFile = $enumerations{$_}->{'file'}) =~ s/^work\/build\/(.*)\.p\.F90/$1.F90/;
    $enumerationFile =~ s/\./_/g;
    print $enumerationsFile "Provided by: & {\\normalfont \\ttfamily module} \\href{https://github.com/galacticusorg/galacticus/releases/download/masterRelease/Galacticus_Source.pdf\\#source.".$enumerationFile.":".lc($enumerations{$_}->{'module'})."}{\\normalfont \\ttfamily ".latex_encode($enumerations{$_}->{'module'})."} \\\\\n";
    my $first = 1;
    foreach my $entry ( @{$enumerations{$_}->{'entry'}} ) {
	print $enumerationsFile "Members:"
	    if ( $first == 1 );
	print $enumerationsFile " & {\\normalfont \\ttfamily ".$entry."}\\\\\n";
	$first = 0;
    }
    print $enumerationsFile          "\\end{tabular}\n";
    print $enumerationSpecifiersFile "\\def\\enum".ucfirst($_)."{\\textcolor{red}{\\hyperlink{ht:AutoEnumerations".ucfirst($_)."}{\\textless ".$_."\\textgreater}}}\n";
}
close($enumerationsFile         );
close($enumerationSpecifiersFile);
