...
# Driver script for FSPS_v2.4 (Conroy, White & Gunn stellar population code).
...
if ( $makeFile == 1 ) {
    ...
    close(pHndl);
    open(pHndl,"svn info ".$galacticusPath."aux/FSPS_v2.4 |");
    while ( my $line = <pHndl> ) {
    ...
    unless ( -e $galacticusPath."aux/FSPS_v2.4/src/galacticus_IMF.f90" ) {
 	foreach my $file ( "galacticus_IMF.f90", "imf.f90.patch", "Makefile.patch", "ssp_gen.f90.patch", "autosps.f90.patch", "qromb.f90.patch" ) {
		...
		if ( $file eq "qromb.f90.patch" );
 	    copy($galacticusPath."aux/FSPS_v2.4_Galacticus_Modifications/".$file,$todir.$file);
 	    if ( $file =~ m/\.patch$/ ) {
 	...
 	unlink($galacticusPath."aux/FSPS_v2.4/src/autosps.exe");
    }
	...
	timestamp     => $now,
	fspsVersion   => "2.4_r".$availableRevision,
	...
	my $iAge     = -1;
	my $gotAge   =  0;
	open(specFile,$galacticusPath."aux/FSPS_v2.4/OUTPUTS/".$outFile.".spec");
