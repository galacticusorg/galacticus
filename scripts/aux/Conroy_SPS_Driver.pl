...
# Driver script for FSPS_v2.5 (Conroy, White & Gunn stellar population code).
...
if ( $makeFile == 1 ) {
    ...
    unless ( -e &galacticusPath()."aux/FSPS_v2.5" ) {
 	...
 	system("git clone git://github.com/cconroy20/fsps.git/ ".&galacticusPath()."aux/FSPS_v2.5");
    ...
    my $fspsPath = &galacticusPath()."/aux/FSPS_v2.5";
    my $currentRevision = `cd $fspsPath; git rev-parse HEAD`;
    chomp($currentRevision);
    open(pHndl,"cd ".&galacticusPath()."aux/FSPS_v2.5; git status |");
    my $upToDate = 0;
    while ( my $line = <pHndl> ) {
	$upToDate = 1
 	if ( $line =~ m/Your branch is up-to-date with \'origin\/master\'/ );
    }
    close(pHndl);
    if ( $upToDate == 0 ) {
 	print "Conroy_SPS_Driver.pl: updating source code.\n";
 	system("cd ".&galacticusPath()."aux/FSPS_v2.5; git checkout -- ."); # Revert the code.
 	system("cd ".&galacticusPath()."aux/FSPS_v2.5; git pull "); # Grab updates
 	unlink(&galacticusPath()."aux/FSPS_v2.5/src/galacticus_IMF.f90") # Remove this file to trigger re-patching of the code.
    }
    
    # Patch the code.
    unless ( -e &galacticusPath()."aux/FSPS_v2.5/src/galacticus_IMF.f90" ) {
 	foreach my $file ( "galacticus_IMF.f90", "imf.f90.patch", "Makefile.patch", "ssp_gen.f90.patch", "autosps.f90.patch" ) {
	    my $todir = &galacticusPath()."aux/FSPS_v2.5/src/";
	    $todir .= "nr/"
		if ( $file eq "qromb.f90.patch" );
 	    copy(&galacticusPath()."aux/FSPS_v2.5_Galacticus_Modifications/".$file,$todir.$file);
 	    if ( $file =~ m/\.patch$/ ) {
 	...
 	unlink(&galacticusPath()."aux/FSPS_v2.5/src/autosps.exe");
    }
    
    # Build the code.
    unless ( -e &galacticusPath()."aux/FSPS_v2.5/src/autosps.exe" ) {
 	print "Conroy_SPS_Driver.pl: compiling autosps.exe code.\n";
 	system("cd ".&galacticusPath()."aux/FSPS_v2.5/src; export SPS_HOME=".&galacticusPath()."aux/FSPS_v2.5/; make clean; make -j 1");
 	die("Conroy_SPS_Driver.pl: FATAL - failed to build autosps.exe code.") unless ( -e &galacticusPath()."aux/FSPS_v2.5/src/autosps.exe" );
    }
    ...
    my $pwd = `pwd`;
    chomp($pwd);
    $ENV{'SPS_HOME'} = &galacticusPath()."aux/FSPS_v2.5/";
    my $iMetallicity = -1;
	...
	timestamp     => $now,
	fspsVersion   => "2.5_r".$currentRevision,
	...
	unless ( -e &galacticusPath()."aux/FSPS_v2.5/OUTPUTS/".$outFile.".spec" ) {
	    open(spsPipe,"|".&galacticusPath()."aux/FSPS_v2.5/src/autosps.exe");
	    print spsPipe "6\n";         # IMF.
	    print spsPipe "0\n";         # Generate SSP.
	    print spsPipe $iZ."\n";      # Specify metallicity.
	    print spsPipe "no\n";        # Do not include dust.
	...
	my $iAge     = -1;
	my $gotAge   =  0;
	open(specFile,&galacticusPath()."aux/FSPS_v2.5/OUTPUTS/".$outFile.".spec");
