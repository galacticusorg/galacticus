...
# Andrew Benson (24-March-2022).

# Specify the data directory.
die("Usage: convertBPASSv2.3SSPsToGalacticus.pl <dataDirectory>")
...
while ( my $modelName = readdir($models) ) {
    next
	unless ( $modelName =~ m/^spectra\-(bin|sin)\-imf(135_300)\.a([\+\-\d]+)\.z([\dem]+)\.dat$/ );
    my $binarity;
    my $imfLabel;
    my $alphaLabel;
    if ( $modelName =~ m/^spectra\-(bin|sin)\-imf(135_300)\.a([\+\-\d]+)\.z([\dem]+)\.dat$/ ) {
	$binarity   = $1;
	$imfLabel   = $2;
	$alphaLabel = $3;
    } else {
    ...
    my $fileName = $ENV{'GALACTICUS_DATA_PATH'}."/dynamic/stellarPopulations/SSP_Spectra_BPASSv2.3_".$binarity."-imf".$imfLabel."-alpha".$alphaLabel.".hdf5";
    next
	if ( -e $fileName );
    # Iterate over SSPs.
    ...
    for(my $iMetallicity=0;$iMetallicity<nelem($metallicities);++$iMetallicity) {
    	my $modelFileName = $dataDirectory."/spectra-".$binarity."-imf".$imfLabel.".a".$alphaLabel.".z".($metallicities->(($iMetallicity)) < 0.99e-3 ? "em".sprintf("%1.1d",-$metallicities->(($iMetallicity))->log10()->long()->sclr()) : sprintf("%3.3d",long(1000.0*$metallicities->(($iMetallicity))->sclr()))).".dat";
    	print $modelFileName."\n";
    	# Read data.
    	my $iWavelength = -1;
    ...
    my $outputFile = new PDL::IO::HDF5(">".$fileName);
    ...
    my $description = "Simple stellar population spectra from the BPASS v2.3 library";
    $description .= $binarity eq "bin" ? "including binary stars" : "not including binary stars";
    $description .= " with α-alement enhancement of Δlog₁₀(α/Fe)=".sprintf("%+4.1f",$alphaLabel/10.0);
    my $alpha = "α-alement enhancement of Δlog₁₀(α/Fe)=".sprintf("%+4.1f",$alphaLabel/10.0).".";
    my $imf;
