...
literal strings (`'files'`, `'nonIncludeDirectives'`, ...) across entries, so
...
`_scan_one` over a fixture exercising every top-level entry shape (a
functionClass directive with its implicit task dependencies), simulates the
worker round-trip, and requires the canonicalized blob to pickle
