...
def build_fortran_reassignments(argument_list, func_class, implementation,
                               extensions, module_uses_impls):
    """Generate Fortran argument reassignments."""
    # Stub: full implementation handles complex type conversions
    for arg in argument_list:
        arg.setdefault('fortran', {})
        if arg.get('intrinsic') == 'logical':
            arg['fortran']['reassignment'] = f"{arg['name']}_=logical({arg['name']})\n"
            arg['fortran']['declarations'] = f"logical :: {arg['name']}_\n"
            arg['fortran']['passAs'] = arg['name'] + '_'
...
def fortran_declarations(argument_list):
    ...
    for arg in argument_list:
        ...
        if attr_str:
            attr_str = ', ' + attr_str
        code += f"  {arg.get('fortran', {}).get('type', 'integer(c_int)')}{attr_str} :: {arg['name']}\n"
