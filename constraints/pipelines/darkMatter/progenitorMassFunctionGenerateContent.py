...
def _base_files(entry_groups,options):
    ...
    for identifier in sorted(entry_groups):
        ...
        config_likelihood += (
            ...
            f'\n'
            f'                         progenitorMassFunctionParameters/errorTreeNormalization\n'
            f'                         progenitorMassFunctionParameters/errorTreeExponentMass\n'
            f'                         progenitorMassFunctionParameters/errorTreeExponentTimeLag\n'
            f'                        "/>\n'
            )
        ...
        base += (
            ...
            f'       These particle-number errors are summed in quadrature with a model for errors arising from\n'
            f'       merger tree construction (e.g. progenitor halos incorrectly linked into a tree), which grow with\n'
            f'       the time lag between progenitor and parent:\n'
            f'         sigma_t(M) = sigma_t * (M/1e12)^gamma_t * Delta^(beta_t/2),   with Delta = ln[(1+z_progenitor)/(1+z_parent)],\n'
            f'       where sigma_t (errorTreeNormalization), gamma_t (errorTreeExponentMass), and beta_t\n'
            f'       (errorTreeExponentTimeLag) are calibrated in the MCMC. This term is zero for parent halos, and for\n'
            f'       the halo mass function. -->\n'
            f'  <nbodyHaloMassError value="summation">\n'
            ...
            f'    <nbodyHaloMassError value="treeConstruction">\n'
            f'      <normalization               value="=[progenitorMassFunctionParameters/errorTreeNormalization]"  />\n'
            f'      <exponentMass                value="=[progenitorMassFunctionParameters/errorTreeExponentMass]"   />\n'
            f'      <exponentTimeLag             value="=[progenitorMassFunctionParameters/errorTreeExponentTimeLag]"/>\n'
            f'    </nbodyHaloMassError>\n'
            f'  </nbodyHaloMassError>\n'
            f'\n'
            f'  <!-- Output control -->\n'
...
def _config_strings(options):
    ...
    active_parameters = (
        ...
        '      </distributionFunction1DPerturber>\n'
        '    </modelParameter>\n'
        # --- N-body halo mass-error (merger tree construction) parameters ---
        '    <modelParameter value="active">\n'
        '      <name value="progenitorMassFunctionParameters/errorTreeNormalization"/>\n'
        r'      <label value="\sigma_\mathrm{t}" ignoreWarnings="true"/>' + '\n'
        '      <distributionFunction1DPrior value="logUniform">\n'
        '        <limitLower value="+1.00e-4"/>\n'
        '        <limitUpper value="+1.00e+0"/>\n'
        '      </distributionFunction1DPrior>\n'
        ...
        '    </modelParameter>\n'
        '    <modelParameter value="active">\n'
        '      <name value="progenitorMassFunctionParameters/errorTreeExponentMass"/>\n'
        r'      <label value="\gamma_\mathrm{t}" ignoreWarnings="true"/>' + '\n'
        '      <distributionFunction1DPrior value="uniform">\n'
        '        <limitLower value="-1.00"/>\n'
        '        <limitUpper value="+1.00"/>\n'
        '      </distributionFunction1DPrior>\n'
        ...
        '    </modelParameter>\n'
        '    <modelParameter value="active">\n'
        '      <name value="progenitorMassFunctionParameters/errorTreeExponentTimeLag"/>\n'
        r'      <label value="\beta_\mathrm{t}" ignoreWarnings="true"/>' + '\n'
        '      <distributionFunction1DPrior value="uniform">\n'
        '        <limitLower value="+0.00"/>\n'
    ...
    if prior_log is not None:
        ...
        config_initializer += (
            ...
            '          parameters, to the Trenti et al. (2010)-equivalent error model, and to a modest\n'
            '          merger tree construction error model. Order must match the active <modelParameter>\n'
            '          declarations: G0 gamma1 gamma2 gamma3 rootVarianceTargetFractional errorAmplitude\n'
            '          errorExponent errorFractionalHighMass errorTreeNormalization errorTreeExponentMass\n'
            '          errorTreeExponentTimeLag -->\n'
            '     <position         value="0.57 0.38 -0.01 0.00 1.0e-2 0.135 -0.333 0.01 0.05 0.0 1.0"/>\n'
    ...
    parameters_opener = (
        ...
        '     <!-- Merger tree construction N-body halo mass-error model parameters (calibrated in the MCMC):\n'
        '          sigma_t(M) = sigma_t * (M/1e12)^gamma_t * Delta^(beta_t/2), with Delta = ln[(1+z_progenitor)/(1+z_parent)]. -->\n'
        '     <errorTreeNormalization      value="0.05"      /> <!-- sigma_t  : fractional error at M=1e12 and Delta=1 -->\n'
        '     <errorTreeExponentMass       value="0.0"       /> <!-- gamma_t  : power-law slope with halo mass         -->\n'
        '     <errorTreeExponentTimeLag    value="1.0"       /> <!-- beta_t   : exponent of time lag in the variance   -->\n'
    )
