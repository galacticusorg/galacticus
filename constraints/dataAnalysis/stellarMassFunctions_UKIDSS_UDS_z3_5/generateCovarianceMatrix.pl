...
# Generate a file containing the UKIDSS UDS stellar mass functions from Caputi et al. (2011) along with their covariance matrices.
...
sub observedMassFunction {
    ...
    # Determine the redshift bin in use.
    my $redshiftBin;
    if      ( $parameters->{'parameter'}->{'massFunctionCovarianceRedshiftMinimum'}->{'value'} == 3.00 ) {
	$redshiftBin = 0;
    } elsif ( $parameters->{'parameter'}->{'massFunctionCovarianceRedshiftMinimum'}->{'value'} == 3.50 ) {
	$redshiftBin = 1;
    } elsif ( $parameters->{'parameter'}->{'massFunctionCovarianceRedshiftMinimum'}->{'value'} == 4.25 ) {
	$redshiftBin = 2;
    }
    die("observedMassFunction(): unknown redshift bin")
	unless ( defined($redshiftBin) );
    
    # Read the XML data file.
    my $observed     = $xml->XMLin($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Functions_Caputi_2011.xml");
    my $dataset      = ${$observed->{'dataset'}}[$redshiftBin];
    $stellarMass     = pdl @{$dataset->{'stellarMass' }->{'datum'}};
    my $massFunction = pdl @{$dataset->{'massFunction'}->{'datum'}};
    ...
    $stellarMass  *= ($H_0/$dataset->{'stellarMass' }->{'hubble'})**$dataset->{'stellarMass' }->{'hubbleExponent'};
    $massFunction *= ($H_0/$dataset->{'massFunction'}->{'hubble'})**$dataset->{'massFunction'}->{'hubbleExponent'};
