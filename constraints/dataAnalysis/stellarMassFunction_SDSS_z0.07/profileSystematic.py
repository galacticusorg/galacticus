...
ax.set_xlabel(r'$\log_{10} M_{\star,{\tt cmodel}}$')
...
for iFit, fit in enumerate(fits):
    systematic = (
        (fit['mu0'] + fit['kappa0'] * logMassNormalized) *
        (1.0 - 1.0 / (1.0 + np.exp(-logMassNormalized / fit['beta']))) +
        (fit['mu1'] + fit['kappa1'] * logMassNormalized) *
        (       1.0 / (1.0 + np.exp(-logMassNormalized / fit['beta'])))
    )
...
plt.tight_layout()
plotFile = 'constraints/dataAnalysis/stellarMassFunction_SDSS_z0.07/profileSystematic.pdf'
