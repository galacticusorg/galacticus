...
sub generateHalosNBody {
    ...
    &CosmoSim::query("select zred,snapnum from BolshoiP.Redshifts",$workDirectory."nbodyRedshifts.csv")
    ...
    &CosmoSim::query("select x,y,z,vx,vy,vz,mass from BolshoiP.FOF where snapnum=".$snapshotNumber->($redshiftSort)->(0)->sclr()." and mass > ".$haloMassMinimum,$workDirectory."nbodyHalos.csv")
