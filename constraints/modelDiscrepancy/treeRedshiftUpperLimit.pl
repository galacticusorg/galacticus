...
die("treeRedshiftUpperLimit.pl: workDirectory must be specified in config file" ) unless ( exists($config->{'likelihood'}->{'workDirectory' }) );
die("treeRedshiftUpperLimit.pl: compilation must be specified in config file"   ) unless ( exists($config->{'likelihood'}->{'compilation'   }) );
die("treeRedshiftUpperLimit.pl: baseParameters must be specified in config file") unless ( exists($config->{'likelihood'}->{'baseParameters'}) );
...
if ( $arguments{'make'} eq "yes" ) {
    ...
    die("treeRedshiftUpperLimit.pl: failed to build Galacticus.exe")
	unless ( $? == 0 );
}
    ...
    (
	 ...
	 parameters => 
	     [
	      ...
	      {
		  name  => "mergerTreeBuildCole2000HighestRedshift",
		  value => "1.0e5"
	      },
	     ]
     }
...
# Specify the output name.
    my $modelDirectory = $workDirectory."/modelDiscrepancy/treeRedshiftUpperLimit/".$model->{'label'};
	...
	my %job =
	    (
	     ...
	     label      => "treeRedshiftUpperLimit".$model->{'label'},
...
foreach my $constraint ( @constraints ) {
    ...
    # Read the results.
    my $noUpperLimitResult = new PDL::IO::HDF5($noUpperLimitResultFileName);
    my $upperLimitResult   = new PDL::IO::HDF5($upperLimitResultFileName   );
    # Extract the results.
    my $fixedX             = $upperLimitResult  ->dataset('x'         )->get();
    my $fixedY             = $upperLimitResult  ->dataset('y'         )->get();
    my $fixedCovariance    = $noUpperLimitResult->dataset('covariance')->get();
    my $variableY          = $noUpperLimitResult->dataset('y'         )->get();
    my $variableCovariance = $noUpperLimitResult->dataset('covariance')->get();
    ...
    my $outputFile = new PDL::IO::HDF5(">".$workDirectory."/modelDiscrepancy/treeRedshiftUpperLimit/discrepancy".ucfirst($constraintDefinition->{'label'}).".hdf5");
    ...
    $outputFile->attrSet(
	description => "Model discrepancy for ".$constraintDefinition->{'name'}." due to use of upper limit to redshift in merger trees."
	);
	...
	my $plotFile = $workDirectory."/modelDiscrepancy/treeRedshiftUpperLimit/discrepancy".$label.".pdf";
