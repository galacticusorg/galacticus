...
die("stellarMassFunction_ZFOURGE_z0.350.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
$massFunctionConfig->{'analysisLabel'      } = "zfourgeStellarMassFunctionZ0.350";
...
$massFunctionConfig->{'xRange'             } = "3.0e7:3.0e11";
...
$massFunctionConfig->{'systematicParameter'} = "zfourgeStellarMassFunctionZ0.350MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5(&galacticusPath()."data/observations/massFunctionsStellar/Stellar_Mass_Function_ZFOURGE_2014_z0.20_0.50.hdf5");
