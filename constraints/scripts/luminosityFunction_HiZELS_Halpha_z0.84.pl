...
# Compute likelihood (and make a plot) for a Galacticus model given the z=0.84 Halpha luminosity function from Sobral et al. (2013;
...
die("luminosityFunction_HiZELS_Halpha_z0.84.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
$luminosityFunctionConfig->{'redshift'           } = pdl 0.84;
$luminosityFunctionConfig->{'analysisLabel'      } = "luminosityFunctionHalphaSobral2013HiZELSZ2";
...
$luminosityFunctionConfig->{'title'              } = "HiZELS H\$\\alpha\$ luminosity function at \$z \\approx 0.84\$";

# Read the observed data.
my $observations                                  = new PDL::IO::HDF5(&galacticusPath()."data/observations/luminosityFunctions/hAlphaLuminosityFunctionSobral2013HiZELSZ0.84.hdf5");
