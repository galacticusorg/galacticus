...
die("stellarMassFunction_ULTRAVISTA_z0.35.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 0;
$massFunctionConfig->{'redshift'           } = pdl 0.35;
$massFunctionConfig->{'analysisLabel'      } = "massFunctionStellarMuzzin2013ULTRAVISTAz0";
...
$massFunctionConfig->{'xRange'             } = "1.0e8:3.0e12";
...
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 0.35\$";
$massFunctionConfig->{'systematicParameter'} = "ultravistaStellarMassFunctionZ0.35MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5(&galacticusPath()."data/observations/massFunctionsStellar/Stellar_Mass_Function_ULTRAVISTA_2013_z0.20_0.50.hdf5");
