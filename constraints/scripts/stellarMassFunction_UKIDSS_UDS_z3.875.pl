...
# http://adsabs.harvard.edu/abs/2011MNRAS.413..162C) for z=3.50 to 4.25.

# Data structure to hold the specification for our mass function.
my $massFunctionConfig;

# Get name of input and output files.
die("stellarMassFunction_UKIDSS_UDS_z3.875.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 0;
$massFunctionConfig->{'redshift'           } = pdl 3.875;
$massFunctionConfig->{'analysisLabel'      } = "ukidssUdsStellarMassFunctionZ3.875";
...
$massFunctionConfig->{'yRange'             } = "2.0e-7:3.0e-4";
$massFunctionConfig->{'xLabel'             } = "\$M_\\star\$ [\$M_\\odot\$]";
$massFunctionConfig->{'yLabel'             } = "\${\\rm d}n/{\\rm d}\\log M_\\star\$ [Mpc\$^{-3}\$]";
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 3.875\$";
$massFunctionConfig->{'systematicParameter'} = "ukidssUdsStellarMassFunctionZ3.875MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Function_UKIDSS_UDS_2011_z3.5_4.25.hdf5");
