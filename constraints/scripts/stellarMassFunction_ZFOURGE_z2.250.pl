...
die("stellarMassFunction_ZFOURGE_z2.250.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 0;
$massFunctionConfig->{'redshift'           } = pdl 2.250;
$massFunctionConfig->{'analysisLabel'      } = "zfourgeStellarMassFunctionZ2.250";
...
$massFunctionConfig->{'yRange'             } = "3.0e-6:1.0e-1";
$massFunctionConfig->{'xLabel'             } = "\$M_\\star\$ [\$M_\\odot\$]";
$massFunctionConfig->{'yLabel'             } = "\${\\rm d}n/{\\rm d}\\log M_\\star\$ [Mpc\$^{-3}\$]";
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 2.250\$";
$massFunctionConfig->{'systematicParameter'} = "zfourgeStellarMassFunctionZ2.250MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Function_ZFOURGE_2014_z2.00_2.50.hdf5");
