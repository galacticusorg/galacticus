...
die("stellarMassFunction_ZFOURGE_z0.875.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 0;
$massFunctionConfig->{'redshift'           } = pdl 0.875;
$massFunctionConfig->{'analysisLabel'      } = "zfourgeStellarMassFunctionZ0.875";
...
$massFunctionConfig->{'xRange'             } = "1.0e8:3.0e11";
$massFunctionConfig->{'yRange'             } = "3.0e-5:1.0e-1";
$massFunctionConfig->{'xLabel'             } = "\$M_\\star\$ [\$M_\\odot\$]";
$massFunctionConfig->{'yLabel'             } = "\${\\rm d}n/{\\rm d}\\log M_\\star\$ [Mpc\$^{-3}\$]";
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 0.875\$";
$massFunctionConfig->{'systematicParameter'} = "zfourgeStellarMassFunctionZ0.875MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Function_ZFOURGE_2014_z0.75_1.00.hdf5");
