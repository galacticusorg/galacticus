...
die("stellarMassFunction_ULTRAVISTA_z1.75.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 0;
$massFunctionConfig->{'redshift'           } = pdl 1.75;
$massFunctionConfig->{'analysisLabel'      } = "massFunctionStellarMuzzin2013ULTRAVISTAz3";
...
$massFunctionConfig->{'xRange'             } = "3.0e9:1.0e12";
$massFunctionConfig->{'yRange'             } = "1.0e-7:3.0e-2";
$massFunctionConfig->{'xLabel'             } = "\$M_\\star\$ [\$M_\\odot\$]";
$massFunctionConfig->{'yLabel'             } = "\${\\rm d}n/{\\rm d}\\log M_\\star\$ [Mpc\$^{-3}\$]";
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 1.75\$";
$massFunctionConfig->{'systematicParameter'} = "ultravistaStellarMassFunctionZ1.75MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5(&galacticusPath()."data/observations/massFunctionsStellar/Stellar_Mass_Function_ULTRAVISTA_2013_z1.50_2.00.hdf5");
