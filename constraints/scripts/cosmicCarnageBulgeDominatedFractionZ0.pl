...
# Compute likelihood (and make a plot) for a Galacticus model given the bulge-dominated fraction
...
die("cosmicCarnageBulgeDominatedFractionZ0.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
$fractionFunctionConfig->{'analysisLabel'                   } = "carnageBulgeDominatedFractionFunctionZ0";
...
$fractionFunctionConfig->{'yLabel'                          } = "\$f_{\\rm bulge-dominated}\$ []";
$fractionFunctionConfig->{'title'                           } = "Bulge-dominated fraction function at \$z=0\$";

# Read the observed data.
my $observations = new PDL::IO::HDF5($galacticusPath."data/observations/cosmicCarnageWorkshop/bulgeDominatedFractionZ0.hdf5");
