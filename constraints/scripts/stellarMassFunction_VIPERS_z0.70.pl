...
die("stellarMassFunction_VIPERS_z0.70.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 0;
$massFunctionConfig->{'redshift'           } = pdl 0.70;
$massFunctionConfig->{'analysisLabel'      } = "vipersStellarMassFunctionZ0.70";
...
$massFunctionConfig->{'xRange'             } = "3.0e9:3.0e12";
$massFunctionConfig->{'yRange'             } = "1.0e-7:1.0e-1";
$massFunctionConfig->{'xLabel'             } = "\$M_\\star\$ [\$M_\\odot\$]";
$massFunctionConfig->{'yLabel'             } = "\${\\rm d}n/{\\rm d}\\log M_\\star\$ [Mpc\$^{-3}\$]";
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 0.70\$";
$massFunctionConfig->{'systematicParameter'} = "vipersStellarMassFunctionZ0.70MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Function_VIPERS_2013_z0.70.hdf5");
