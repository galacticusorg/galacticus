...
die("stellarMassFunction_ULTRAVISTA_z2.75.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 0;
$massFunctionConfig->{'redshift'           } = pdl 2.75;
$massFunctionConfig->{'analysisLabel'      } = "massFunctionStellarMuzzin2013ULTRAVISTAz5";
...
$massFunctionConfig->{'xRange'             } = "3.0e10:1.0e12";
$massFunctionConfig->{'yRange'             } = "1.0e-7:3.0e-3";
$massFunctionConfig->{'xLabel'             } = "\$M_\\star\$ [\$M_\\odot\$]";
$massFunctionConfig->{'yLabel'             } = "\${\\rm d}n/{\\rm d}\\log M_\\star\$ [Mpc\$^{-3}\$]";
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 2.75\$";
$massFunctionConfig->{'systematicParameter'} = "ultravistaStellarMassFunctionZ2.75MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5(&galacticusPath()."data/observations/massFunctionsStellar/Stellar_Mass_Function_ULTRAVISTA_2013_z2.50_3.00.hdf5");
