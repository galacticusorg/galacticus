...
# Compute likelihood (and make a plot) for a Galacticus model given the PRIMUS z=0.900 stellar mass function data from
...
die("stellarMassFunction_PRIMUS_z0.900.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 6;
$massFunctionConfig->{'redshift'           } = pdl 0.900;
$massFunctionConfig->{'analysisLabel'      } = "primusStellarMassFunctionZ0.900";
...
$massFunctionConfig->{'xRange'             } = "3.0e10:2.0e12";
...
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 0.900\$";

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Function_PRIMUS_z0.80_1.00.hdf5");
