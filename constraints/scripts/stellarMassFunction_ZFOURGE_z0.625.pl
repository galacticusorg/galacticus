...
die("stellarMassFunction_ZFOURGE_z0.625.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 0;
$massFunctionConfig->{'redshift'           } = pdl 0.625;
$massFunctionConfig->{'analysisLabel'      } = "zfourgeStellarMassFunctionZ0.625";
...
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 0.625\$";
$massFunctionConfig->{'systematicParameter'} = "zfourgeStellarMassFunctionZ0.625MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Function_ZFOURGE_2014_z0.50_0.75.hdf5");
