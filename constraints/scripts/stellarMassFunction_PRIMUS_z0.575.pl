...
# Compute likelihood (and make a plot) for a Galacticus model given the PRIMUS z=0.575 stellar mass function data from
...
die("stellarMassFunction_PRIMUS_z0.575.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 4;
$massFunctionConfig->{'redshift'           } = pdl 0.575;
$massFunctionConfig->{'analysisLabel'      } = "primusStellarMassFunctionZ0.575";
...
$massFunctionConfig->{'xRange'             } = "3.0e9:2.0e12";
...
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 0.575\$";

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Function_PRIMUS_z0.50_0.65.hdf5");
