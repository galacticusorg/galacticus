...
# Compute likelihood (and make a plot) for a Galacticus model given the PRIMUS z=0.450 stellar mass function data from
...
die("stellarMassFunction_PRIMUS_z0.450.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 3;
$massFunctionConfig->{'redshift'           } = pdl 0.450;
$massFunctionConfig->{'analysisLabel'      } = "primusStellarMassFunctionZ0.450";
...
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 0.450\$";

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Function_PRIMUS_z0.40_0.50.hdf5");
