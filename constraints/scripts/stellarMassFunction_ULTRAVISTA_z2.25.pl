...
die("stellarMassFunction_ULTRAVISTA_z2.25.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 0;
$massFunctionConfig->{'redshift'           } = pdl 2.25;
$massFunctionConfig->{'analysisLabel'      } = "massFunctionStellarMuzzin2013ULTRAVISTAz4";
...
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 2.25\$";
$massFunctionConfig->{'systematicParameter'} = "ultravistaStellarMassFunctionZ2.25MassSystematic";
$massFunctionConfig->{'systematicOrder'    } = 1;
$massFunctionConfig->{'systematicZeroPoint'} = 11.3;

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5(&galacticusPath()."data/observations/massFunctionsStellar/Stellar_Mass_Function_ULTRAVISTA_2013_z2.00_2.50.hdf5");
