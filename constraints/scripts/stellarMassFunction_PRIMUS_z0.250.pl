...
# Compute likelihood (and make a plot) for a Galacticus model given the PRIMUS z=0.250 stellar mass function data from
...
die("stellarMassFunction_PRIMUS_z0.250.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
my $entry                                    = 1;
$massFunctionConfig->{'redshift'           } = pdl 0.250;
$massFunctionConfig->{'analysisLabel'      } = "primusStellarMassFunctionZ0.250";
...
$massFunctionConfig->{'yRange'             } = "3.0e-5:1.0e-2";
$massFunctionConfig->{'xLabel'             } = "\$M_\\star\$ [\$M_\\odot\$]";
$massFunctionConfig->{'yLabel'             } = "\${\\rm d}n/{\\rm d}\\log M_\\star\$ [Mpc\$^{-3}\$]";
$massFunctionConfig->{'title'              } = "Stellar mass function at \$z\\approx 0.250\$";

# Read the observed data.
my $observations                                      = new PDL::IO::HDF5($galacticusPath."data/observations/massFunctionsStellar/Stellar_Mass_Function_PRIMUS_z0.20_0.30.hdf5");
