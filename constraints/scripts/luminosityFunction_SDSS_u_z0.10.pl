...
# Compute likelihood (and make a plot) for a Galacticus model given the u-band luminosity function from Montero-Dorta & Prada
...
die("luminosityFunction_SDSS_u_z0.10.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
$luminosityFunctionConfig->{'analysisLabel'      } = "luminosityFunctionMonteroDorta2009SDSSu";
$luminosityFunctionConfig->{'discrepancyFileName'} = "discrepancy".ucfirst($luminosityFunctionConfig->{'analysisLabel'}).".hdf5";
$luminosityFunctionConfig->{'xRange'             } = "-22.0:-15.0";
$luminosityFunctionConfig->{'yRange'             } = "1.0e-8:1.0e-1";
$luminosityFunctionConfig->{'xLabel'             } = "\$M_\\mathrm{0.1u}\$";
$luminosityFunctionConfig->{'yLabel'             } = "\$\\mathrm{d}n/\\mathrm{d}M_\\mathrm{0.1u}\$ [Mpc\$^{-3}\$]";
$luminosityFunctionConfig->{'title'              } = "SDSS u-band luminosity function at \$z = 0.1\$";

# Read the observed data.
my $observations                                  = new PDL::IO::HDF5(&galacticusPath()."data/observations/luminosityFunctions/uLuminosityFunctionMonteroDorta2009SDSS.hdf5");
