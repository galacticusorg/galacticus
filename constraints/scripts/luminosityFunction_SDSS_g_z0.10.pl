...
# Compute likelihood (and make a plot) for a Galacticus model given the g-band luminosity function from Montero-Dorta & Prada
...
die("luminosityFunction_SDSS_g_z0.10.pl <galacticusFile> [options]") unless ( scalar(@ARGV) >= 1 );
...
$luminosityFunctionConfig->{'analysisLabel'      } = "luminosityFunctionMonteroDorta2009SDSSg";
$luminosityFunctionConfig->{'discrepancyFileName'} = "discrepancy".ucfirst($luminosityFunctionConfig->{'analysisLabel'}).".hdf5";
$luminosityFunctionConfig->{'xRange'             } = "-23.0:-15.9";
$luminosityFunctionConfig->{'yRange'             } = "1.0e-8:1.0e-1";
$luminosityFunctionConfig->{'xLabel'             } = "\$M_\\mathrm{0.1g}\$";
$luminosityFunctionConfig->{'yLabel'             } = "\$\\mathrm{d}n/\\mathrm{d}M_\\mathrm{0.1g}\$ [Mpc\$^{-3}\$]";
$luminosityFunctionConfig->{'title'              } = "SDSS g-band luminosity function at \$z = 0.1\$";

# Read the observed data.
my $observations                                  = new PDL::IO::HDF5(&galacticusPath()."data/observations/luminosityFunctions/gLuminosityFunctionMonteroDorta2009SDSS.hdf5");
