...
while ( $failCount > 0 ) {
	...
	if ( exists($parameters[$i]->{'define'}) ) {
	    ...
	    if ( $parameters[$i]->{'define'} =~ m/\%([a-zA-Z0-9_]+)/ ) {
		while ( $parameters[$i]->{'define'} =~ m/\%([a-zA-Z0-9_]+)/ ) {
		    my $parameterName = $1;
		    if ( exists($parameterValues{$parameterName}) && $parameterValues{$parameterName} !~ m/\%([a-zA-Z0-9_]+)/ ) {
...
$runCommand .= " --diagonalize ".$config->{'diagonalize'} if ( exists($config->{'diagonalize'}) );
system($runCommand);
