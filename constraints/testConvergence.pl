...
die("testConvergence.pl: baseParameters must be specified in config file") unless ( exists($config->{'baseParameters'}) );
...
foreach my $constraint ( @constraints ) {
	...
	my $plotFileName = $workDirectory."/convergence/".$constraintDefinition->{'label'}."_".$convergence->{'parameter'};
	$plotFileName =~ s/\./_/g;
	$plotFileName .= ".pdf";
    ...
    open(my $report,">".$workDirectory."/convergence/".$constraintDefinition->{'label'}."Report.txt");
